    ...
    public function getOrganizationalDocumentationManageHtm(){
        ...
        if($acm->hasAccess('warrantyManage')) {
            ...
            if($acm->hasAccess('editWarranty')){
                ...
                $b++;
            if($acm->hasAccess('deleteWarranty')){
                $bottons2[$b]['title'] = "ابطال تضمین";
                $bottons2[$b]['jsf'] = "deleteWarranty";
                $bottons2[$b]['icon'] = "fa-minus-square";
                $b++;
            }
            if($acm->hasAccess('endWarranty')){
...
/*        $items[$c]['type'] = "text";
    ...
    public function editWarranty($cid,$w_title,$w_acc_type,$w_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc,$warranty_cost){
        $acm = new acm();
        if(!$acm->hasAccess('WarrantyManage')){
            die("access denied");
            exit;
        ...
        $sql = "UPDATE `sale_warranty` SET `warranty_title`='{$w_title}',`warranty_type`='{$w_type}',`account_type`='{$w_acc_type}',`account_id`='{$w_account_id}',`doc_owner_name`='{$w_doc_owner}',`doc_owner_nat_id`='{$w_doc_owner_nat_id}',`warranty_cost`=$warranty_cost,`doc_other_info`='{$w_doc_info}',`access_id`='{$w_access_id}',`start_date`='{$SDate}',`end_date`='{$EDate}',`desc`='{$desc}' where RowID={$cid} ";
        $ut->fileRecorder($sql);
        $db->Query($sql);
        $res = $db->AffectedRows();
        $res = (($res == -1 || $res == 0) ? 0 : 1);
    ...
    public function createWarranty($w_title,$w_acc_type,$w_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc,$warranty_cost){
        $acm = new acm();
        if(!$acm->hasAccess('WarrantyManage')){
            die("access denied");
            exit;
        ...
        $sql = "INSERT INTO `sale_warranty` (`warranty_title`,`warranty_type`,`account_type`,`account_id`,`doc_owner_name`,`doc_owner_nat_id`,`warranty_cost`,`doc_other_info`,`access_id`,`start_date`,`end_date`,`desc`) 
                                    VALUES ('{$w_title}','{$w_type}','{$w_acc_type}','{$w_account_id}','{$w_doc_owner}','{$w_doc_owner_nat_id}','{$warranty_cost}','{$w_doc_info}','{$w_access_id}','{$SDate}','{$EDate}','{$desc}')";
        $ut->fileRecorder($sql);
        $res = $db->Query($sql);
        if (intval($res) > 0){
            return true;
    ...
    public function getWarrantyManageListCountRows($Name,$Code,$SDate,$EDate,$status,$type){
        ...
        $w = array();
        if(strlen(trim($Name)) > 0){
            $w[] = '`warranty_title` LIKE "%'.$warranty_title.'%" ';
        }
        ...
        foreach($result_array as $key=>$value){
            ...
            switch($key){
                ...
                case "end_date":
                    ...
                    break;
                case "warranty_cost":
                        $result=number_format($res[0][$key])." ریال"; 
                    break;
            }
            $html.="<tr><td>".$value."</td><td>".$result."</td></tr>";
        }
        $html_header='<table class="table table-borderd '.$class.'">';
        $html.="<tr><td>پیوست ها</td><td><button class='btn btn-primary' onclick='show_attachments(".$cid.")'><i class='fa fa-download'></i></button></td></tr></table>";
    ...
    public function getWarrantyInfo($cid){
        ...
        $acm=new acm();
        if($acm->hasAccess('EditWarranty')){
            $manage_has_access_edit=1;
            if($acm->hasAccess('EditLastConfirmedWarranty')){
                $manage_has_access_edit=2;
            }
        }
        else{
            $manage_has_access_edit=0;
        }
        ...
        if(count($res) == 1){
            $res = array("cid"=>$cid,
                        ...
                        "desc"=>$res[0]['desc'],
                        "status"=>$res[0]['status'],
                        "has_access_manage"=>$manage_has_access_edit
                    );
            return $res;
        }else{
            return false;
    ...
    public function attachedWarrantyFileHtm($cid){
        ...
        $cnt = count($res);
        $iterator = 0;
        $w_status="SELECT `status` FROM `sale_warranty` ";
        $w_res=$db->ArrayQuery($w_status);
        $w_status=$res[0]['status'];
        $htm = '';
        ...
        $htm .= '<td style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 55%;">نام فایل</td>';
        $htm .= '<td style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 20%;">لینک دانلود</td>';
        if($acm->hasAccess('delete_warranty_files')){
            if($w_status==10){
                $htm .= '<td style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 20%;">حذف فایل</td>';
            }
            else{
                if($acm->hasAccess('DeleteAllWarrantyAttachments')){
                    $htm .= '<td style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 20%;">حذف فایل</td>';
                }
            }
