...
class Events{
    ...
    public function calculate_hugging_end_date_alarm(){
        $acm = new acm();
        if(!$acm->hasAccess('eventsManage')){
            die("false");
        }
        $db = new DBi();
        $sql = "SELECT `RowID` FROM `car_info_meta` WHERE CURDATE() >= (`value` - INTERVAL 20 DAY) AND `status`=1 AND `group`='hugging' AND `key`='hugging_end_date' AND `type`='str'";
        $res = $db->ArrayQuery($sql);
    ...
    public function calculatePaymentMaturity(){
        ...
        $db = new DBi();
            $sql = "SELECT `RowID` ,Amount,IFNULL(d.dAmount, 0) AS dAmount ,IFNULL(b.check_amount,0) as check_amount ,(ifnull(Amount,0)-ifnull(dAmount,0)-ifnull(check_amount,0)) as remine from pay_comment as p 
LEFT JOIN(select sum(dAmount) as dAmount,pid from  deposit  GROUP BY pid) as d on d.pid=p.RowID
LEFT JOIN(select sum(check_amount) as check_amount,cid from  bank_check  GROUP BY cid) as b on b.cid=p.RowID
where p.isEnable=1 AND sendType <>3 AND CURRENT_DATE() > IFNULL(p.paymentMaturityCash,paymentMaturityCheck) AND  CURRENT_DATE() > IFNULL(paymentMaturityCheck,p.paymentMaturityCash)  AND  CURRENT_DATE() > IFNULL(p.paymentMaturityCash,paymentMaturityCheck) AND  CURRENT_DATE() > IFNULL(paymentMaturityCheck,p.paymentMaturityCash)    AND (Amount-ifnull(d.dAmount,0)-ifnull(b.check_amount,0))>0";
//(CURRENT_DATE >= p.paymentMaturityCash OR CURRENT_DATE >=p.paymentMaturityCheck) AND (ifnull(Amount,0)-ifnull(dAmount,0)-ifnull(check_amount,0))>0 ";
    ...
    public function showPaymentMaturity(){
        ...
        $sql = "SELECT p.`RowID` ,p.Amount,p.unCode,p.cDate,p.CashSection,p.NonCashSection,p.accName,p.Toward,p.paymentMaturityCash,p.paymentMaturityCheck,
        IFNULL(d.dAmount, 0) AS dAmount ,IFNULL(b.check_amount,0) as check_amount ,(ifnull(Amount,0)-ifnull(dAmount,0)-ifnull(check_amount,0)) as remine from pay_comment as p 
LEFT JOIN(select sum(dAmount) as dAmount,pid from  deposit  GROUP BY pid) as d on d.pid=p.RowID
LEFT JOIN(select sum(check_amount) as check_amount,cid from  bank_check  GROUP BY cid) as b on b.cid=p.RowID
where p.isEnable=1  AND sendType <>3  AND CURRENT_DATE() > IFNULL(p.paymentMaturityCash,paymentMaturityCheck) AND  CURRENT_DATE() > IFNULL(paymentMaturityCheck,p.paymentMaturityCash)  AND  CURRENT_DATE() > IFNULL(p.paymentMaturityCash,paymentMaturityCheck) AND  CURRENT_DATE() > IFNULL(paymentMaturityCheck,p.paymentMaturityCash)    AND (Amount-ifnull(d.dAmount,0)-ifnull(b.check_amount,0))>0";// (CURRENT_DATE >= p.paymentMaturityCash OR CURRENT_DATE >=p.paymentMaturityCheck) AND (ifnull(Amount,0)-ifnull(dAmount,0)-ifnull(check_amount,0))>0 ";
        $res = $db->ArrayQuery($sql);
        $Countevent = count($res);
        $iterator = 0;
        $htm = '';
        ...
        $htm .= "<td style='color: #ffc107;width:10%;text-align: center;'>سایر اطلاعات</td>";
        $htm .= "</tr>";
        $htm .= '</thead>';
        $htm .= '<tbody>';
        for($y=0;$y<$Countevent;$y++){
            ...
            $htm .= "<td style='text-align: center;'>".(!empty($res[$y]['CashSection'])?number_format($res[$y]['CashSection'])."ریال":0)."</td>";
            $htm .= "<td style='text-align: center;'>".(!empty($res[$y]['NonCashSection'])?number_format($res[$y]['NonCashSection'])."ریال" :0)."</td>";
            ...
            $htm .= " <td><div class='btn-report-box'><button onclick=\"ShowOtherInfoComment({$res[$y]['RowID']})\" title='مشاهده اظهارنظر' class='btn btn-secondary btn_report'><i class='fa fa-tv'></i></button><button title='مشاهده گردش کار' class='btn_report btn btn-info' onclick=\"ShowWorkflowComment({$res[$y]['RowID']})\"><i class='fa fa-history'></i> </button></div></td>";
            $htm .="</tr>";
        }
        $htm .= '</tbody>';
        $htm .= "</table>";
    ...
    public function showWarrantyAlarmModal(){
        ...
        if($Countevent>0){
            ...
            $htm .= '<thead>';
            $htm .= "<tr>";
            $htm .= "<td style='width:5%;text-align:  center;color:#fff'>ردیف</td>";
            $htm .= "<td style='width:10%;text-align: center;color:#fff'>نوع تضمین</td>";
            $htm .= "<td style='width:10%;text-align: center;color:#fff'>تاریخ شروع</td>";
            $htm .= "<td style='width:10%;text-align: center;color:#fff'>تاریخ پایان</td>";
            $htm .= "<td style='width:15%;text-align: center;color:#fff'>عنوان تضمین</td>";
            $htm .= "<td style='width:20%;text-align: center;color:#fff'>نام طرف حساب </td>";
            $htm .= "<td style='width:30%;text-align: center;color:#fff'>توضیحات</td>";
            $htm .= "</tr>";
            $htm .= '</thead>';
            $htm .= '<tbody>';
            for($y=0;$y<$Countevent;$y++){
    ...
    public function showPendingWarrantyAlarmModal(){
        $acm = new acm();
        if(!$acm->hasAccess('eventsManage')){
            die("false");
        }
        $ut = new Utility();
        $db = new DBi();
        $sql = "SELECT * FROM `sale_warranty` WHERE  `status`=10 ";
        ...
        if($Countevent>0){
            ...
            $htm .= "<td style='width:10%;text-align: center;color:#fff'>تاریخ پایان</td>";
            $htm .= "<td style='width:15%;text-align: center;color:#fff'>عنوان تضمین</td>";
            $htm .= "<td style='width:15%;text-align: center;color:#fff'>نام طرف حساب </td>";
            $htm .= "<td style='width:20%;text-align: center;color:#fff'>توضیحات</td>";
            $htm .= "<td style='width:7%;text-align:  center;color:#fff'>فایل پیوست</td>";
            $htm .= "<td style='width:8%;text-align:  center;color:#fff'> تایید نهایی</td>";
            $htm .= "</tr>";
            $htm .= '</thead>';
            $htm .= '<tbody>';
            for($y=0;$y<$Countevent;$y++){
                ...
                $acc_user_array=explode(',',$res[$y]['access_id']);
                $dl_permossion=in_array($user_id,$acc_user_array)?1:0;
                $start_date = $ut->greg_to_jal($res[$y]['start_date']);
                $end_date = $ut->greg_to_jal($res[$y]['end_date']);
                $htm .= "<tr>";
                $htm .= "<td style='text-align: center;'>".$iterator."</td>";
                ...
                $htm .= "<td style='text-align: center;'>".$this->get_account_name($res[$y]['account_id'])."</td>";
                $htm .= "<td style='text-align: center;'>".$res[$y]['desc']."</td>";
                if($dl_permossion==1){ $dl_btn="<button class='btn btn-primary'><i class=' fa fa-download' onclick='show_attachments(".$res[$y]['RowID'].")'></i></button>";}
                else{
                    $dl_btn="<button disabled class='btn btn-danger disabled'><i  class=' fa fa-download'></i></button>";
                }
                $htm .= "<td style='text-align: center;'>".$dl_btn."</td>";
                if($acm->hasAccess('saleWarrantyFinalConfirm')){
                    $f_conf_btn="<button class='btn btn-success' onclick='final_confirm_sale_warranty(".$res[$y]['RowID'].")'><i class=' fa fa-check'></i></button>";
                }
                else{
                    $f_conf_btn="<button disabled class='btn btn-success disabled' ><i class=' fa fa-check'></i></button>";
                }
                $htm .= "<td style='text-align: center;'>".$f_conf_btn."</td>";
                $htm .="</tr>";
            }
            $htm .= '</tbody>';
            $htm .= "</table>";
    ...
    public function final_confirm_sale_warranty($wid){
        $db=new DBi();
        $ut=new Utility();
        $current_date=date('Y-m-d H:i:s');
        $sql="UPDATE  `sale_warranty` set `status`=1 WHERE `status`=10  AND `RowID`=".$wid; 
        $res=$db->Query($sql);
        if($res){
            $ins_meta_query="INSERT INTO sale_warranty_meta (`w_id`,`key`,`value`,`desc`)VALUES(
            ('{$cid}','user_confirmed','{$_SESSION['userid']}','کاربر تایید کننده'),
            ('{$cid}','delete_date_time','{$current_date}','تاریخ و زمان تایید')";
            $res1=$db->Query($ins_meta_query);
            $result=$this->showPendingWarrantyAlarmModal();
    ...
    function get_sale_warranty_files($w_id){
        $db=new DBi();
        $sql="SELECT `fileName` FROM warranty_attachment where cid={$w_id} AND `status`=1";
        $res=$db->ArrayQuery($sql);
        if(count($res)>0){
            return count($res);
        }
