...
class Inquires {
    ...
    public function do_inq_primary_confirm($arr){
        $ut=new Utility();
        $db=new DBi();
        extract($arr);
        if($inq_primary_confirm==1){
            $status=1;
            $history=array("approver_id"=>4,"approve_date"=>date("Y-m-d H:i:s"),'desc'=>'درخواست شفاهی استعلام مورد تایید می باشد');
        }
        else{
            $status=0;
            $history=array("approver_id"=>4,"approve_date"=>date("Y-m-d H:i:s"),'desc'=>'درخواست شفاهی استعلام مورد تایید نمی باشد');
        }
        $history=json_encode($history,JSON_UNESCAPED_SLASHES ,JSON_UNESCAPED_UNICODE );
        $sql="UPDATE `inquiries` set `status`={$status},`history`='{$history}' WHERE `RowID`={$inq_id}";
    ...
    public function set_first_inq($arr){
        $ut=new Utility();
        $db=new DBi();
        extract($arr);
        $sql="UPDATE `inquired_goods` set parent_id=-1 where good_id={$good_id} AND inq_id={$inq_id}";
        $res=$db->Query($sql);
        return $res;
    }
    ...
    public function save_last_inq($array){
        ...
        $ultimate_rent=
        $insert_seller_detailes="INSERT INTO `inq_sellers` (`inq_good_tbl_id`,`inquiry_sellers_tbl_id`,`is_vat`,`rent_percent`,`unit_price`,`pay_term`,`price_valid`,`rent`,`ultimate_price`,`ultimate_rent`,`status`,`description`)
                        VALUES('{$last_good_row_id}','{$last_seller_tbl_id}',{$is_vat},{$rent_percent},{$unit_price},{$pay_term},{$price_valid},{$rent},{$last_ultimat_price_user},$ultimate_rent,20,'ثبت آخرین استعلام ')";
       $res=$db->Query($insert_seller_detailes);
       ////$ut->fileRecorder('insert_seller_detailes:'.$insert_seller_detailes);
       return true;
    ...
    public function get_last_inq_data($array){
        ...
        $sql="SELECT `parent_id` from inquired_goods where good_id={$good_id} AND inq_id={$inq_id}";
        $res=$db->ArrayQuery($sql);
        if($res[0]['parent_id']==-1){
           // ////$ut->fileRecorder('tessssssssssssssssssssssssssssssssssssstttttttttppppppppppppppppppppppppppppppppppppppp');
            return array('parent_id'=>-1);
        }
        else{
        ...
        $sql_1="SELECT a.`ultimate_price`,a.`pay_term`,d.`Name`,e.`inquiry_date` from `inq_sellers`  as a 
            LEFT JOIN inquired_goods as b on a.`inq_good_tbl_id`=b.`RowID`
            LEFT JOIN inquiry_sellers as c on c.`RowID`=a.`inquiry_sellers_tbl_id`
            LEFT JOIN account as d on c.`seller_id`=d.`RowID`
            LEFT JOIN inquiries as e  on e.`RowID`=b.`inq_id`
            where a.`status`=20 AND b.`good_id`={$good_id}";
        $res1=$db->ArrayQuery($sql_1);
      
        ////$ut->fileRecorder($sql_1);
        if(count($res1)>0)
        {
           $res1[0]['inq_date']=$ut->greg_to_jal($res1[0]['inquiry_date']);
    ...
    public function get_inq_detailes_info($RowID){
        ...
        $res=$db->ArrayQuery($query);
        $query_2="SELECT * FROM  `inq_pay_method` where `inq_seller_tbl_id`={$RowID}";
        
        $res2=$db->ArrayQuery($query_2);
        for($i=0;$i<count($res2);$i++){
            $res2[$i]['pay_date']=$ut->greg_to_jal($res2[$i]['pay_date']);
        }

        return array($res[0],$res2);

    }

    public function do_inq_last_confirm($arr){
        $ut=new Utility();
        $db=new DBi();
        extract($arr);
        $selected_rowids=json_decode($selected_suggest_stat,true); 
    
        $update_sql="UPDATE `inquiries` SET `status`=20,final_confirm=1 where `RowID`={$inq_id} ";
        ...
        if($res2)
        {
            foreach($selected_rowids as $RowID){
                $rowid=intval($RowID);
                $up_sql = "UPDATE `inq_sellers` SET `status`=20 where RowID={$rowid}";
                $res=$db->Query($up_sql);
                if(!$res){
                    return false;
                }
    ...
    public function do_send_inquiry($arr){
        ...
        if($final_confirm==1){
            $inq_info="SELECT `creator_id` from inquiries where RowID={$inq_id}";
            $res1=$db->ArrayQuery($inq_info);
            $reciever = $res1[0]['creator_id'];
        }
        ////$ut->fileRecorder('testttttttttttttttttttttttttttttt');
        $sql="INSERT INTO inquiry_workflow (`sender`,`reciever`,`inquiry_id`,`status`,`create_date`,`create_time`,`description`)
                VALUES({$_SESSION['userid']},{$reciever},{$inq_id},1,'{$current_date}','{$current_time}','{$desc}')";   
        $res=$db->Query($sql);
        ////$ut->fileRecorder('testttttttttttttttttttttttttttttt');
        if($res){

                $update_sql="UPDATE `inquiries` set `last_receiver`={$reciever},final_confirm=0 ,`status`=3 where `RowID`={$inq_id}  ";
                $res2=$db->Query($update_sql);
	...
	public function get_inquires($status,$page=1,$other_params=[]){
      ...
      //  $w = "i.`status` <> -1 AND s.`group`='inq_status' AND ";
        ...
        switch($status){
            ...
            case 2:
                if($_SESSION['userid']==1){
                    $w.="i.status NOT IN(20,0,1)";
                }
                else{
                    $w.="    i.`last_receiver`={$_SESSION['userid']} AND  i.status NOT IN(20,0,1)";
                }
            ...
            case 11:
                if($_SESSION['userid']==1 || $_SESSION['userid']==4){
                   ...
                   // $w.="  i.`status`={$status} ";
                    $status_desc="استعلام  در انتظار تایید  ";
                    break;
                
            case 20:
                if(in_array($_SESSION['userid'],[4,14,20,1])){
                    $w.="  i.`status`={$status} ";
                }
                else{
                    $w.="  i.`creator_id`={$_SESSION['userid']} AND  i.`status`={$status} ";
        ...
        $sql="SELECT i.*,s.description as `status_name`,( select `sender` from inquiry_workflow where reciever = i.last_receiver ORDER BY RowID desc limit 1) as `sender`   FROM inquiries as i 
        ...
        if($status!=-1)
        {
            ...
            if (count($result) > 0) 
            {
                if ($status == 0) {
                    ...
                    $table_header = ['ردیف', 'گروه استعلام ', 'تاریخ استعلام', 'کد استعلام', 'ارسال کننده', 'مدیریت'];
                } elseif ($status == 3) {
                ...
                foreach ($result as $row) {
                    ...
                    if($status==2){
                        $htm .= "<td>" . $ut->get_user_fullname($row['sender']) . "</td>";
                    }
                    else{
    ...
    public function add_new_seller($array){
        $db=new DBi();
		$ut=new Utility();
        extract($array);
        ...
        `mobile`,
        `phone`
        ) VALUES('{$seller_name}','{$tafzili}',0,'ندارد','{$_SESSION['userid']}','{$code_melli}',2,'{$address}','{$mobile}',{$phone})";
        $res=$db->Query($sql);
        //$ut->fileRecorder('test'.$sql);
        return $res;
    }
    ...
    public function create_pay_comment_modal($inq_id){
        ...
        $sql = "SELECT SUM(b.ultimate_price) as `ultimate_price`,SUM(b.unit_price) as `unit_price`, SUM(b.ultimate_rent) as `ultimate_rent`,c.`code`,c.`Name` from inquiry_sellers as a  
                ...
                where a.`inq_id`=${inq_id} AND a.`status`=1 AND b.`status`=20 Group by a.`seller_id` ";
        $final_array = [];
        $array_handler = [];
        $res = $db->ArrayQuery($sql);
        ////$ut->fileRecorder('pay_comment:'.$sql);
...
//--------------------------------------------------
    ...
    public function display_inq_counts(){
        ...
        //-----------------------------------------------------------------------------------------------------------------
        $get_received_inqs="SELECT RowID from `inquiries` where `status` NOT IN(0,1)  AND `last_receiver`={$_SESSION['userid']}";//get received inqs
        $res_received=$db->ArrayQuery($get_received_inqs);
        //$ut->fileRecorder('res_received:'.$get_received_inqs);
    ...
    public function save_seller_info($arr){
        $db=new DBi();
		$ut=new Utility();
        extract($arr);
        
        $pay_array=json_decode($arr['pay_method_array'],true);
       
        if($inq_detailes_id==0)
        {
            $sql2="SELECT RowID FROM `inquired_goods` where `status`=1 AND `good_id`={$good_id} AND  `inq_id`={$inq_id}";
            $res2=$db->ArrayQuery($sql2);
            ...
            $sql="INSERT INTO `inq_sellers`(
                    ...
                    `ultimate_rent`,`description`,`tax_risk`,`tamin_risk`) 
                    VALUES('{$res2[0]['RowID']}','{$seller_tbl_id}', '{$is_vat}',
                            '{$rent_percent}','{$unit_price}','{$pay_term}',
                            '{$price_valid}','{$rent}','{$ultimate_price}',
                            '{$ultimate_rent}','{$description}','{$tax_risk}','{$tamin_risk}')";
            $res=$db->Query($sql);
        ...
        $del_sql="DELETE from inq_pay_method where inq_seller_tbl_id={$inq_detailes_id}";
        $res=$db->Query($del_sql);
         $sql_p="INSERT INTO `inq_pay_method` (`pay_method`,`pay_amount`,`pay_type`,`pay_date`,`inq_seller_tbl_id`,`description`) VALUES";
         $values="";
        foreach($pay_array as $key=>$value){
           
            $values.="('{$value['pay_method']}','{$value['pay_amount']}','{$value['pay_type']}','{$ut->jal_to_greg($value['pay_date'])}','{$inq_detailes_id}','{$value['pay_description']}'),";
        }
        $values_u=rtrim($values,',');
        $sql_p.=$values_u;
    ...
    public function show_summary_suggests($inq_id,$good_id,$seller_id,$RowID){
        $db=new DBi();
		$ut=new Utility();
        $sql2="SELECT RowID from `inquired_goods` where good_id={$good_id} AND inq_id={$inq_id} AND `status`=1";
        $res2=$db->ArrayQuery($sql2);
       // $sql="SELECT * FROM `inq_sellers` where `status`=1 AND inq_good_tbl_id={$good_id}  AND `inquiry_sellers_tbl_id`={$RowID}";
        $sql="SELECT iq.* ,iq2.`ostan_id`,iq2.`city_id`,iq2.`phone` FROM `inq_sellers` as iq 
                LEFT JOIN `inquiry_sellers` as iq2 on iq.`inquiry_sellers_tbl_id`=iq2.`RowID` 
                 WHERE iq.`status`=1 AND iq2.`seller_id`={$seller_id} AND iq.`inq_good_tbl_id`={$res2[0]['RowID']}  AND iq.`inquiry_sellers_tbl_id`={$RowID}";
        
        ////$ut->fileRecorder('***************//////////////////////*************');
        ////$ut->fileRecorder($sql2);
        $ut->fileRecorder('ssqqll:'.$sql);
        $res=$db->ArrayQuery($sql);
        return $res;
    }
    ...
    public function get_inq_seller_detailes($good_id,$seller_id,$RowID,$inq_id){
        $db=new DBi();
		$ut=new Utility();

        $sql1="SELECT RowID FROM `inquired_goods` where `status`=1 AND good_id={$good_id} AND inq_id={$inq_id}";
        $res1=$db->ArrayQuery($sql1);

        $sql  = "SELECT a.*,b.`quantity` FROM `inq_sellers` as a 
                    left join `inquired_goods` as b  on a.`inq_good_tbl_id`=b.`RowID` 
                    WHERE a.`inq_good_tbl_id`='{$res1[0]['RowID']}' AND a.`inquiry_sellers_tbl_id`={$RowID} AND a.`status`=1 ";
    ...
    public function get_last_inq_detailes($inq_id,$good_id){
        $ut=new Utility();
        
        $res=$this->get_last_inq_data(array('inq_id'=>$inq_id,'good_id'=>$good_id));
        if($res['parent_id']==-1){
            $html='<div><p class="text-center color-secondary"> &#10004;  &nbsp;خرید برای اولین بار</p></div>';
        }
        else{
            $ultimat_price=number_format($res['ultimate_price']);
            $html="<table class='table'>
                ...
                <tr>
                    <td>قیمت </td>
                    <td>{$ultimat_price} ریال</td>
                </tr>
                <tr><td>مدت بازپرداخت</td><td>{$res['pay_term']} روز</td></tr>
                <tr><td>تاریخ خرید</td><td>{$res['inq_date']}</td></tr>
            </table>";
        }
        return $html;
    }
    public function get_inq_comments_confirm($user_id){
        $db = new DBi();
        $ut = new Utility();
        $sql = "SELECT `RowID` FROM `inquiry_workflow` where `sender` = {$user_id} AND `status`=1 ORDER BY `RowID` DESC";
        $res = $db->ArrayQuery($sql);
        return count($res);
    ...
    public function get_inquired_goods_detailes($good_id,$inq_id){
        ...
        public function get_inquired_detailes($seller_id,$good_id,$inq_id,$inq_status){
            $ut=new Utility();
            $db=new DBi();
            $sql="SELECT a.* FROM `inq_sellers` as a 
                left join `inquiry_sellers` as b on a.`inquiry_sellers_tbl_id`=b.`RowID`
                left join `inquired_goods` as c on a.`inq_good_tbl_id`=c.`RowID`
                where b.`seller_id` = {$seller_id} AND c.`good_id`={$good_id} AND c.`inq_id`={$inq_id} AND a.`status` IN(1,20) AND b.`status`=1 AND c.`status`=1";
                $res=$db->ArrayQuery($sql);
                ////$ut->fileRecorder('sql...:'.$sql);
            $counter=1;
            if(count($res)>0){
                $fa_title=['مدت بازپرداخت','اعتبار قیمت','قیمت نهایی','توضیحات','وضعیت'];
                $en_title=['pay_term','price_valid','ultimate_price','description','status'];
                $html="<table border='1' class='table'>";
                for($i=0;$i<count($en_title);$i++){
                    $html.="<tr>";
                    $html.="<td>{$fa_title[$i]}</td>";
                    for($j=0;$j<count($res);$j++){
                        $td_comment=$res[$j][$en_title[$i]];
                        switch($en_title[$i]){
                            case 'pay_term' :
                                $td_comment = $td_comment." روز";
                                break;
                            case 'price_valid':
                                $td_comment = $td_comment." ساعت";
                                break;
                            case 'ultimate_price' :
                                $td_comment = number_format($td_comment)." ساعت";
                                break;
                            case 'status':
                                ////$ut->fileRecorder('td_comment:'.$td_comment);
                                if($inq_status==20){
                                   
                                   
                                    if($td_comment==20){
                                        
                                        $td_comment="<span class='text-success'>&#10004;</span>";
                                    }
                                    else{
                                        $td_comment="<span class='text-danger'>&#10006;</span>";
                                    }
                                }
                                else{
                                    $td_comment="<span class='text-secondary'>درحال بررسی ...</span>";
                                }
                                
                                break;
                        }
                        $html.="<td>{$td_comment}</td>";
                    } 
                    $html.="</tr>";
                }
        ...
        public function get_inq_users_comments($user_id,$inq_id,$comment_RowID=0){
            ...
            $sql="SELECT `paraf_text`,`paraf_date_time` FROM `inq_paraf` where inq_id={$inq_id} AND `user_id`={$user_id} AND`status`=1"; 
            if($comment_rowID==0){
            ...
            $show_status=$this->get_inq_comments_confirm($user_id);
            $res = $db->ArrayQuery($sql);
            $comment_array['paraf_text']=$res[0]['paraf_text'];
            $comment_array['paraf_date']=$res[0]['paraf_date_time'];
            $comment_array['show_status']=$show_status;
        ...
        public function inqiury_print($inq_id,$print_paraf){
            $db = new DBi();
            $ut = new Utility();
            ...
            $inq_status=$res_header[0]['status'];
            ...
            foreach($good_ids_array as $good_id){
                $get_last_inq_detailes=$this->get_last_inq_detailes($inq_id,$good_id);
                $inq_tr.="<tr class='inq_detailes'>
                        <td>".$this->get_inquired_goods_detailes($good_id,$inq_id)."</td><td>".$get_last_inq_detailes."</td>";
                foreach($sellers_res as $k_s=>$v_s){
                    $inq_tr.="<td><div>".$this->get_inquired_detailes($v_s['seller_id'],$good_id,$inq_id,$inq_status)."</div></td>";
                }
                $inq_tr.="</tr>";
            }
            $logo_img = ADDR."images/logo.jpg";
            //---------------------------------------------------------------create footer -------------------------------------------------
            $get_inq_users_comment="SELECT `user_id` FROM inq_paraf where inq_id={$inq_id} group by `user_id`";
           
            $users_res=$db->ArrayQuery($get_inq_users_comment);
            $footer_array=[];
    
            foreach($users_res as $key=>$value){
                $role=$this->check_inq_users_role($value['user_id']);
                $comment_array=$this->get_inq_users_comments($value['user_id'],$inq_id);
                $footer_array[$role]['fullname'] = $ut->get_user_fullname($value['user_id'])." ".$this->get_full_shamsi_date($comment_array['paraf_date']);
                $footer_array[$role]['comment'] = $comment_array['paraf_text'];
                $footer_array[$role]['sign'] = $this->get_user_sign($value['user_id']);
                $footer_array[$role]['show_status'] = $comment_array['show_status'];
           ...
           $html .= '
            <footer style="bottom: 0;width: 100% ;position:absolute;bottom:0">
                ...
                <thead>
                    <tr>
                        <td class="'.($footer_array['agent']['show_status']>0?"show_paraf":"hide_paraf").'">این قسمت توسط کارشناس بازرگانی خرید تکمیل می گردد.</td>
                        <td class="'.($footer_array['si']['show_status']>0?"show_paraf":"hide_paraf").'">این قسمت توسط سرپرست بازرگانی خرید تکمیل می گردد.</td>
                        <td class="'.($footer_array['mc']['show_status']>0?"show_paraf":"hide_paraf").'">این قسمت توسط مدیر بازرگانی خرید تکمیل می گردد.</td>
                        <td class="'.($footer_array['vpc']['show_status']>0?"show_paraf":"hide_paraf").'">این قسمت توسط معاونت بازرگانی تکمیل می گردد.</td>
                        <td class="'.($footer_array['hm']['show_status']>0?"show_paraf":"hide_paraf").'">این قسمت توسط مدیر عامل تکمیل می گردد.</td>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td class="'.($footer_array['agent']['show_status']>0?"show_paraf":"hide_paraf").'">
                            <div class="inq_comment_container">
                                <div class="inq_comment">
                                    <span>'   .   $footer_array['agent']['fullname']   . '</span><br><span>' .  $footer_array['agent']['comment']  .   '</span>
                                </div>
                                <div class="inq_sign">
                                    <img width="70" height="70" src="../Signature/'    .   $footer_array['agent']['sign']  .   '">
                                </div>
                            <div>
                        </td>

                        <td class="'.($footer_array['si']['show_status']>0?"show_paraf":"hide_paraf").'">
                            <div class="inq_comment_container">
                                <div class="inq_comment">
                                    <span>'   .   $footer_array['si']['fullname']      . '</span><br><span>' .  $footer_array['si']['comment']     .   '</span>
                                </div>
                                <div class="inq_sign">
                                    <img width="70" height="70" src="../Signature/'    .   $footer_array['si']['sign']     .   '">
                                </div>
                            </div>
                        </td>

                        <td class="'.($footer_array['mc']['show_status']>0?"show_paraf":"hide_paraf").'">
                            <div class="inq_comment_container">
                                <div class="inq_comment">
                                    <span>'   .   $footer_array['mc']['fullname']      . '</span><br><span>' .  $footer_array['mc']['comment']     .   '</span>
                                </div>
                                <div class="inq_sign">
                                    <img width="70" height="70" src="../Signature/'    .   $footer_array['mc']['sign']     .   '">
                                </div>
                            </div>
                        </td>

                        <td class="'.($footer_array['vpc']['show_status']>0?"show_paraf":"hide_paraf").'">
                            <div class="inq_comment_container">
                                <div class="inq_comment">
                                    <span>'   .   $footer_array['vpc']['fullname']     . '</span><br><span>' .  $footer_array['vpc']['comment']    .   '</span>
                                </div>
                                <div class="inq_sign">
                                    <img width="70" height="70" src="../Signature/'    .   $footer_array['vpc']['sign']    .   '">
                                </div>
                            </div>
                        </td>

                        <td class="'.($footer_array['hm']['show_status']>0?"show_paraf":"hide_paraf").'">
                            <div class="inq_comment_container">
                                <div class="inq_comment">
                                    <span>'   .   $footer_array['hm']['fullname']      . '</span><br><span>' .  $footer_array['hm']['comment']     .   '</span>
                                </div>
                                <div class="inq_sign">
                                    <img width="70" height="70" src="../Signature/'    .   $footer_array['hm']['sign']     .   '">
                                </div>
                            </div>
                        </td>                 
                    </tr>
            ...
            if($print_paraf==1){
                ...
                $html.='<div class="inq_header">
                    <div class="inq_logo">  <img src="'.$logo_img.'" ></div>
                    <div style="width: 60%;border-left:2px solid #000;text-align: center;display: flex;align-items: center;justify-content: center">پاراف ها و توضیحات ثبت شده</div>
                    <div style="width: 20%;padding: 10px" >
                        <table >
                            <tr><td>کد فرم :</td><td >F111012</td></tr>
                            <tr><td>سطح تغییرات  :</td><td>1</td></tr>
                ...
                <div style="display: flex;align-items: center;gap:4rem;border-bottom:1px solid #000;height:1cm">
                    ...
                    $sql_paraf="SELECT * FROM `inq_paraf` where inq_id={$inq_id} and `status`=1";
                    $res_p=$db->ArrayQuery($sql_paraf);
                    $counter2=1;
                    $html.='<h3 class="w-100 p-4 text-center bg-gray">پاراف های ثبت شده </h3>';
                    $html.='<table class="table">
                    <tr>
                        <th> ردیف</th>
                        <th> نویسنده</th>
                        <th> تاریخ و زمان درج </th>
                        <th> توضیحات</th>
                    </tr>';
                    foreach($res_p as $key => $value){
                        ...
                        <td>".$this->get_full_shamsi_date($value['paraf_date_time'])."</td>
                        
                        <td>".$value['paraf_text']."</td>
                        </tr>
                        ";
                        $counter2++;
                    }
                    ...
                    $counter=1;
                    $html.='<h3 class="w-100 p-4 text-center bg-gray">توضیحات ارجاع </h3>';
                    $html.='<table class="table">
                    <tr>
                        <th> ردیف</th>
                        <th> فرستنده</th>
...
public function attach_inq_file($file,$file_title,$good_id,$inq_id){
   ...
   // ////$ut->fileRecorder($group_code);
    ...
    public function get_inquiry_detailes($RowID){
        ...
        $total_value=0;
        $show_confirm=0;
        $sql1="SELECT (a.quantity*AVG(s.ultimate_price)) as `ultimat_price` FROM `inquired_goods` as a  left join `inq_sellers` as s on s.`inq_good_tbl_id`=a.`RowID` where a.`inq_id`={$RowID} group by a.`good_id` group by a.`good_id`";
        
        $res1=$db->ArrayQuery($sql1);
        foreach($res1 as $key=>$value){
           $total_value+=$value['ultimat_price']; 
        }
        //$ut->fileRecorder('testtttttttt'.$inq_status);
        if($inq_status==3){
        ...
        return array($res_inq[0],$show_confirm);
    }

    // public function inq_check_ready_final_confirm($inq_id){
    //     $sql_inq="SELECT * FROM inquiries where RowID={$inq_id}";
    //     $res_inq=$db->ArrayQuery($sql_inq);
    //     $inq_status=$res_inq[0]['status'];
    //     if($inq_status==2){
    //         $get_transition_lebvel_sql="SELECT * FROM trading_levels where status=1 AND `corroborant` like '%{$_SESSION['userid']}%'";
    ...
    public function get_inq_footer($inq_id){
       ...
       $ut=new Utility();
        ...
        foreach($res_role as $key=>$value){
            if($_SESSION['userid']==1){
                $get_last_paraf="SELECT * From inq_paraf where  `status`=1 AND inq_id={$inq_id} order by `RowID`  DESC LIMIT 1";
