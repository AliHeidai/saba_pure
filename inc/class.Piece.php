...
class Piece{
    ...
    public function getManagePieceHtm(){
        ...
        $c++;
        //----- add update_history----
        $bottons[$c]['title'] = "مشاهده سوابق بروزرسانی";
    ...
    //             $update_date_time=$ut->greg_to_jal($update_date_time_array[0])." ".$update_date_time_array[1];
    //             $update_status=$value['status']==1?'<span class="text-success">بروزرسانی موفق</span>':'<span class="text-danger">خطا در بروزرسانی</span>';
    //             $html_out.="<tr>";
    //             $html_out.="<td>".($key+1)."</td>";
    //             $html_out.="<td>{$value['result']}</td>";
    //             $html_out.="<td>{$update_date_time}</td>";
    //             $html_out.="<td>{$update_status}</td>";
    //             if($developer_access==1){
    //                 if($value['status']==0 && !empty($value['error_text'])){
    //                     $error_btn="<a class='text-danger' onclick='display_update_error({$value['RowID']})'>مشاهده خطا</a>";
    //                 }
    //                 else{
    //                     $error_btn="<span class='text-success'>فاقد خطا</span>";
    //                 }
    //                 $html_out.="<td>{$error_btn}</td>";
    //             } 
