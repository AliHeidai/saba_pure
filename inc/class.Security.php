...
class Security{
    ...
    public function get_event_type($id=0){
        $acm = new acm();
        ...
        $db = new DBi();
        if($id==0){
            $sql="SELECT `RowID`,`desc` FROM `event_type` where `status` =1";
            $res=$db->ArrayQuery($sql);
            return $res;
        }else{
            $sql="SELECT `RowID`,`desc` FROM `event_type` where `status` =1 and `RowID`='{$id}'";
    ...
    public function getSecurityAccessManageHtm(){
        ...
        $events_type=$this->get_event_type();
        $pagename = array();
        $pageIcon = array();
        $contentId = array();
        $menuItems = array();
        ...
        if($acm->hasAccess('recordEventManage')) {
            ...
            $b++;
            if($acm->hasAccess('CreateEventsType')) {
                $bottons1[$b]['title'] = "مدیریت نوع رویداد";
                $bottons1[$b]['jsf'] = "manageEventsType";
                $bottons1[$b]['icon'] = "fa-plus-square";
            }
            ...
            $headerSearch1[$a]['width'] = "220px";
            $headerSearch1[$a]['id'] = "RecordEventsManageTypeSearch";
            $headerSearch1[$a]['title'] = "نوع رویداد";
            $headerSearch1[$a]['options'] = array();
            $headerSearch1[$a]['options'][0]["title"] = "نوع رویداد را انتخاب نمایید";
            $headerSearch1[$a]['options'][0]["value"] = -1;
            $counter=1;
            foreach($events_type as $row){
                $headerSearch1[$a]['options'][$counter]["value"] = $row['RowID'];
                $headerSearch1[$a]['options'][$counter]["title"] = $row['desc'];
                $counter++;
        ...
        if($acm->hasAccess('agencyManage')) {
            ...
            $headerSearch2[$a]['options'][3]["title"] = "اسنپ پنل";
            ...
            $headerSearch2[$a]['options'][6]["value"] = 5;
             $headerSearch2[$a]['options'][6]["title"] = "اسنپ کارکنان";
        ...
        if($acm->hasAccess('PropertyManagement')){
            ...
            $pagename[$x] = "مدیریت اموال(اموال/ابزارگردانی)";
            $pageIcon[$x] = "fa-tools";
            $contentId[$x] = "PropertyManageBody";
            $menuItems[$x] = 'PropertyManageTabID';
            ...
            $b = 0;
            if($acm->hasAccess('property_manage_create')){
                $bottons8[$b]['title'] = "ثبت تراکنش";
                $bottons8[$b]['jsf'] = "editcreatePropertyManage";
                $bottons8[$b]['icon'] = "fa-plus-square";
                $b++;
            }
            $bottons8[$b]['title'] = "گزارش اکسل";
            $bottons8[$b]['jsf'] = "create_prop_excel_report";
            $bottons8[$b]['icon'] = "fa-file-excel";
            $b++;
            $bottons8[$b]['title'] = "پرینت ";
            $bottons8[$b]['jsf'] = "print_prop_report";
            $bottons8[$b]['icon'] = "fa-print";
            $b++;

            // $bottons8[$b]['title'] = "ایجاد ابزار جدید";
            // $bottons8[$b]['jsf'] = "createProperty";
            // $bottons8[$b]['icon'] = "fa-plus-square";
        ...
        $items[$c]['type'] = "select";
        $items[$c]['id'] = "recordEventsManageType";
        $items[$c]['title'] = " نوع واقعه";
        $items[$c]['options'][0]["title"] = "--------";
        $items[$c]['options'][0]["value"] = -1;
        $opt=1;
        for($i=0;$i<count($events_type);$i++){
            if($events_type[$i]['RowID']!=15){
                $items[$c]['options'][$opt]["title"] = $events_type[$i]['desc'];
                $items[$c]['options'][$opt]["value"] = $events_type[$i]['RowID'];
                $opt++;
            }
        ...
        $items[$c]['options'][3]["title"] = "اسنپ پنل";
        ...
        $items[$c]['options'][6]["title"] = "اسنپ کارکنان";
    ...
    public function getRecordEventsList($sDate,$eDate,$desc,$event_type,$page=1){
        $acm = new acm();
        if(!$acm->hasAccess('recordEventManage')){
            die("access denied");
            exit;
        ...
        for($y=0;$y<$listCount;$y++){
            ...
            $finalRes[$y]['e_type'] = !empty($this->get_event_type($res[$y]['e_type'])['desc']) ? $this->get_event_type($res[$y]['e_type'])['desc'] : 'نامشخص';
            $finalRes[$y]['name'] = $rst[0]['fname'].' '.$rst[0]['lname'];
        }
        return array_values($finalRes);
    }

    public function getRecordEventsListCountRows($sDate,$eDate,$desc,$event_type){
        $db = new DBi();
        $ut = new Utility();
        $w = array();
        if(intval($event_type) > 0){
    ...
    public function eventsInfo($infoID){
        ...
        $res = $db->ArrayQuery($sql);
        if(count($res) == 1){
            $res = array("infoID"=>$infoID,"cTime"=>$res[0]['cTime'],"event"=>$res[0]['event'],'e_type'=>($res[0]['e_type']),"event_date"=>$ut->greg_to_jal($res[0]['cDate']));
            return $res;
        }else{
            return false;
        }
    ...
    public function createEvents($arr){
        $acm = new acm();
        $ut=new Utility();
        if(!$acm->hasAccess('recordEventManage')){
            die("access denied");
        ...
        $rDate = date('Y-m-d');
        if ($h == '00'){
            $rDate = date('Y-m-d',strtotime("yesterday"));
        }
        //
        ...
        if(intval($e_type)==3){
            $info_on_off="SELECT * from  info where e_type=2 AND parent_id=-1";
            $res_on_off=$db->ArrayQuery($info_on_off);
            if(count($res_on_off)==0){
                return -1;
            }
            elseif(count($res_on_off)==1){
                $parent_id=$info_on_off[0]['RowID'];
            }
            else{
                $parent_id=$info_on_off[0]['RowID'];
                $diff_time_d=0;
                foreach($res_on_off as $event){
                    $e_date_time=$cDate." ".$cTime;
                    $reg_date_time=$event['cDate']." ".$event['cTime'];
                    $diff_time=strtotime($reg_date_time)-strtotime($e_date_time);
                    
              
                    if($diff_time<$diff_time_d){
                      
                       
                        $diff_time_d=$diff_time;
                        if(!$this->get_parent_id($event['RowID'])){
                            $parent_id=$event['RowID'];
                            continue;
                        }
        ...
        $sql = "INSERT INTO `info` (`uid`,`cTime`,`cDate`,`event`,`e_type`,`rDate`,`parent_id`) VALUES ({$_SESSION['userid']},'{$cTime}','{$cDate}','{$desc}','{$e_type}','{$rDate}',{$parent_id})";
    ...
    public function get_parent_id($id){
        $db=new DBi();
        $sql="SELECT RowID from info where parent_id='{$id}'";
        $res=$db->ArrayQuery($sql);
        if(count($res)>0){
            return true;
        }
    ...
    public function editEvents($arr){
        $acm = new acm();
        $ut=new Utility();
        if(!$acm->hasAccess('recordEventManage')){
            die("access denied");
        ...
        $sql1 = "INSERT INTO `info_backup` (`infoID`,`uid`,`cTime`,`cDate`,`event`,`e_type`) VALUES ({$eid},{$res[0]['uid']},'{$res[0]['cTime']}','{$res[0]['cDate']}','{$res[0]['event']}','{$res[0]['e_type']}')";
        $db->Query($sql1);

        $sql2 = "UPDATE `info` SET `uid`={$_SESSION['userid']},`cTime`='{$cTime}',`event`='{$desc}',`cDate`='{$ut->jal_to_greg($cDate)}',`e_type`='{$e_type}' WHERE `RowID`={$eid}";
        ...
        for($y=0;$y<$listCount;$y++){
            ...
            switch ($res[$y]['agencyID']){
                ...
                case 5:
                    ...
                    break;
                     case 6:
                    $finalRes[$y]['agencyID'] = ' اسنپ کارکنان';
                    break;
        ...
        for ($i=0;$i<$cnt;$i++){
            ...
            switch ($res[$i]['agencyID']){
                ...
                case 5:
                    ...
                    break;
                     case 6:
                    $finalRes[$i]['agencyID'] = ' اسنپ کارکنان';
                    break;
            }
            switch ($res[$i]['serviceID']){
                case 0:
...
/*        $query = "SELECT `pid` FROM `overtime_lunch` WHERE `createDate`='{$cDate}'";
    ...
    public function get_units(){
        $db = new DBi();
        $query = "SELECT `RowID`,`Uname` FROM `official_productive_units` where `Utype`=1 ";
        $rst = $db->ArrayQuery($query);
        if (count($rst) > 0) {
            return $rst;
        } else {
    ...
    public function getEnventsTypeList(){
        $db = new DBi();
        $ut=new Utility();
        $query = "SELECT `RowID`,`desc`,`units_allowed`
                    FROM `event_type` WHERE `status`=1";
        $rst = $db->ArrayQuery($query);
        if (count($rst) > 0) {
            $html="<table class='table table-bordered table-hover table-sm'>
                <thead>
                    <tr class='bg-info'>
                        <th style='text-align: center;color: #ffc107;font-family: dubai-Bold;width: 5%;'>ردیف</th>
                        <th style='text-align: center;color: #ffc107;font-family: dubai-Bold;width: 10%;'>نام</th>
                        <th style='text-align: center;color: #ffc107;font-family: dubai-Bold;width: 15%;'>واحد های مجاز</th>
                        <th style='text-align: center;color: #ffc107;font-family: dubai-Bold;width: 15%;'>  مدیریت واحد ها</th>
                    </tr>
                </thead>";
                $counter=1;
            foreach($rst as $key=>$value){
                $allowed_units=$this->get_allowed_units($value['units_allowed']);
               
                $html.="<tr class='table-secondary'>
                        <td style='text-align: center;font-family: dubai-Regular;padding: 10px;'>".($counter)."</td>
                        <td style='text-align: center;font-family: dubai-Regular;padding: 10px;'>".$value['desc']."</td>
                        <td style='text-align: center;font-family: dubai-Regular;padding: 10px;'>".$allowed_units."</td>
                        <td style='text-align: center;font-family: dubai-Regular;padding: 10px;'><button class='btn btn-info fa fa-edit' onclick='manage_allowed_users({$value['RowID']})' ></button></td>
                    </tr>";
                    $counter++; 
           ...
           $html=  "<p class='text-center text-danger'>موردی یافت نشد</p>";
        }
        return $html;
    }

    public function get_allowed_units($unit_ids){
        $db = new DBi();
        $ut=new Utility();
        $query = "SELECT `RowID`,`Uname` FROM `official_productive_units` WHERE `RowID` IN ({$unit_ids})";
    ...
    public function doEditCreateEventType($arr){
        ...
        $units_allowed = $event_type_allowed_unit;
        if($event_type_id >0){
            $sql = "UPDATE `event_type` SET `units_allowed`='{$units_allowed}',`updated_at`='{$created_at}',`uid`='{$uid}' WHERE `RowID`={$event_type_id}";
            ...
            $sql = "INSERT INTO `event_type` (`desc`,`units_allowed`,`created_at`,`uid`) VALUES ('{$event_type}','{$units_allowed}','{$created_at}','{$uid}')";
            $res = $db->Query($sql);
            if (intval($res) > 0){
                return true;
            }else{
    ...
    public function get_events_report($arr){
       $db=new DBi();
       $ut=new Utility();
       extract($arr);
       $get_unit_id="SELECT * from users where `RowID`='{$_SESSION['userid']}'";
       $res_unit=$db->ArrayQuery($get_unit_id);
       $unit_id=$res_unit[0]['unitID'];
       $w=[];
       
       $w[]="e.`units_allowed` like '%{$unit_id}%'";
      
       $change_format=0;
        if($from_date!=""){
            $w[]="io.`cDate`>='{$ut->jal_to_greg($from_date)}'";
        }
        if($to_date!=""){
            $w[]="io.`cDate`<='{$ut->jal_to_greg($to_date)}'";
        }
     
        if(!in_array($event_type,[-1,'15'])){
            $w[]="e.`units_allowed` like '%{$unit_id}%'";
            $w[]="`e_type`='{$event_type}'";
        ...
        if($event_type=='15'){
            $get_total="SELECT count(io.RowID) as `count` , 
                SEC_TO_TIME(SUM(TIME_TO_SEC(it.`cTime`)-TIME_TO_SEC(`io`.`cTime`)))
                AS total_duration from `info` as `io` 
                LEFT JOIN `event_type` as e  on e.`RowID`=`io`.`e_type`
                LEFT JOIN `info` as `it`  on io.`RowID`=it.`parent_id` {$w} ";
        }
        else{
            $get_total="SELECT count(i.RowID) as `count` from `info` as `io`  left join `event_type` e on io.e_type=e.RowID {$w} ";
        }
       
     $ut->fileRecorder('get_total:'.$get_total);
      
       $res_t=$db->ArrayQuery($get_total);
       $total_records=$res_t[0]['count'];
       $total_dura='';
       if($event_type=='15'){
        $total_dura= "<h6 style='padding:15px 0' class='text-danger'>جمع کل زمان قطعی برق  :<span  >{$res_t[0]['total_duration']}<span></h6>";
       }
      
    //    if($event_type=='15'){
    //     $total_records=intval($total_records/2);
       
    //    }
       $start=($page-1)*20;
       $query = "SELECT io.*,e.`units_allowed`,e.`desc` as event_type from info as io  left join event_type e on io.e_type=e.RowID  {$w} order by io.RowID desc LIMIT {$start},20"; 
       if($event_type=='15'){
      
        $query = "SELECT CONCAT(io.`event`,' | ',it.`event`) as `event`, 
                        CONCAT(io.`cDate`,' ',io.`cTime`) as `start_datetime`,
                        CONCAT(it.`cDate`,' ',it.`cTime`) as `end_datetime`,
                        CONCAT (io.`uid`,'-',it.`uid`) as `uid`
                    FROM info as io 
                    LEFT JOIN `event_type` as e  on e.`RowID`=io.`e_type` 
                    LEFT JOIN `info` as it  on io.`RowID`=it.`parent_id`  {$w} order by io.RowID desc LIMIT {$start},20"; 
       }
         
       $ut->fileRecorder($query);
         $evnt_type="SELECT * FROM `event_type` where units_allowed like '%{$unit_id}%'";
         $res_e=$db->ArrayQuery($evnt_type);
         
         $evnt_type_html="<option value='-1'>نوع رویداد را انتخاب نمایید</option>";
            foreach($res_e as $key=>$value){
                $selected="";
                if(!in_array($value['RowID'],[2,3])){
                if($event_type==$value['RowID']){
                    $selected="selected";}

                $evnt_type_html.="<option {$selected} value='{$value['RowID']}'>{$value['desc']}</option>";
                }
            }
            ...
            $html="<h4 class='text-center'>گزارش رویدادها</h4> ";
                $html.='
                    <form class="form-inline" style="margin: 10px 0 0 20px;">
                <div ">
                    <label class="sr-only" for="from_date">از تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" value="'.$from_date.'" id="from_date" autocomplete="off" style="width:100px;" placeholder="از تاریخ" data-mdpersiandatetimepicker="" data-original-title="" title="">
                </div>
                <div >
                    <label class="sr-only" for="to_date">تا تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" value="'.$to_date.'" id="to_date" autocomplete="off" style="width:100px;" placeholder="تا تاریخ" data-mdpersiandatetimepicker="" data-original-title="" title="">
                </div>
                <div id="">
                    <label class="sr-only" for="receivedCustomerRTypeSearchSended">نوع رویداد </label>
                    <select class="form-control form-control-sm mb-2 mr-sm-2 headsearch" style="width:120px" id="event_type">'.
                        $evnt_type_html
                    .'</select>
                </div>
               
                <div id="receivedCustomerCNameSearchSended-div">
                    <label class="sr-only" for="receivedCustomerCNameSearch"> شرح رویداد</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="event" value="'.$event.'" autocomplete="off" style="width:150px;" placeholder="شرح رویداد ">
                </div>
                
                <button type="button" class="btn btn-sm btn-secondary mb-2 mr-2" style="padding: 0.25rem 0.5rem;" onclick="get_events_report();">جستجو&nbsp;&nbsp;<i class="fa fa-search"></i></button></form>';
                $html.=$total_dura;
                if(count($res)){
                    if($change_format==0){
                $html.="<table class='table table-striped  table-bordered ' id='events_report_table'>
                <thead>
                    <tr>
                        <th style='width:5%'>ردیف</th>
                        <th style='width:55%'>شرح رویداد</th>
                        <th style='width:10%'>نوع رویداد</th>
                        <th style='width:10%'> تاریخ رویداد</th>
                        <th style='width:10%'> زمان رویداد</th>
                        <th style='width:10%'>  کاربر ثبت کننده</th>
                    </tr>
                </thead><tbody>";
                $counter=$start+1;
                foreach($res as $key=>$value){
                    ...
                    $html.="<tr class='table-light '>
                            ...
                            <td style='border:1px solid gray'>".$value['event']."</td>
                            <td style='border:1px solid gray'>".$value['event_type']."</td>
                            <td style='border:1px solid gray'>".$ut->greg_to_jal($value['cDate'])."</td>
                            <td style='border:1px solid gray'>".$value['cTime']."</td>
                            <td style='border:1px solid gray'>{$ut->get_user_fullname($value['uid'])}</td>
                        </tr>";
                        $counter++; 
                ...
                <thead>
                    <tr>
                        ...
                        <th style='width:30%'>شرح رویداد</th>
                        <th style='width:5%'>نوع رویداد</th>
                        <th style='width: 10%'> تاریخ شروع رویداد</th>
                        <th style='width:10%'> تاریخ پایان رویداد</th>
                        <th style='width:5%'> مدت  رویداد</th>
                        <th style='width:15%'>  کاربر ثبت کننده</th>
                    </tr>
                </thead><tbody>";
                $counter=$start+1;
                foreach($res as $key=>$value){
                    $start_array=explode(' ',$value['start_datetime']);
                    $end_array=explode(' ',$value['end_datetime']);
                    $date1=strtotime($value['start_datetime']);
                    $date2=strtotime($value['end_datetime']);
                    $users_arr=explode('-',$value['uid']);
                    $event_custom_type="";
                    switch(intval($value['e_type'])){
                        case 2:
                            $event_custom_type="قطع و وصل برق";
                            break;
                        case 14:
                            $event_custom_type="خرابی ماشین آلات";
                            break;
                    }
                   
                    
                  //  $ut->fileRecorder('date2'.$date2);
                    if(intval($users_arr[0])===intval($users_arr[1])){
                        $ut->fileRecorder('eq');
                        $uid_e=$ut->get_user_fullname($users_arr[0]);
                       
                    
                    }
                    else{
                        $ut->fileRecorder('nor eq');
                        $ut->fileRecorder(intval($users_arr[0]));
                        $ut->fileRecorder(intval($users_arr[1]));
                        $uid_e1=$ut->get_user_fullname($users_arr[0]);
                        $uid_e2=$ut->get_user_fullname($users_arr[1]);
                        $uid_e=$uid_e1."-".$uid_e2;
                    ...
                    $result=$date2-$date1;
                    $event_time=gmdate("H:i:s", $result);
                    $html.="<tr class='table-light '>
                            <td style='border:1px solid gray'>".($counter)."</td>
                            <td style='border:1px solid gray'>".$value['event']."</td>
                            <td style='border:1px solid gray'>{$event_custom_type}</td>
                            <td style='border:1px solid gray'>".$ut->greg_to_jal($start_array[0])." ".$start_array[1]."</td>
                            <td style='border:1px solid gray'>".$ut->greg_to_jal($end_array[0])." ".$end_array[1]."</td>
                            <td style='border:1px solid gray'>{$event_time}</td>
                            <td style='border:1px solid gray'>{$uid_e}</td>
                        </tr>";
                        $counter++; 

                }
                //get_sum_times($times_array)
            }
                $html.="</tbody></table>";
               
               // $total_res=$res_t[0]['count'];    
                $section="events_report_section";
                $section="events_report_section";
                $js_func="get_events_report";
                $pageinate=$ut->generatePagination($total_records, 20, $page, $js_func,$section, $max_pages = 5);
                $html.=$pageinate;
            }
            else{
                $html.=  "<div class='border rounded mh-100'><p class='text-center text-danger'>موردی یافت نشد</p></div>";
            }
           
            return $html;
    }

    public function get_sum_times($times_array){
        $totalSeconds=0;
        foreach($times_array as $time){
            $totalSeconds+= strtotime("1970-01-01 $time UTC") - strtotime("1970-01-01 00:00:00 UTC") ;
        }
        $hours = floor($totalSeconds / 3600);
        $minutes = floor(($totalSeconds % 3600) / 60);
        $seconds = $totalSeconds % 60;

    // نمایش نتیجه به فرمت زمان
        $totalTime = sprintf("%02d:%02d:%02d", $hours, $minutes, $seconds);
        return $totalTime;

    }

    public function manage_allowed_users($arr){
        $db = new DBi();
        extract($arr);
        $query = "SELECT `RowID`,`desc`,`units_allowed`
                    FROM `event_type` WHERE `status`=1 AND `RowID`={$row_id}";
        $rst = $db->ArrayQuery($query);
        if (count($rst) > 0) {
            return $rst[0];
        } else {
