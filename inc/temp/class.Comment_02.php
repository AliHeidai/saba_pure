...
/*        $items[$c]['options'] = array();
        ...
        $footerBottons[0]['title'] = "بستن";
        $footerBottons[0]['type'] = "dismis";
        $contractDetailsModal = $ut->getHtmlModal($modalID,$modalTitle,$items,$footerBottons,'','','',$ShowDescription);
        ...
        if($is_exception_user==0)
        {
            ...
            if(count($res)>0)
            {
                $htm.='<div style="display:flex;justify-content:space-between;align-items:center;width:30%">
                            ...
                            <button onclick="unSelectAllRows()" class="btn btn-danger">انتخاب هیچکدام</button>
                    </div>';
                ...
                for($i=0;$i<count($res);$i++){
                    $htm.="<tr>
                                ...
                                <td>".$res[$i]['desc'].'</td>
                                <td><button onclick="ShowWorkflowCommentAgain(\''.$res[$i]['RowID'].'\')" title=" گردش کار" class="btn btn-success">  <i class="fa fa-history"></></button></td>
                            </tr>';
                }
                $htm.="<tbody></table>";
                $send=array(true,$htm);
    ...
    public function createPayComment($pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalTransactionAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$files,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type){
       ...
       $ut = new Utility();
        ...
        if (strlen(trim($ContractNum)) > 0)
        {
            ...
            //****************************************************** */
            $addendum_sql="SELECT SUM(`Addendum_price`) as addendum_amount from `contract_addendum` where `contract_id`={$rstaql[0]['RowID']}";
	...
	public function editPayComment($cid,$pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalTransactionAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$files,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type){
        ...
        $db = new DBi();
        $ut = new Utility();
        $full_access_users_edit_pay_comment=[1];// (مدیریت سیستم )کاربران قادر به ویرایش اظهار نظر در هر  مرحله ای 
        ...
        if (strlen(trim($ContractNum)) > 0){
            ...
            else
            {
                ...
                $addendum_contract_sql="SELECT SUM(`Addendum_price`) as addendum_amount from contract_addendum where contract_id={$rstaql[0]['RowID']}";
        ...
        if ($flag) {
            ...
            $sql = "UPDATE `pay_comment` SET `type`='{$Type}',`Toward`='{$Toward}',`totalAmount`={$totalTransactionAmount},`Amount`={$Amount},`CashSection`={$CashSection},`NonCashSection`={$NonCashSection},
                   ...
                   `sabtDarHesabType`={$sabtDarHesabType},`cardNumber`='{$CardNumber}',`layer3`={$layer3},`accountID`={$accountID},`goodLoan`='{$goodLoan}',`checkOutType`='{$checkOutType}',`related_project`='{$related_project}',`related_VAT`='{$related_vat}' WHERE `RowID`={$cid}";
            $db->Query($sql);
            $res = $db->AffectedRows();
            $res = (($res == -1) ? 0 : 1);
            if (intval($res)) {
    ...
    public function sendPayComment($cid,$confType,$receiver,$desc,$PriorityLevel,$isPaid){
       ...
       //--------------------------------------------------------- //End for send automatic when  the reciver user is on leave
        ...
        $sqls = "SELECT `Amount` FROM `pay_comment` WHERE `RowID`={$cid}";
    ...
    public function transferPartnersPayCommentCartable($pid_array,$sender,$receiver){
       $acm = new acm();
        ...
        $db=new DBi();
        $sql="UPDATE pay_comment Set lastReceiver={$receiver} WHERE lastReceiver={$sender} AND RowID in(".$pid_array.")";
        $res=$db->Query($sql);
        //--------------------------------------------------
       // $get_last_pw_id_sql="SELECT `RowID` FROM `payment_workflow` WHERE `receiver`={$_SESSION['userid']}  AND `pid`={$cid} order by RowID DESC LIMIT 1";
        $get_last_pw_id_sql="SELECT `RowID` FROM `payment_workflow` WHERE `receiver`={$receiver}  AND `pid`={$cid} order by RowID DESC LIMIT 1";
        $get_last_pw_id=$db->ArrayQuery($get_last_pw_id_sql);
        $last_done_row_id=$get_last_pw_id[0]['RowID'];
        //--------------------------------------------------
        if($res)
        {
            ...
            if($result>0)
            {
                ...
                $pid_array=explode(",",$pid_array);
                $values="";
                $dateCreate=date('Y-m-d');
                //$timeCreate=date('G:i:s');
                //$timeCreate= date('H:i:s', strtotime('-1 hour'));
                $timeCreate= $this->current_time;
                for($k=0;$k<count($pid_array);$k++){
                    $values.="({$sender},{$receiver},{$pid_array[$k]},0,'{$dateCreate}','{$timeCreate}','{$description}'),";
                }
                $values=rtrim($values,',');
    ...
    public function commentWorkflowHtm($pid,$meta_data=""){
        ...
        if (intval($rstun[0]['unitID']) == 1){  // اگر اداری بود
            switch (intval($rsq[0]['Transactions'])){
                case 1:  // جزئی
                    $sqlu = "SELECT `uids` FROM `relatedunits` WHERE `RowID` IN (32,22,23)";  // پشتیبانی - اداری - مالی
                    $rstu = $db->ArrayQuery($sqlu);
                    for ($i=0;$i<count($rstu);$i++){
                        $rowids[] = $rstu[$i]['uids'];
                    }
        ...
        if (intval($rstun[0]['unitID']) == 36 || intval($rstun[0]['unitID']) == 24 ){  // اگر پشتیبانی یا حقوقی بود
            switch (intval($rsq[0]['Transactions'])){
                case 1:  // جزئی
                    $sqlu = "SELECT `uids` FROM `relatedunits` WHERE `RowID`=23";
                    $rstu = $db->ArrayQuery($sqlu);
        ...
        if(!empty($meta_data)){ //
            switch($meta_data){
                ...
                //     $get_unit_sql="SELECT u.`RowID`, rm.`key`, u.`uids` from `relatedunits` as u LEFT JOIN `relatedunits_meta` as rm on u.`RowID`=rm.`unit_id`
                //     where rm.`key`='related_to_barnameh'";
                //     $res=$db->ArrayQuery($get_unit_sql);
                //     $uides="";
                //    foreach($res as $key=>$value){
                //         $uides.=$value['uids'];
                //    }
                //    $uides_array=explode(",",$uides);
                //    $uides_array=array_unique($uides_array);
                //   unset($uides_array[array_search(1,$uides_array)]);
              
                    $get_mata_sql = "SELECT p.`RowID`,p.`sendType`,pm.`value` FROM `pay_comment` as p LEFT JOIN `pay_comment_meta` AS pm ON p.`RowID`=pm.`pay_comment_id`  WHERE pm.`key`='{$meta_data}' ";
...
//**********************************************
    ...
    public function getAccountNumbers($cfor){
        $db = new DBi();
        $rowID = explode(',',$cfor);
        $sql = "SELECT `RowID`,`accountNumber` FROM `account` WHERE `RowID`={$rowID[0]}";
        $res = $db->ArrayQuery($sql);
        if(count($res) == 1){
    ...
    public function getAccountNumbersWithName($cfor){
		...
		$sql = "SELECT `accountNumber`,`RowID`,`code` FROM `account` WHERE `Name`= '{$cfor}'";
        $res = $db->ArrayQuery($sql);
		if(count($res)==0){
			$sql = "SELECT `accountNumber`,`RowID`,`code` FROM `account` WHERE `Name` LIKE '%{$cfor}%'";
			$res = $db->ArrayQuery($sql);
		}
...
/*            $htm .= '<table style="width: 100%;border-right: 2px solid #000;border-left: 2px solid #000;">';
    ...
    public function createTransferPayComment($radioValue,$desc,$tick,$pid)
    {
       ...
       // $receiverTime=date('H:i:s', strtotime('-1 hour'));
        ...
        $ut->fileRecorder('test:'.$meta_comment_is_drivers_fare_count);
		if($rsq[0]['layer2']==59)
		{  
			$promised_manager_array=[3,72,75];
        ...
        elseif($meta_comment_is_drivers_fare_count>0)
        {
            ...
            $varede_flag=0;
            $sadereh_flag=0;
            if($comment_meta_info['is_drivers_fare_type']==2){//اگر بارنامه وارده بود----------------------------------
                $varede_flag=1;
            ...
            if($comment_meta_info['is_drivers_fare_type']==1){//اگر بارنامه صادره بود----------------------------------
    ...
    public function getFundListExcel($cid){
        ...
        foreach($rst as $rows){
            ...
            $array_handler['createDate']=$ut->greg_to_jal($rows['createDate']);
    ...
    function get_vat_pay_comment($cid){
      ...
      if($res[0]['related_vat']==1){
          ...
          if(count($res2)>0){
              ...
              return $res[0]['related_vat'];
          }
      }
      return !empty($res[0]['related_vat'])?$res[0]['related_vat']:0;
    }
    //++++++++++++++++++++ مدیریت قراردادها ++++++++++++++++++++
    ...
    public function getContractAddendumDetailes($cid)
    {
        $db=new DBi();
        $ut=new Utility();
        $getAddendumDetailes="SELECT * FROM `contract_addendum` WHERE `contract_id`={$cid} AND `status`=1";
            ...
            $html.=
                '<table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <td>#</td>
                            <td>شرح مسئولیت الحاقیه</td>
                            <td> ارزش مالی الحاقیه</td>
                            <td>تاریخ ثبت الحاقیه</td>
                            <td>عملیات</td>
                        </tr>
                    </thead>
                    <tbody>';
                    for($i=0;$i<count($res);$i++){
                        $html.='<tr>
                                    <td>'.($i+1).'</td>
                                    <td>'.$res[$i]['Addendum_duty'].'</td>
                                    
                                    <td>'.number_format($res[$i]['Addendum_price']).' ریال </td>
                                    <td>'.$ut->greg_to_jal($res[$i]['Addendum_date']).'</td>
                                    <td style="display:flex;justify-content:space-between;align-items:center">
                                        <i class="btn btn-danger fa fa-trash pointer" onclick="confirmDelete('.$res[$i]['RowID'].')"></i>
    ...
    public function editCreateContractAddendum($addendumDuty,$addendumCost,$addendumDate,$contract_id,$addendum_id){
        ...
        if($addendum_id==0)
        {
            $insertSql="INSERT INTO `contract_addendum` ( `contract_id`,`Addendum_duty`,`Addendum_price`,`Addendum_date`,`user_id`)VALUES({$contract_id},'{$addendumDuty}',{$addendumCost},'{$addendumDate}',{$_SESSION['userid']})";
            $db->Query($insertSql);
            $res=$db->InsertrdID();
    ...
    public function getContractPayComments($cid){
        ...
        $res = $db->ArrayQuery($sql);
        //----------------------------------------------- اعمال الحاقیه------------------------
        $addendum_sql="SELECT SUM(`Addendum_price`) as addendum_amount from contract_addendum where contract_id={$res[0]['RowID']}";
        //error_log($addendum_sql);
...
/*        $ut = new Utility();
    ...
    public function commentDepositHTM($pid,$deleteShow,$payOrReport,$sendMali,$confMali){
        ...
        for ($i=0;$i<$cnt;$i++){
            ...
            $htm .= '<td style="text-align: center;font-family: dubai-Regular;padding: 10px;">'.$res[$i]['dDesc'].'</td>';
...
/*    public function getReportPayCommentManageList($unCode,$rcsDate,$rceDate,$rcaName,$rcToward,$rcAmount,$rcPaytype,$rcPaySend,$rcSortType,$rpcsDate,$rpceDate,$rpnsDate,$rpneDate,$dUnit,$mUnit,$layer1,$layer2,$layer3,$page=1){
    ...
    public function getReportPayCommentManageList($unCode,$rcsDate,$rceDate,$rcaName,$rcToward,$rcAmount,$rcPaytype,$rcPaySend,$rcSortType,$rpcsDate,$rpceDate,$rpnsDate,$rpneDate,$dUnit,$mUnit,$layer1,$layer2,$layer3,$page=1){
        ...
        $sql = "SELECT `pay_comment`.`RowID` AS `pcid`,`unCode`,`cDate`,`CashSection`,`NonCashSection`,`Amount`,`Toward`,`accName`,`sendType`,`priorityLevel` FROM `pay_comment` ";
        if(count($w) > 0){
            $where = implode(" AND ",$w);
            $sql .= " WHERE ".$where;
        }
    ...
    public function getReportPayCommentManageListCountRows($unCode,$rcsDate,$rceDate,$rcaName,$rcToward,$rcAmount,$rcPaytype,$rcPaySend,$rpcsDate,$rpceDate,$rpnsDate,$rpneDate,$dUnit,$mUnit){
        $ut = new Utility();
        $db = new DBi();
        $full_access_array=$ut->get_full_access_users(7);
        $w = array();
    ...
    public function getTotalAmountPayComment($unCode,$rcsDate,$rceDate,$rcaName,$rcToward,$rcAmount,$rcPaytype,$rcPaySend,$rpcsDate,$rpceDate,$rpnsDate,$rpneDate,$dUnit,$mUnit){
        $acm = new acm();
        if(!$acm->hasAccess('reportPayCommentManage')){
            die("access denied");
            exit;
    ...
    public function getPayCommentReportExcel($unCode,$rcsDate,$rceDate,$rcaName,$rcToward,$rcAmount,$rcPaytype,$rcPaySend,$rpcsDate,$rpceDate,$rpnsDate,$rpneDate,$dUnit,$mUnit){
        $ut = new Utility();
        $db = new DBi();
        $full_access_array=$ut->get_full_access_users(7);
        $w = array();
        ...
        $sql = "SELECT `unCode`,`cDate`,`accName`,`codeTafzili`,`Toward`,`Amount`,`CashSection`,`NonCashSection`,`desc`,`sendType` FROM `pay_comment` ";
        if(count($w) > 0){
            $where = implode(" AND ",$w);
            $sql .= " WHERE ".$where;
        }
    ...
    public function getFinancialConfirmationList($fcsDate,$fceDate,$fcaName,$fcToward,$fcAmount,$page=1){
        ...
        $sql .= " ORDER BY `paymentMaturityCash` ASC ,`paymentMaturityCheck`  ASC LIMIT $start,".$numRows;
        $res = $db->ArrayQuery($sql);
        $listCount = count($res);
        $finalRes = array();
        for($y=0;$y<$listCount;$y++){
