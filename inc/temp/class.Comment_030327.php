...
/*        $items[$c]['options'] = array();
    ...
    public function commentWorkflowHtm($pid){
        $db = new DBi();
        $ut = new Utility();
		$acm = new acm();
			...
			for($manage_index=0;$manage_index  < count($promised_manager_array) ;$manage_index++ ){
			 ...
			 if($confirm_manages==count($promised_manager_array)){
				...
				if (intval($res)) 
				{
					...
					$sqq = "INSERT INTO `payment_workflow` (`sender`,`receiver`,`pid`,`status`,`createDate`,`createTime`,`description`) VALUES 
                    ({$_SESSION['userid']},{$receiver},{$pid},1,'{$receiverDate}','{$receiverTime}','{$desc}')";
					$db->Query($sqq);
            ...
            elseif($sadereh_flag==1)
            {
                ...
                if($sadereh_permission == count($permission_users_array)){
                    switch (intval($rsq[0]['Transactions'])){
                        ...
                        //     $rsq1 = $db->ArrayQuery($sqq1);
                        //     if (count($rsq1) <= 0){
                        //         $res = "امضا مدیر بازرگانی خرید الزامی 5می باشد !!!";
                        //         $out = "false";
                        //         response($res, $out);
                        //         exit;
                        //     }
                        ...
                        //     $rsq1 = $db->ArrayQuery($sqq1);
                        //     if (count($rsq1) <= 0){
                        //         $res = "امضا معاونت بازرگانی444 الزامی می باشد !!!!!";
                        //         $out = "false";
                        //         response($res, $out);
                        //         exit;
                        //     }
    ...
    public function finalApprovalComment($pid){
       ...
       // $$receiverTime=date('H:i:s',strtotime("-1 hour"));
        ...
        if ((intval($totalAmount) + $fAmount) >= ($res[0]['Amount'] + $rAmount)){   // پرداختی مازاد یا برابر با مبلغ کل اظهارنظر بود
            ...
            if ($res[0]['sendType'] == 0) { // فورج نقدی بود
