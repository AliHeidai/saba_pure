        ...
        if($res)
        {
            ...
            if($result>0)
            {
                ...
                for($k=0;$k<count($pid_array);$k++){
                    $values.="({$sender},{$receiver},{$pid_array[$k]},1,'{$dateCreate}','{$timeCreate}','{$description}'),";
                }
                $values=rtrim($values,',');
      ...
      if($res[0]['related_vat']==1){
          ...
          if(count($res2)>0){
             $res_array=[];
               ...
               $res_array['related_vat'] = $res[0]['related_vat'];
...
public function OtherInfoContractHTM($cid){
    ...
    $ut = new Utility();
    $db = new DBi();
    $sql_addendum="SELECT * FROM `contract_addendum` WHERE `contract_id`={$cid}";
