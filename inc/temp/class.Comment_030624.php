...
/*        $items[$c]['options'] = array();
        ...
        if($is_exception_user==0)
        {
            ...
            if(count($res)>0)
            {
                ...
                if($acm->hasAccess('fullAccessEditPayComment')){
                   $htm.='<button onclick="editSelectedPayComment" class="btn btn-primary"> ویرایش</button>';
            ...
            if (intval($res) > 0){
               ...
               // if (intval($_SESSION['userid']) !== 14){
                ...
                if($send_sms==1){
                    ...
                    $res_s=$sms-> send_by_pattern_api($phone,"hyg4w6794bvh1ws",json_encode(array("cyekta"=>$un_code,"sabauser"=>$sender_name)));
    ...
    public function transferPartnersPayCommentCartable($pid_array,$sender,$receiver){
        ...
        foreach($pid_array as $key=>$value){
         ...
         $get_last_pw_id_sql="SELECT `RowID` FROM `payment_workflow` WHERE `receiver`={$receiver}  AND `pid`={$value['pid']} order by RowID DESC LIMIT 1";
