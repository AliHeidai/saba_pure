...
class Comment{
    public $current_date="";
    public $current_time="";
    private $finance_manager_user_arr=[72];
...
/*        $items[$c]['options'] = array();
    ...
    public function createPayComment($pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalTransactionAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$files,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type,$PropertyNumber,$creditName){
       ...
       $limit_access_users_edit_pay_comment=[14]; //کاربران قادر به ویرایش اظهارنظر    تازمانی که اظهار نظر از گردش خارج از واحد نداشته باشد  (مدیران واحدها)
        ...
        if ($flag) {
            ...
            $sql = "UPDATE `pay_comment` SET `type`='{$Type}',`Toward`='{$Toward}',`totalAmount`={$totalTransactionAmount},`Amount`={$Amount},`CashSection`={$CashSection},`NonCashSection`={$NonCashSection},
                   ...
                   `sabtDarHesabType`={$sabtDarHesabType},`cardNumber`='{$CardNumber}',`layer3`={$layer3},`accountID`={$accountID},`goodLoan`='{$goodLoan}',`checkOutType`='{$checkOutType}',`related_project`='{$related_project}',`related_VAT`='{$related_vat}',`PropertyNumber`='{$PropertyNumber}',`creditor_id`='{$creditName}' WHERE `RowID`={$cid}";
            $db->Query($sql);
            $res = $db->AffectedRows();
            $res = (($res == -1) ? 0 : 1);
            if (intval($res)) {
    ...
    public function commentWorkflowHtm($pid,$meta_data=""){
        ...
        { //
           ...
           // unset($rowids[$user_array_key]);
            $rids = implode(',',$rowids);
            $ut->fileRecorder(intval('unit1:'.$rstun[0]['unitID']));
        }
        else
        {
            ...
            {//اگر بارنامه صادره بود----------------------------------
                ...
                if($rsq[0]['sendType']==2){// اظهارنظر سهامی  باشد
                 //  $permission_users_array=$ut->get_full_access_users(17);//کاربران مجاز به تایید بارنامه صادره سهامی
                    $permission_users_array=[14,72,54];//کاربران مجاز به تایید بارنامه صادره سهامی
