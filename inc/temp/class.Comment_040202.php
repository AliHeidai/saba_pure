...
//             $htm .= '</table>';
    ...
    public function createReceivedCustomer($RType,$RMethod,$amount,$rDate,$CstName,$CstCode,$desc,$bank,$pos,$serial,$chDate,$chOwner,$chOwnerC,$chBankIssuingBank,$is_sayyad,$depositDate){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        $sql = "INSERT INTO `customer_receive` (`receiveType`,`receiveMethod`,`checkNumber`,`receiveAmount`,`receiveDate`,`depositDate`,`uid`,`depositor`,`codeTafzili`,`description`,`bankID`,`posID`,`checkDate`,`checkSerial`,`checkOwner`,`checkOwnerCode`,`check_issuing_bank`,`is_sayyad`)
                VALUES ({$RType},{$RMethod},{$checkNumber},{$amount},'{$rDate}',{$depositDate},{$_SESSION['userid']},'{$CstName}',{$CstCode},'{$desc}',{$bank},{$pos},'{$chDate}','{$serial}','{$chOwner}','{$chOwnerC}','{$chBankIssuingBank}','{$is_sayyad}')";
        $res = $db->Query($sql);
        if (intval($res) > 0){
            return true;
        }else{
    ...
    public function editReceivedCustomer($rid,$RType,$RMethod,$amount,$rDate,$CstName,$CstCode,$desc,$bank,$pos,$chBankIssuingBank,$is_sayyad,$depositDate){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        $sql = "UPDATE `customer_receive` SET `receiveType`={$RType},`receiveMethod`={$RMethod},
                `receiveAmount`={$amount},`receiveDate`='{$rDate}',`depositDate`={$depositDate},`uid`={$_SESSION['userid']},`depositor`='{$CstName}',
                `codeTafzili`={$CstCode},`description`='{$desc}',`bankID`={$bank},`posID`={$pos},`check_issuing_bank`='{$chBankIssuingBank}',`is_sayyad`='{$is_sayyad}' WHERE `RowID`={$rid}";
        $db->Query($sql);
        $res = $db->AffectedRows();
