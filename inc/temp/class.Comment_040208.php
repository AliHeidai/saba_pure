...
/*                $bottons9[3]['title'] = "تاریخ های صدور اظهانظر";
        ...
        if (in_array('receivedCustomerManage',$access_array)) {
            ...
            if(in_array($_SESSION['userid'], $claims_arriveـusers)){
                $bottons10[0]['title'] = "ثبت دریافتی جدید";
...
//**********************************************
    ...
    public function getPrintPayCommentHtm($cid){
        ...
        for ($i=0;$i<$cnt;$i++) {
            ...
            if ($rsig[$i]['sender'] == 44 || $rsig[$i]['sender'] == 149){  // کارشناس حسابداری
...
//             $htm .= '</table>';
            ...
            {//اگر بارنامه صادره بود----------------------------------
                ...
                if($rsq[0]['sendType']==2){// اظهارنظر سهامی  باشد
                 //  $permission_users_array=$ut->get_full_access_users(17);//کاربران مجاز به تایید بارنامه صادره سهامی
                    $permission_users_array=[14,$this->finance_manager_user_arr[0],54];//کاربران مجاز به تایید بارنامه صادره سهامی
    ...
    public function createReceivedCustomer($arr){
        ...
        $sql = "INSERT INTO `customer_receive` (`receiveType`,`receiveMethod`,`checkNumber`,`receiveAmount`,`receiveDate`,`depositDate`,`uid`,`depositor`,`codeTafzili`,`description`,`bankID`,`posID`,`checkDate`,`checkSerial`,`checkOwner`,`checkOwnerCode`,`check_issuing_bank`,`is_sayyad`,`did`)
                VALUES ({$RType},{$RMethod},{$checkNumber},{$amount},'{$rDate}','{$depositDate}',{$_SESSION['userid']},'{$Customer_name}',{$Customer_tafzoili},'{$desc}',{$bank},{$pos},'{$chDate}','{$serial}','{$chOwner}','{$chOwnerC}','{$chBankIssuingBank}','{$is_sayyad}','{$CstName}')";
        $res = $db->Query($sql);
        $ut->fileRecorder($sql);
        if (intval($res) > 0){
            return true;
    ...
    public function editReceivedCustomer($arr){
        ...
        $sql = "UPDATE `customer_receive` SET `receiveType`={$RType},`receiveMethod`={$RMethod},
                `receiveAmount`={$amount},`receiveDate`='{$rDate}',`depositDate`='{$depositDate}',`uid`={$_SESSION['userid']},`depositor`='{$Customer_name}',
                `codeTafzili`={$Customer_tafzoili},`description`='{$desc}',`bankID`={$bank},`posID`={$pos},`check_issuing_bank`='{$chBankIssuingBank}',`is_sayyad`='{$is_sayyad}',`did`='{$CstName}' WHERE `RowID`={$rid}";
        $db->Query($sql);
