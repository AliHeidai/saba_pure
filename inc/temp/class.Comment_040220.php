...
class Comment{
    public $current_date="";
    public $current_time="";
    private $finance_manager_user_arr=[175];
    private $it_manager_user_arr=[67];
    private $ceo_manager_arr=[4];
    private $human_resource_manager_arr=[3];
    private $chief_accounting_arr=[173];
    ...
    public function getCommentRecivers(){
        $db=new DBi();
        $ut=new Utility();
        $sql="SELECT u.`RowID`,u.`fname`,u.`lname` FROM `users` as u inner join `access_table` as a on a.user_id=u.RowID  WHERE u.`isEnable`=1 and a.access_type=1 AND u.`RowID`=a.`user_id` AND a.`item_id`=38   ";
    ...
    public function getCommentManagementHtm(){
       ...
       // in_array('commentManagement',$access_array)
        ...
        $countPayrolls = count($payrolls);

        $get_recivers = $this->getCommentRecivers();
        ...
        if(in_array('accountManagement',$access_array)) {
            ...
            if(in_array('createAccount',$access_array)) {
                ...
                if(in_array('groupCreateAccount',$access_array)){
                    $n++;
                    $bottons2[$n]['title'] = "ایجاد گروهی طرف حساب";
                    $bottons2[$n]['jsf'] = "createGroupAccount";
        ...
        if (in_array('sendCommentManagement',$access_array)) {
            ...
            //-------------------------------------------
            $c=0;
            if(in_array('changeCommentForceStatus',$access_array)){
                
                $bottons8[$c]['title'] = "مدیریت وضعیت در اسرع وقت";
                $bottons8[$c]['jsf'] = "toggle_comment_force_status";
                $bottons8[$c]['icon'] = "fa-solid fa-meteor";
                $bottons8[$c]['icon'] = "manage_partners_cartable";
            }
            //-------------------------------------------
            $bottons[$y] = $bottons8;
...
/*                $bottons9[3]['title'] = "تاریخ های صدور اظهانظر";
        ...
        if (in_array('receivedCustomerManage',$access_array)) {
            ...
            $claims_arriveـusers=$ut->get_full_access_users(30);
            $btn_index=0;
           if(in_array($_SESSION['userid'], $claims_arriveـusers)){
                if(in_array('createReceivedCustomerManage',$access_array)){
                    $bottons10[$btn_index]['title'] = "ثبت دریافتی جدید";
                    $bottons10[$btn_index]['jsf'] = "createReceivedCustomer";
                    $bottons10[$btn_index]['icon'] = "fa-plus-square";
                    $bottons10[$btn_index]['title'] = "ثبت دریافتی";
                    $bottons10[$btn_index]['jsf'] = "createReceivedCustomer";
                    $bottons10[$btn_index]['icon'] = "fa-plus-square";
                    $btn_index++;
                }
                
                if(in_array('editReceivedCustomerManage',$access_array)){
                    $bottons10[$btn_index]['title'] = "ویرایش دریافتی";
                    $bottons10[$btn_index]['jsf'] = "editReceivedCustomer";
                    $bottons10[$btn_index]['icon'] = "fa-edit";
                    $btn_index++;
                }

                $bottons10[ $btn_index]['title'] = "ارسال";
                $bottons10[ $btn_index]['jsf'] = "sendReceivedCustomer";
                $bottons10[ $btn_index]['icon'] = "fa-paper-plane";
                $btn_index++;

                if(in_array('attachReceivedCustomerManage',$access_array)){
                    $bottons10[$btn_index]['title'] = "پیوست فایل";
                    $bottons10[$btn_index]['jsf'] = "attachReceivedCustomerFile";
                    $bottons10[$btn_index]['icon'] = "fa-link";
                    $btn_index++;
                }
           }
                $bottons10[$btn_index]['title'] = "خروجی اکسل";
                $bottons10[$btn_index]['jsf'] = "getExcelReceivedCustomer";
                $bottons10[$btn_index]['icon'] = "fa-file-excel";
                $btn_index++;
            ...
            $headerSearch10[$a]['type'] = "text";
            $headerSearch10[$a]['width'] = "150px";
            $headerSearch10[$a]['id'] = "receivedCustomerChkNumSearch";
            $headerSearch10[$a]['title'] = "شماره چک";
            $headerSearch10[$a]['placeholder'] = "شماره چک";
            $a++;
           if(in_array('AccessAllReceivedCustomers',$access_array)){
            $headerSearch10[$a]['type'] = "select";
            $headerSearch10[$a]['width'] = "150px";
            $headerSearch10[$a]['id'] = "receivedCustomerRLastReciverSearch";
            $headerSearch10[$a]['title'] = "آخرین دریافت کننده";
            $headerSearch10[$a]['options'][0]["title"] = ' آخرین دریافت کننده';
            $headerSearch10[$a]['options'][0]["value"] = -1;
            for($i=1;$i<count($get_recivers); $i++){
                $headerSearch10[$a]['options'][$i]["title"] = $get_recivers[$i]['fname']." ".$get_recivers[$i]['lname'];
                $headerSearch10[$a]['options'][$i]["value"] =$get_recivers[$i]['RowID'];
            }
...
/*            for ($i = 0; $i < $cntu; $i++) {
        ...
        //+++++++++++++++++++++++++++++++++++++ثبت طرف حساب گروهی++++++++++++++++++++++++++++++++++
        $items=[];
        $modalID = "CreateGroupAccounts";
        $modalTitle = "ایجاد گروهی طرف حسابها";
        $ShowDescription = 'CreateGroupAccounts-body';

        $c = 0;
        
        $items[$c]['type'] = "file";
        $items[$c]['id'] = "CreateGroupAccountsAttachmentFile";
        $items[$c]['title'] = "بارگذاری فایل های پیوست";
        $items[$c]['name'] = 'name="files[]"';
        $items[$c]['helpText'] = "نوع فایل باید XLSX,XLS باشد.";
        $style = 'style="max-width: 900px;"';
        $c++;

        $items[$c]['type'] = "hidden";
        $items[$c]['id'] = "CreateGroupAccountsAttachmentID";

        $footerBottons = array();
        $footerBottons[0]['title'] = "ارسال فایل";
        $footerBottons[0]['jsf'] = "doAttachGroupAccountsFile";
        $footerBottons[0]['type'] = "btn";
        $footerBottons[0]['data-dismiss'] = "NO";
        $footerBottons[1]['title'] = "بستن";
        $footerBottons[1]['type'] = "dismis";
        $CreateGroupAccountsAttachmentFile = $ut->getHtmlModal($modalID,$modalTitle,$items,$footerBottons,'','',$style,$ShowDescription);
             //+++++++++++++++++++++++++++++++++++++ثبت طرف حساب گروهی++++++++++++++++++++++++++++++++++
        ...
        //--------------------------------------------------------
        $items[$c]['type'] = "select";
        $items[$c]['id'] = "RevolvingPayroll";
        $items[$c]['options'][0]["title"] = 'تنخواه گردان را انتخاب نمایید';
        $items[$c]['options'][0]["value"] = 0;
        for ($i=0;$i<$countPayrolls;$i++){
            $items[$c]['options'][$i+1]["title"] = $payrolls[$i]['fullname'];
            $items[$c]['options'][$i+1]["value"] = $payrolls[$i]['RowID'];
        }
        $items[$c]['title'] = "تنخواه گردان";
        $c++;
       ...
       // $items[$c]['style'] = "style='width: 50%;float: right;'";
        ...
        $items[$c]['type'] = "select";
        $items[$c]['id'] = "creditorName";
        $items[$c]['title'] = "طرف حساب بستانکار";
...
/*        $items[$c]['options'] = array();
        ...
        $items[$c]['title'] = "بارگذاری فایل های پیوست";
        $items[$c]['name'] = 'name="files[]"';
        $items[$c]['helpText'] = "نوع فایل باید PDF, JPG , PNG , XLSX  باشد.";
        $items[$c]['multiple'] = 'multiple';
        $c++;
        ...
        $items[$c]['type'] = "select";
        $items[$c]['id'] = "receivedCustomerCstName";
        $items[$c]['onclick'] = "onclick='getCustomerCodeTafzili()'";
        $items[$c]['title'] = "طرف مقابل";
        $items[$c]['placeholder'] = "نام";
        $items[$c]['options'][0]["title"] = "نام طرف حساب را وارد نمایید";
        $items[$c]['options'][0]["value"] = 0;
        $c++;
        ...
        $items[$c]['type'] = "text";
        $items[$c]['id'] = "receivedCustomerCheckBankName";
        $items[$c]['title'] = " نام بانک صادر کننده چک";
        $items[$c]['placeholder'] = "بانک صادر کننده چک";
        $c++;

        $items[$c]['type'] = "radio";
        $items[$c]['name'] = "receivedCustomerIsSayyad";
        $items[$c]['title'] = " چک در سامانه صیاد ثبت شده است؟";
        $items[$c]['options'][0]['title'] = "بله ";
        $items[$c]['options'][0]['value'] = 1;
        $items[$c]['options'][1]['title'] = "  خیر";
        $items[$c]['options'][1]['value'] = 0;
        ...
        $items[$c]['type'] = "text";
        $items[$c]['id'] = "customerDepositDate";
        $items[$c]['style'] = "style='width: 70%;float: right;'";
        $items[$c]['title'] = "تاریخ واریز";
        $items[$c]['placeholder'] = "تاریخ";
        $c++;
        ...
        $htm .= $CreateGroupAccountsAttachmentFile;
    ...
    public function get_customers_recived_users(){
        $ut=new Utility();
        $receivers=[array('RowID'=>0,'fullname'=>'گیرنده را انتخاب نمایید')];
        $users=$ut->get_full_access_users(30);
        foreach($users as $user){
            $array=[];
            $array['RowID']=$user;
            $array['fullname']=$ut->get_user_fullname($user);
            $receivers[]=$array;
        }
        return $receivers;
    ...
    public function get_customer_receiver_workflow($arr){
        $db=new DBi();
        $ut=new Utility();
        extract($arr);
        $sql="SELECT * FROM `customer_receive_workflow` where CRid={$CRid}";
        $ut->fileRecorder('testttttttttttttttttttttttttt.'.$sql);
        $res=$db->ArrayQuery($sql);
        if(count($res)>0){
            $htm="<table class='table table-bordered table-striped'><thead><tr><th>ردیف</th><th>فرستنده</th><th>گیرنده</th><th>تاریخ ارسال</th><th>زمان ارسال</th><th>توضیحات</th></tr><tbody>";
            $counter=1;
            foreach($res as $row){
                $htm.="<tr>
                        
                        <td class=\"text-right\">{$counter}</td>
                        <td class=\"text-right\">{$ut->get_user_fullname($row['sender'])}</td>
                        <td class=\"text-right\">{$ut->get_user_fullname($row['receiver'])}</td>
                        <td class=\"text-right\">{$ut->greg_to_jal($row['createDate'])}</td>
                        <td class=\"text-right\">{$row['createTime']}</td>
        ...
        return '<p style="font-family:IranSans" class="text-danger text-center"> فاقد گردش</p>';
    }

    public function doSendReceivedCustomer($arr){
        $db=new DBi();
        extract($arr);
        $currDate=date('Y-m-d');
        $currTime=date('H:i:s');
        $sql="INSERT INTO `customer_receive_workflow` (`sender`,`receiver`,`CRid`,`status`,`createDate`,`createTime`,`description`)
        VALUES('{$_SESSION['userid']}','{$receiver}','{$CRid}','{$status}','{$currDate}','{$currTime}','{$desc}')";
        $res=$db->Query($sql);
        if($res){
            $u_sql="UPDATE `customer_receive` SET `lastReceiver`='{$receiver}' WHERE `RowID`='{$CRid}'";
            $res_u=$db->Query($u_sql);
            return 1;
        }
        return 0;
    }
    ...
    public function getPayCommentManageList($csDate,$ceDate,$cUnit,$coUnit,$caName,$cToward,$Uncode,$amount,$page=1,$layer_one,$layer_two,$layer_three){
        ...
        for($y=0;$y<$listCount;$y++){
            ...
            $finalRes[$y]['is_vat'] = $res[$y]['related_vat']?$res[$y]['related_vat']:0;
        }
        ...
        if(count($res) == 1){
            if (strlen(trim($res[0]['accNumber'])) > 0){
                $res = array("cid"=>$cid,"Unit"=>$res[0]['Unit'],"type"=>$res[0]['type'],"Toward"=>$res[0]['Toward'],"consumerUnit"=>$res[0]['consumerUnit'],"Amount"=>number_format($res[0]['Amount']),"totalAmount"=>number_format($res[0]['totalAmount']),
                             ...
                             "checkOutType"=>$res[0]['checkOutType'],'related_project'=>$res[0]['related_project'],'related_vat'=>!empty($res[0]['related_vat'])?$res[0]['related_vat']:0,'transfer'=>!empty($res[0]['transfer'])?$res[0]['transfer']:0,
                             'PropertyNumber'=>!empty($res[0]['PropertyNumber'])?$res[0]['PropertyNumber']:0,'creditor_id'=>!empty($res[0]['creditor_id'])?$res[0]['creditor_id']:0,
                ...
                $res = array("cid"=>$cid,"Unit"=>$res[0]['Unit'],"type"=>$res[0]['type'],"Toward"=>$res[0]['Toward'],"consumerUnit"=>$res[0]['consumerUnit'],"totalAmount"=>number_format($res[0]['totalAmount']),
                             ...
                             'related_project'=>$res[0]['related_project'],'related_vat'=>!empty($res[0]['related_vat'])?$res[0]['related_vat']:0,'transfer'=>!empty($res[0]['transfer'])?$res[0]['transfer']:0,
                             'PropertyNumber'=>!empty($res[0]['PropertyNumber'])?$res[0]['PropertyNumber']:0,'creditor_id'=>!empty($res[0]['creditor_id'])?$res[0]['creditor_id']:0,
                             'is_drivers_fare'=>!empty($meta_array['is_drivers_fare'])?$meta_array['is_drivers_fare']:0,'is_drivers_fare_type'=>!empty($meta_array['is_drivers_fare_type'])?$meta_array['is_drivers_fare_type']:0
                );
                return $res;
    ...
    public function createPayComment($pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalTransactionAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$files,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type,$PropertyNumber,$creditName,$payrolls){
       ...
       $ut = new Utility();
        ...
        // if((intval($MaturityCheck)+intval($MaturityCash))!=intval($Amount)){
        //     return -1;
        // }
        // if(intval($totalTransactionAmount)<intval($MaturityCheck)+intval($MaturityCash)){
        //     return -3;
        // }
	...
	public function editPayComment($cid,$pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalTransactionAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$files,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type,$PropertyNumber,$creditName,$payrolls){
       ...
       $limit_access_users_edit_pay_comment=[14]; //کاربران قادر به ویرایش اظهارنظر    تازمانی که اظهار نظر از گردش خارج از واحد نداشته باشد  (مدیران واحدها)
        $this->set_comment_view_date($cid,'ویرایش اظهارنظر &edit');
        ...
        if(in_array($_SESSION['userid'],$limit_access_users_edit_pay_comment)){
            ...
            $res_rec=$db->ArrayQuery($sql_reciver);
          //  if(count($res_rec)==1 && $res_rec[0]['unitID']==9){
            if(count($res_rec)==1){
                $user_sql="SELECT unitID from users where RowID={$_SESSION['userid']} AND unitID={$res_rec[0]['unitID']}";
        ...
        if ($flag) {
            $this->duplicate_payComment_row($cid,'pay_comment_history','pay_comment');
            $sql = "UPDATE `pay_comment` SET `type`='{$Type}',`Toward`='{$Toward}',`totalAmount`={$totalTransactionAmount},`Amount`={$Amount},`CashSection`={$CashSection},`NonCashSection`={$NonCashSection},
                   ...
                   `sabtDarHesabType`={$sabtDarHesabType},`cardNumber`='{$CardNumber}',`layer3`={$layer3},`accountID`={$accountID},`goodLoan`='{$goodLoan}',`checkOutType`='{$checkOutType}',`related_project`='{$related_project}',`related_VAT`='{$related_vat}',`PropertyNumber`='{$PropertyNumber}',`creditor_id`='{$creditName}',`pay_rolls`='{$payrolls}' WHERE `RowID`={$cid}";
            $db->Query($sql);
            $res = $db->AffectedRows();
            $res = (($res == -1) ? 0 : 1);
            if (intval($res)) {
    ...
    public function duplicate_payComment_row($cid,$dist_table,$origin_table){
        $db=new DBi();
        $ut=new Utility();
        $user_id=$_SESSION['userid'];
        $editing_date=date('Y-m-d H:i:s');
        $sql="INSERT INTO `$dist_table` (
            `pid`,
            `type`,                  
            `cDate`,
            `Toward`,
            ...
            '{$editing_date}',
            'pay_rolls'
            from `{$origin_table}` where `RowID`={$cid}";
            $res=$db->Query($sql);
    ...
    public function duplicate_payComment_row2($cid,$dist_table,$origin_table,$codeTafzili){
        $db=new DBi();
        $ut=new Utility();
        $user_id=$_SESSION['userid'];
        $editing_date=date('Y-m-d H:i:s');
        ...
        $unCode = $datetostring.$codeTafzili.substr(time(), -4).rand(100,999);
        $ut->fileRecorder('unCode'.$unCode);
        $sql="INSERT INTO `$dist_table` (
            ...
            if($res){
                $ut->fileRecorder('InsertrdID:',$db->InsertrdID());
                return $db->InsertrdID();
            }
    }
    ...
    public function attachFileToComment($cid,$info,$files){
       ...
       // $cTime = date('H:i:s');
        ...
        $this->set_comment_view_date($cid,'پیوست فایل &attach');
        $SFile = array();
            ...
            if (intval($res) > 0){
               ...
               // if (intval($_SESSION['userid']) !== 14){
                ...
                if($send_sms==1){
                    ...
                    $res_s=$sms-> send_by_pattern_api($phone,"l6ob1mton7j89q8",json_encode(array("cyekta"=>$un_code,"sabauser"=>$sender_name,'server'=>SERVER)));
    ...
    public function attachmentFileCommentHtm($pid,$show){
        ...
        $this->set_comment_view_date ($pid,'مستندات &mostanadat');
        return $htm;
    }
    ...
    public function commentWorkflowHtm($pid,$meta_data=""){
        ...
        $this->set_comment_view_date($pid,'ارجاع اظهارنظر &erja');
       
        $sqq = "SELECT `Transactions`,`checkOutType`,`related_vat`,`lastReceiver` FROM `pay_comment` WHERE `RowID`={$pid}";
        $rsq = $db->ArrayQuery($sqq);
        $lastReceiver=$rsq[0]['lastReceiver'];

        $sqqq="SELECT * FROM `payment_workflow` where `receiver`={$lastReceiver} AND `pid`={$pid}  ORDER BY RowID DESC LIMIT 1";
        $last_sender=$db->ArrayQuery($sqqq);
      
        $users_see_temp_comments=[$lastReceiver];
        $html_seen_sign='<i class="fa fa-eye text-primary"></i>';
                    ...
                    if($get_meta_res[0]['value'] == 1){//بارنامه صادره می باشد
                       if($get_meta_res[0]['sendType']==0 || $get_meta_res[0]['sendType']==1){
                           // $rowids=$ut->get_full_access_users(16);
                            $rowids=$ut->get_full_access_users(29);
                       }
        ...
        {
            $import_expert_users_arry=$ut->get_full_access_users(28);
            //************************************** */
            if (intval($rstun[0]['unitID']) == 9 || intval($rstun[0]['unitID']) == 37){  // اگر تدارکات یا انبار محصول بود
                ...
                $in_related_units_array=[20,23];
                if($_SESSION['userid']==14){
                    $in_related_units_array=[20,23,22];
                }
                $in_statement=implode(",",$in_related_units_array);
                switch (intval($rsq[0]['Transactions'])){
                    case 1:  // جزئی
                        $sqlu = "SELECT `uids` FROM `relatedunits` WHERE `RowID` IN ({$in_statement})";  // تدارکات - مالی
                        $rstu = $db->ArrayQuery($sqlu);
                        for ($i=0;$i<count($rstu);$i++){
                            $rowids[] = $rstu[$i]['uids'];
                        }
                        ...
                        break;
                    case 2:  // متوسط
                        $sqlu = "SELECT `uids` FROM `relatedunits` WHERE `RowID` IN ({$in_statement})";
                        $rstu = $db->ArrayQuery($sqlu);
                        for ($i=0;$i<count($rstu);$i++){
                            $rowids[] = $rstu[$i]['uids'];
                        }
                ...
                if(in_array($_SESSION['userid'],$import_expert_users_arry)){
                    $rids.=",4";
                }
            }
            //************************************** */
            ...
            {  // اگر پشتیبانی یا حقوقی بود
                ...
                switch (intval($rsq[0]['Transactions'])){
                    case 1:  // جزئی
                        $sqlu = "SELECT `uids` FROM `relatedunits` WHERE `RowID` IN(23,20,21)";
                        $rstu = $db->ArrayQuery($sqlu);
                        for ($i=0;$i<count($rstu);$i++){
                            $rowids[] = $rstu[$i]['uids'];
                        }
            ...
            // if (intval($rstun[0]['unitID']) == 26){//} || intval($rstun[0]['unitID']) == 27 || intval($rstun[0]['unitID']) == 28){  // اگر مدیریت یا فناوری اطلاعات یا مالی بود
        ...
        $this->set_comment_view_date ($pid,'مشاهده سوابق ارجاع &savabegh erja');
        ...
        for ($i=0;$i<$cnt;$i++){
            ...
            $res[$i]['createDate'] = $ut->greg_to_jal($res[$i]['createDate']);
            // $ut->fileRecorder('users_see_temp_comments:');
            // $ut->fileRecorder($users_see_temp_comments);
            if ($res[$i]['temp'] == 1){// && in_array($_SESSION['userid'],$users_see_temp_comments)){
            ...
            $receiver_name=$rst[0]['fname'].' '.$rst[0]['lname'];
            ...
            $htm .= '<td style="text-align: center;font-family: dubai-Regular;padding: 10px;'.$style.'">'.$receiver_name.'</td>';
            ...
            if($acm->hasAccess('ViewHistoryCommentManagement')){
                ...
                {
                    $htm .= '<td style="text-align: center;font-family: dubai-Regular;padding: 10px;'.$style.'"><span onclick="get_view_detailes('.$iterator.','.$res[$i]['RowID'].',\''.$receiver_name.'\')" style="cursor:pointer">'.$html_seen_sign.'</span></td>';
                }
                else{
...
//**********************************************
    ...
    public function get_view_detailes($arr){
        $db=new DBi();
        $ut=new Utility();
        extract($arr);
        $sql="SELECT `view_history` FROM payment_workflow where RowID={$row_id}";
        $res=$db->ArrayQuery($sql);
        $view_json=$res[0]['view_history'];
        $view_array=json_decode($view_json,true);
        $html.='<table class="table table-striped table-borderd">';
        $html.='<thead> <tr><th>ردیف</th><th>تاریخ بازدید</th><th>توضیحات</th></tr></thead>';
        $html.='<tbody>';
        $counter=1;
        $last_time='';
        foreach($view_array as $key=>$value){
            $value_arr=explode('&',$value['desc']);
            switch(trim($value_arr[0])){
                
                case "u0645u0633u062au0646u062fu0627u062a"://مستندات
                    $utf8_string="مستندات";
                break;
                case "u0645u0634u0627u0647u062fu0647 u0644u06ccu0633u062a u062au0646u062eu0648u0627u0647": //tankhah
                    $utf8_string="مشاهده لیست تنخواه ";
                break;
               
                case "u0627u0631u062cu0627u0639 u0627u0638u0647u0627u0631u0646u0638u0631": //erja
                    $utf8_string="ارجاع اظهار نظر";
                break;
                case "u0645u0634u0627u0647u062fu0647 u0633u0648u0627u0628u0642 u0627u0631u062cu0627u0639": //savabegh erja
                    $utf8_string="مشاهده سوابق ارجاع اظهارنظر";
                break;
                case "print u0627u0638u0647u0627u0631u0646u0638u0631": //print
                    $utf8_string="print اظهارنظر";
                break;
                case "u0633u0627u06ccu0631 u0627u0637u0644u0627u0639u0627u062a": //print
                    $utf8_string="مشاهده سایر اطلاعات اظهارنظر";
                break;
                default:
                $utf8_string = $value['desc'];
                break;
            }
           
            
            $date_time=explode(' ',$value['date']);
            if($last_time!=$date_time){
                $html.="<tr>
                <td>".$counter."</td>
                <td>".$ut->greg_to_jal($date_time[0])." ".$date_time[1]."</td>
                <td>".$utf8_string."</td>
                <tr>";
            $counter++;
            
            }
            $last_time=strtotime($date_time);
    ...
    public function getAccountNames(){
        $db = new DBi();
        $sql = "SELECT `Name`,`RowID`,`code` FROM `account`";
        $res = $db->ArrayQuery($sql);
        if(count($res)>0){
            return $res;
        }else{
    ...
    public function OtherInfoCommentHTM($cid){
        ...
        $db = new DBi();
        $other_pay_info="";
        $sql = "SELECT `unCode`,`Amount`,`layer1`,`layer2`,`layer3`,`type`,`creditor_id`,`totalAmount`,`CashSection`,`paymentMaturityCash`,`BillingID`,`PaymentID`,`Transactions`,`RequestSource`,`RequestNumbers`,`desc`,`related_project` FROM `pay_comment` WHERE `RowID`={$cid}";
        ...
        if ($res[0]['type'] == 'پرداخت قبض' || $res[0]['type'] == 'پرداخت جریمه'){
           ...
           // }
            ...
            $sql = "SELECT `unCode`,`layer1`,`layer2`,`layer3`,`clearingFundDate`,`type`,`creditor_id`,`totalAmount`,`CashSection`,`paymentMaturityCash`,`NonCashSection`,`paymentMaturityCheck`,`Transactions`,`accName`,`accNumber`,`accBank`,`codeTafzili`,`nationalCode`,`RequestSource`,`RequestNumbers`,`contractNumber`,`tick`,`checkNumber`,`checkDate`,`checkCarcass`,`checkDeliveryDate`,`RowID`,`desc`,`cardNumber` FROM `pay_comment` WHERE `pay_comment`.`RowID`={$cid}";
            $res = $db->ArrayQuery($sql);
            $infoNames = array('شماره یکتا','سر گروه','زیر گروه','زیرگروه فرعی','تاریخ تسویه تنخواه','نوع','طرف حساب بستانکار','مبلغ مربوط به کل معامله','بخش نقدی','سررسید پرداخت نقدی','بخش چک','سررسید پرداخت چک','طبقه معاملات','نام طرف حساب','شماره حساب','نام بانک و شعبه','کد تفضیلی','کد ملی','منبع درخواست','شماره درخواست','شماره قرارداد','پرینت گرفته و مستندات پیوست شده است','شماره چک','تاریخ چک','لاشه چک تحویل واحد مالی','تعهد تاریخ تحویل لاشه چک به واحد مالی','رسید تحویل چک','توضیحات','شماره کارت');
            $direction = '';
            for ($i=0;$i<count($infoNames);$i++){
                ...
                if($i==6){
                    $acc_sql="SELECT Name FROM `account` where RowID={$res[0]["$keyName"]}";
                    $res_a=$db->ArrayQuery($acc_sql);
                    $res[0]["$keyName"] = $res_a[0]['Name'];
                }
                if ($i == 7 || $i == 8 || $i == 10){
                    $res[0]["$keyName"] = number_format($res[0]["$keyName"]).' ریال';
                }
        ...
        $htm .= '</table>';
        $this->set_comment_view_date ($cid,'سایر اطلاعات & sayer etelaat');
        return $htm;
    }
    ...
    public function get_account_name($acc_id){
        $db=new DBi();
        $sql="SELECT * FROM `account` WHERE `RowID`={$acc_id}";
        $res=$db->ArrayQuery($sql);
        return $res[0];
    }
    ...
    public function getPrintPayCommentHtm($cid){
        ...
        $ut = new Utility();
        $db = new DBi();
        $this->set_comment_view_date ($cid,'print اظهارنظر &print');
        //------------------------------------------------------------------watermark---------------------------------------------
        $watermark="";
        ...
        if($res[0]['type']=='ثبت در حساب بستانکاری طرف مقابل'){
            $creditor_info="<span style='font-size: 30px;font-family: BNazanin;font-weight: bold;text-align: right'> طرف مقابل بستانکار :</span> <span style='font-size: 25px;font-family: BTitr;'>".$this->get_account_name($res[0]['creditor_id'])['Name']."</span>";
        }
        if($res[0]['related_project']>0){
        ...
        $beginner = '<img class="signature_class" width="120px;" height="120px;" src="'.ADDR.'Signature/'.$res[0]['signature'].'">';
        ...
        for ($i=0;$i<$cnt;$i++) {
            if ($rsig[$i]['sender'] == 3 || $rsig[$i]['sender'] == 14 || $rsig[$i]['sender'] == 67 || $rsig[$i]['sender'] == 68){  // حقیقت و مصطفوی، عفت پناه، پور حسین
                $Sarparast = '<img class="signature_class" width="120px;" height="120px;" src="'.ADDR.'Signature/'.$rsig[$i]['signature'].'">';
                $fSarparast = $rsig[$i]['postJob'].' '.$ut->greg_to_jal($rsig[$i]['createDate']).' '.$rsig[$i]['fname'].' '.$rsig[$i]['lname'];
            }
            if ($rsig[$i]['sender'] == 20){  // معاونت بازرگانی
                $Moavenat = '<img class="signature_class" width="120px;" height="120px;" src="'.ADDR.'Signature/'.$rsig[$i]['signature'].'">';
                $fMoavenat = $rsig[$i]['postJob'].' '.$ut->greg_to_jal($rsig[$i]['createDate']).' '.$rsig[$i]['fname'].' '.$rsig[$i]['lname'];
            }
            if ($rsig[$i]['sender'] == 44 || $rsig[$i]['sender'] == 180){  // کارشناس حسابداری
                $Hesabdar = '<img class="signature_class" width="120px;" height="120px;" src="'.ADDR.'Signature/'.$rsig[$i]['signature'].'">';
                $fHesabdar = $rsig[$i]['postJob'].' '.$ut->greg_to_jal($rsig[$i]['createDate']).' '.$rsig[$i]['fname'].' '.$rsig[$i]['lname'];
            }
            if ($rsig[$i]['sender'] == 151 || $rsig[$i]['sender'] == 39){  // رئیس حسابداری
                $RHesabdar = '<img class="signature_class" width="120px;" height="120px;" src="'.ADDR.'Signature/'.$rsig[$i]['signature'].'">';
                $fRHesabdar = $rsig[$i]['postJob'].' '.$ut->greg_to_jal($rsig[$i]['createDate']).' '.$rsig[$i]['fname'].' '.$rsig[$i]['lname'];
            }
            if ($rsig[$i]['sender'] == 27 || in_array($rsig[$i]['sender'],$this->finance_manager_user_arr)){  // مدیر مالی
                $MHesabdar = '<img class="signature_class" width="120px;" height="120px;" src="'.ADDR.'Signature/'.$rsig[$i]['signature'].'">';
                $fMHesabdar = $rsig[$i]['postJob'].' '.$ut->greg_to_jal($rsig[$i]['createDate']).' '.$rsig[$i]['fname'].' '.$rsig[$i]['lname'];
            }
            if ($rsig[$i]['sender'] == 4){  // مدیر عامل
                $Modiriat = '<img class="signature_class" width="120px;" height="120px;" src="'.ADDR.'Signature/'.$rsig[$i]['signature'].'">';
        ...
        $htm.=$watermark;
            ...
            $htm .= '<table style="width: 100%;border-right: 2px solid #000;border-left: 2px solid #000;border-bottom: 2px solid #000;">';
                $htm .= '<thead>';
                    ...
                    $htm .= '<td style="padding: 3px 0;width: 50%;background-color: #fff;font-size: 30px;font-family: BNazanin;font-weight: bold;text-align: right;" class="pr-3">'.$creditor_info.'</td>';
                    $htm .= '</tr>';
                $htm .= '</thead>';
            $htm .= '</table>';
...
//             $htm .= '</table>';
           // ************************ امضا کارشناس و سرپرست و مدیر واحد *************************
            ...
            $htm .= '<footer style="width: 100%;border: 2px solid #000;display:flex justify-content:center">';
            $htm .= '<span style="padding: 2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;position:relative">'.$beginner.$fbeginner.'</span>';
            $htm .= '<span style="padding: 2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;position:relative">'.$Sarparast.$fSarparast.'</span>';
            $htm .= '<span style="padding: 2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;position:relative">'.$Moavenat.$fMoavenat.'</span>';
   
            //************************ امضا حسابداری *************************
            $htm .= '<span style="padding:2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;position:relative">'.$Hesabdar.$fHesabdar.'</span>';
            $htm .= '<span style="padding:2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;position:relative">'.$RHesabdar.$fRHesabdar.'</span>';
            $htm .= '<span style="padding:2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;position:relative">'.$MHesabdar.$fMHesabdar.'</span>';
            
            if (strtotime($res[0]['clearingFundDate']) > 0) {
            
                $htm .= '<p colspan="3" class="pr-3" style="padding: 10px 0 20px 0;width: 100%;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: right;">قابل توجه واحد مالی : تاریخ تسویه تنخواه '.$ut->greg_to_jal($res[0]['clearingFundDate']).' می باشد</p>';
            
            }
       
            $htm .= '<p class="pl-3" style="padding: 50px 0 20px 0;width: 100%;background-color: #fff;font-size: 30px;font-family: BNazanin;font-weight: bold;text-align: center;position:relative">'.$Modiriat.$fModiriat.'</p>';
            $htm .= '</footer>';
    ...
    public function createTransferPayComment($radioValue,$desc,$tick,$pid){
       ...
       // $receiverTime=date('H:i:s', strtotime('-1 hour'));
		...
		if($rsq[0]['layer2']==59)//
		{
			$promised_manager_array = [     $this->human_resource_manager_arr[0],
                                            $this->finance_manager_user_arr[0],
                                            $this->chief_accounting_arr[0]
                                     ];
        ...
        elseif($meta_comment_is_drivers_fare_count>0)
        {
            ...
            {//اگر بارنامه وارده بود----------------------------------
               ...
               // $permission_users_array=$ut->get_full_access_users(15);//کاربران مجاز به تایید بارنامه وارده
                $permission_users_array=[14,$this->finance_manager_user_arr[0]];
            ...
            {//اگر بارنامه صادره بود----------------------------------
                ...
                if($rsq[0]['sendType']==2){// اظهارنظر سهامی  باشد
                 //  $permission_users_array=$ut->get_full_access_users(17);//کاربران مجاز به تایید بارنامه صادره سهامی
                    $permission_users_array=[14,$this->finance_manager_user_arr[0],149];//کاربران مجاز به تایید بارنامه صادره سهامی
            ...
            {
                for($i=0;$i<count($permission_users_array);$i++){
                    $sqq2 = "SELECT `RowID` FROM `payment_workflow` WHERE `sender`={$permission_users_array[$i]} AND `status`=1 AND `pid`={$pid} group by RowID  order by RowID DESC limit 1";  
		...
		else
		{
            ...
            switch (intval($rsq[0]['Transactions'])){
				...
				case 2 :
					...
					$sqq2 = "SELECT `RowID` FROM `payment_workflow` WHERE `sender`={$this->finance_manager_user_arr[0]} AND `status`=1 AND `pid`={$pid}";  // امضا مدیر مالی
					$rsq2 = $db->ArrayQuery($sqq2);
					if (count($rsq2) <= 0){
						$res = "امضا مدیر مالی الزامی می باشد !!!";
						$out = "false";
				...
				case 4 :
					...
					$sqq3 = "SELECT `RowID` FROM `payment_workflow` WHERE `sender`={$this->finance_manager_user_arr[0]} AND `status`=1 AND `pid`={$pid}";  // امضا مدیر مالی
					$rsq3 = $db->ArrayQuery($sqq3);
					if (count($rsq3) <= 0){
						$res = "امضا مدیر مالی الزامی می باشد !!!";
						$out = "false";
        ...
        if($transfer_flag==1)
        {
            ...
            $query = "SELECT `sendType`,`creditor_id`,`accountID` FROM `pay_comment` WHERE `RowID`={$pid}";
			$rst = $db->ArrayQuery($query);
			...
			if (intval($rst[0]['sendType']) != 3) { // ثبت در حساب بستانکاری نبود
				...
				if (intval($res)) {
					...
					$sqq = "INSERT INTO `payment_workflow` (`sender`,`receiver`,`pid`,`status`,`createDate`,`createTime`,`description`) VALUES ({$_SESSION['userid']},{$receiver},{$pid},1,'{$receiverDate}','{$receiverTime}','{$desc}')";
					$db->Query($sqq);
                    if(!empty($rst[0]['creditor_id']) && !empty($rst[0]['accountID']) &&$rst[0]['accountID']!=$rst[0]['creditor_id']) {
                        $result=$this->create_sabt_dar_hesab_pay_comment($pid);
                    }
                    
					if ($acm->hasAccess('financialConfirmation')) {
						$res = 'bale';
                        $sql2 = "UPDATE `pay_comment` SET `transfer`=0,`receiverDate`='{$receiverDate}',`receiverTime`='{$receiverTime}',`senderUid`={$_SESSION['userid']},`descKesho`='{$desc}',`payType`={$radioValue},`lastReceiver`={$receiver},`tick`={$tick} WHERE `RowID`={$pid}";
				        $db->Query($sql2);
						return $res;
					} else {
						$res = 'na';
    ...
    public function create_sabt_dar_hesab_pay_comment($pid){
        $db     = new DBi();
        $ut     = new Utility();
        $sql    = "SELECT * FROM `pay_comment` where `RowID`= {$pid} AND `isEnable`=1";
        $res1   = $db->ArrayQuery($sql);
        $current_rowID  = $res1[0]['RowID'];
        $creditor_id    = $res1[0]['creditor_id'];
        $account_id_1   = $res1[0]['accountID'];
        $codeTafzili_1  = $res1[0]['codeTafzili'];
        $acc_name_1     = $res1[0]['accName'];
        $Amount_1     = $res1[0]['Amount'];
        $sql_acc="SELECT code from account where RowID={$creditor_id}";
       
        $res_acc=$db->ArrayQuery($sql_acc);
        $codeTafzili=$res_acc[0]['code'];
        $sq1_d = "DELETE FROM `pay_comment` where RowID={$res1[0]['child_sabt_dar_hesab_id']} and isEnable=20";
        $ut->fileRecorder($sql_d);
        $res=$db->Query($sq1_d);
        $last_id        = $this->duplicate_payComment_row2($pid,'pay_comment','pay_comment',$codeTafzili);
        $get_account_detailes = "SELECT * FROM `account` where `RowID`={$creditor_id} ";
        
        $res_a  = $db->ArrayQuery($get_account_detailes);
      
      
        $cFor = $res_a[0]['RowID'].","."0";
        $accNumber = 0;
        $accName = $res_a[0]['Name'];
        $accBank = "ندارد";
        $codeTafzili = $res_a[0]['code'];
        $nationalCode = $res_a[0]['codeMelli'];
       
        $sql_u  = "UPDATE `pay_comment` set 
                                        `accountID`='{$creditor_id}',`cFor`='{$cFor}',`accNumber`='{$accNumber}',`accName`='{$accName}',
                                        `accBank`='{$accBank}',`codeTafzili`='{$codeTafzili}',`nationalCode`='{$nationalCode}',
                                        `type`='واریز از طریق سایرین',`sendType`='4',`nationalCode`='{$nationalCode}' 
                                        where RowID={$last_id}";
        $res    = $db->Query($sql_u);
        $sql_u2 = "UPDATE `pay_comment` set `child_sabt_dar_hesab_id`={$last_id} where RowID={$pid}";
        $res2   = $db->Query($sql_u2);
        $curr_date=date('Y-m-d');
        $curr_time=date('H:i:s');
        $sq1_d2 = "DELETE FROM `deposit` where pid={$last_id}";
        $ut->fileRecorder($sq1_d2);
        $res_d2=$db->Query($sql_d2);
       
        $dDesc="پرداخت شده توسط {$acc_name_1} (ثبت خودکار جهت اظهارنظر های ثبت در حساب)";
        $depositor_sql = "INSERT INTO `deposit` (`pid`,`dDate`,`depositor`,`dBank`,`dDesc`,`codeTafzili`,`uid`,`status`,`maliRecord`,`maliRecordDate`,`maliRecordTime`,`accID`,`dAmount_payed_by_others`)
                        VALUES({$last_id},'{$curr_date}','{$acc_name_1}','','{$dDesc}','{$codeTafzili_1}',{$_SESSION['userid']},1,1,'{$curr_date}','{$curr_time}',{$account_id_1},'{$Amount_1}')";
        $res_d=$db->Query($depositor_sql);
    ...
    public function getAttachFundToCommentList($cid){
        $db = new DBi();
        $ut = new Utility();
        $this->set_comment_view_date($cid,'مشاهده لیست تنخواه & tankhah');
    ...
    public function  getReceivedCustomerList($rsDate,$reDate,$RType,$RMethod,$CName,$RAmount,$lastReceiver,$chkNum,$page=1){
        $acm = new acm();
        ...
        if(strlen(trim($chkNum)) > 0){
            $w[] = '`checkSerial`='.$chkNum.' ';
        }

        if(intval($lastReceiver) > 0){
            $w[] = '`lastReceiver`='.$lastReceiver.' ';
        }

        if(!$acm->hasAccess('AccessAllReceivedCustomers')){
            $w[] = '`lastReceiver`='.$_SESSION['userid'].' ';
        }
        ...
        $ut->fileRecorder('SQL......'.$lastReceiver);
        for($y=0;$y<$listCount;$y++){
            $finalRes[$y]['RowID'] = $res[$y]['RowID'];
            $finalRes[$y]['receiveType'] = $this->get_received_type(intval($res[$y]['receiveType']));//($res[$y]['receiveType'] == 0 ? 'بابت فروش' : 'بابت چک برگشتی');
            ...
            //         break;
            // }
            $finalRes[$y]['receiveMethod']=$this->get_received_method($res[$y]['receiveMethod']);
            ...
            $finalRes[$y]['depositDate'] = (strtotime($res[$y]['depositDate']) ? $ut->greg_to_jal($res[$y]['depositDate']) : '');
            ...
            if($res[$y]['status']==2){
                $finalRes[$y]['trColor'] = '#ffa5004a';
            }
            if($res[$y]['status']==1){
                $finalRes[$y]['trColor'] = '#d6d8db';
            }
    ...
    public function get_received_method($received_id){
        $title="";
        switch ($received_id){
            case 1:
                $title = 'واریزی به بانک ها';
                break;
            case 2:
                $title = 'pos';
                break;
    ...
    public function get_received_type($receiveType){
        $title= ($receiveType== 0 ? 'بابت فروش' : 'بابت چک برگشتی');
        return $title;
    }
    ...
    public function getExcelReceivedCustomer($rsDate,$reDate,$RType,$RMethod,$CName,$RAmount){
        ...
        $cnt = count($res);
        for ($i=0;$i<$cnt;$i++){
            $res[$i]['receiveType'] = $this->get_received_type(intval($res[$i]['receiveType']));//(intval($res[$i]['receiveType']) == 0 ? 'بابت فروش' : 'بابت چک برگشتی');
            $res[$i]['receiveDate'] = (strtotime($res[$i]['receiveDate']) > 0 ? $this->set_shamsi_date($ut->greg_to_jal($res[$i]['receiveDate'])) : '');
            $res[$i]['checkDate'] = (strtotime($res[$i]['checkDate']) > 0 ? $this->set_shamsi_date($ut->greg_to_jal($res[$i]['checkDate'])) : '');
            $res[$i]['receiveMethod']=$this->get_received_method($res[$i]['receiveMethod']);
    ...
    public function set_shamsi_date($shamsi_date){
        $ut=new Utility();
        $seprator="";
        if(strpos($shamsi_date,'/')){
            $seprator="/";
        }
        if(strpos($shamsi_date,'-')){
            $seprator="-";
        }
        $shamsi_date_array=explode($seprator,$shamsi_date);
        $date= $shamsi_date_array[0]."/".sprintf("%02d",$shamsi_date_array[1])."/".sprintf("%02d",$shamsi_date_array[2]);
        
        return $date;
    }

    public function  getReceivedCustomerListCountRows($rsDate,$reDate,$RType,$RMethod,$CName,$RAmount,$lastReceiver,$chkNum){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
    ...
    public function getTotalAmountReceivedCustomer($rsDate,$reDate,$RType,$RMethod,$CName,$RAmount,$lastReceiver,$chkNum){
        $acm = new acm();
        ...
        if(strlen(trim($chkNum)) > 0){
            $w[] = '`checkSerial`='.$chkNum.' ';
            $w1[] = '`checkSerial`='.$chkNum.' ';
            $z[] = '`checkSerial`='.$chkNum.' ';
        }
        ...
        if(intval($lastReceiver) > 0){
            $w[] = '`lastReceiver`='.$lastReceiver.' ';
            $w1[] = '`lastReceiver`='.$lastReceiver.' ';
            $z[] = '`lastReceiver`='.$lastReceiver.' ';
        }
    ...
    public function get_company_banks($bank_id){
        $db=new DBi();
        $sql="SELECT `Name` from `company_banks` where `RowID`={$bank_id}";
        $res=$db->ArrayQuery($sql);
        return $res[0]['Name'];
    }

    public function get_company_pos($pos_id){
        $db=new DBi();
        $ut=new Utility();
        $sql="SELECT `Name` from `company_pos` where `RowID`={$pos_id}";
       
        $res=$db->ArrayQuery($sql);
        return $res[0]['Name'];
    }

    public function get_customer_received_status($status){
       $title="";
        switch($status){
            case "1":
                $title="در حال بررسی ";
                break;
            case "2":
                $title="دریافت توسط واحد مالی";
                break;
            case "3":
                $title="تایید نهایی مالی";
                break;
            case "4":
                $title="تایید  موقت";
                break;
    ...
    public function getArchivedCustomerRecieved($arr){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        extract($arr);
        $start=($page-1)*20;
        $ut->fileRecorder($page);
        $end=$strat+20;
        $js_func="getArchivedCustomerRecieved";
        $section="archived";
        $w=["`status`=3"];
        if(!empty($from_date) && !empty($to_date)){
            $g_from_date=$ut->jal_to_greg($from_date);
            $g_to_date=$ut->jal_to_greg($to_date);
            $w[]="`receiveDate` Between '{$g_from_date}' AND '{$g_to_date}' ";
        }
        if(!empty($receiveType) && $received_type >-1) {
            
            $w[]="`receiveType` = '{$received_type}' ";
        }
        if(!empty($recived_method) && $recived_method >-1) {
            
            $w[]="`receiveMethod` = '{$recived_method}' ";
        }
        if(!empty($customer_name)) {
            
            $w[]="`depositor` LIKE  '%{$customer_name}%' ";
        }
        if(intval($amount) >0) {
            
            $w[]="`receiveAmount` = '{$amount}' ";
        }
        if(!empty($check_serial)) {
            
            $w[]="`checkSerial` = '{$check_serial}' ";
        }
        ...
        $total_count_sql="SELECT Count(RowID) as `count` FROM customer_receive $where ";
         $ut->fileRecorder($total_count_sql);
        $total_res=$db->ArrayQuery($total_count_sql);
       
        $total_records=$total_res[0]['count'];
       
       
        $total_rec="SELECT * FROM customer_receive $where ORDER BY `RowID` DESC  LIMIT {$start},{$end}";
        $res=$db->ArrayQuery($total_rec);
        $r_type_array=[-1=>'نوع دریافتی',0=>'بابت فروش',1=>'بابت چک برگشتی'];
        $r_method_array=[-1=>'روش دریافتی',1=>'واریزی به بانکها',2=>' pos',3=>'چک',4=>'نقدی'];
        $total_res=count($res);
        $counter=$start+1;
        $html='<form class="form-inline" style="margin: 10px 0 0 20px;">
                <div id="receivedCustomerSDateSearchSended-div">
                    <label class="sr-only" for="receivedCustomerSDateSearchSended">از تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerSDateSearchArchived" autocomplete="off" style="width:100px;" placeholder="از تاریخ" data-mdpersiandatetimepicker="" value="'.@$from_date.'" data-original-title="" title="">
                </div>
                <div id="receivedCustomerEDateSearchSENDED-div">
                    <label class="sr-only" for="receivedCustomerEDateSearch">تا تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerEDateSearchArchived" autocomplete="off" style="width:100px;" placeholder="تا تاریخ" data-mdpersiandatetimepicker="" value="'.@$to_date.'" data-original-title="" title="">
                </div>
                <div id="receivedCustomerRTypeSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRTypeSearchSended">نوع دریافتی </label>
                    <select class="form-control form-control-sm mb-2 mr-sm-2 headsearch" style="width:120px" id="receivedCustomerRTypeSearchArchived">';
                        foreach($r_type_array as $key=>$value){
                            if($key==intval($received_type)){
                                $selected="selected";
                            }else{
                                $selected="";
                            }
                ...
                <div id="receivedCustomerRMethodSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRMethodSearchSended">روش دریافتی </label>
                    <select class="form-control form-control-sm mb-2 mr-sm-2 headsearch" style="width:120px" id="receivedCustomerRMethodSearchArchived">';
                    foreach($r_method_array as $key=>$value){
                        if($key==intval($recived_method)){
                            $selected="selected";
                        }else{
                            $selected="";
                        }
                ...
                <div id="receivedCustomerCNameSearchSended-div">
                    <label class="sr-only" for="receivedCustomerCNameSearch">نام مشتری</label>
                    <input type="text" value="'.@$customer_name.'" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerCNameSearchArchived" autocomplete="off" style="width:150px;" placeholder="نام مشتری">
                </div>
                <div id="receivedCustomerRAmountSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRAmountSearchSended">مبلغ</label>
                    <input type="text" value="'.@$amount.'" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerRAmountSearchArchived" autocomplete="off" style="width:150px;" placeholder="مبلغ">
                </div>
                <div id="receivedCustomerRAmountSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRAmountSearchSended">شماره چک</label>
                    <input value="'.@$check_serial.'" type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerCheckSerialSearchArchived" autocomplete="off" style="width:150px;" placeholder="شماره چک">
                </div>
                <button type="button" class="btn btn-sm btn-secondary mb-2 mr-2" style="padding: 0.25rem 0.5rem;" onclick="getArchivedCustomerRecieved();">جستجو&nbsp;&nbsp;<i class="fa fa-search"></i></button></form>';
        ...
        $html.="<thead><tr>
                            ...
                            <th> نوع دریافتی</th>
                            <th>	روش دریافتی</th>
                            <th>	تاریخ واریز	طرف مقابل</th>
                            <th>نام طرف مقابل</th>
                            <th>	تاریخ دریافتی</th>
                            <th>	کد تفضیلی</th>
                            <th>	نام بانک</th>
                            <th>نام pos</th>
                            <th>	(دریافتی/چک)مبلغ</th>
                            <th>آخرین وضعیت</th>
                            <th>	توضیحات	</th>
                            <th>	اطلاعات	</th>
                            <th>	ضمیمه ها</th>
       ...
       foreach($res as  $row)
       {
            $depositDate    =   $ut->greg_to_jal($row['depositDate']);
            $receiveDate    =   $ut->greg_to_jal($row['receiveDate']);
            $receiveMethod  =   $this->get_received_method($row['receiveMethod']);
            $receiveType    =   $this->get_received_type(intval($res['receiveType']));
            $bankName       =   $this->get_company_banks($row['bankID']);
            $pos_name       =   $this->get_company_pos($row['posID']);
            $r_status       =   $this->get_customer_received_status($row['status']);
            $receiveAmount  =   (intval($row['receiveAmount'])>0?number_format($row['receiveAmount'])." ریال":'');
            $tr_class       =   "";
            if($row['status']==1){
                $tr_class="bg_danger_custom";
            }
            if($row['status']==2){
                $tr_class="bg_warning_custom";
            }
            if($row['status']==3){
                $tr_class="bg_success_custom";
            }
                $html.="<tr class='$tr_class'>
                        ...
                        <td>{$row['depositor']}</td>
                        <td>{$receiveDate}</td>
                        <td>{$row['codeTafzili']}</td>
                        <td>{$bankName}</td>
                        <td>{$pos_name}</td>
                        <td>{$receiveAmount} </td>
                        <td>{$r_status} </td>
                        <td>{$row['description']}</td>
                        <td><i onclick='ShowFinanceManage({$row['RowID']},1)' class='btn btn-info text-light fa fa-tv'></i></td>
                        <td><i onclick='ShowAttachFileReceivedCustomer({$row['RowID']})' class='btn btn-info text-light fa fa-link'></i></td>
                        <td><i onclick='getSendHistory({$row['RowID']})' class='btn btn-info text-light fa fa-history'></i></td>
                        </tr>";
                $counter++;
        }

        $html.="</tbody></table>";
        $pageinate=$ut->generatePagination($total_records, 20, $page, $js_func,$section, $max_pages = 5);
        $html.=$pageinate;
       
        return $html;
    }

    public function getTempCustomerRecieved($arr){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        $end=$strat+20;
        $js_func="getTempConfirmedCustomerRecieved";
        $section="temp_conf";
        $w=["`status`=4"];
        if(!empty($from_date) && !empty($to_date)){
            $g_from_date=$ut->jal_to_greg($from_date);
        ...
        $total_count_sql="SELECT Count(RowID) as `count` FROM customer_receive $where ";
         $ut->fileRecorder('tttteeemmmpp:'.$total_count_sql);
        $total_res=$db->ArrayQuery($total_count_sql);
       
        $total_records=$total_res[0]['count'];
       
       
        $total_rec="SELECT c.* FROM customer_receive as c $where ORDER BY `RowID` DESC  LIMIT {$start},{$end}";
        $res=$db->ArrayQuery($total_rec);
        $ut->fileRecorder('tttteeemmmpp:'.$total_rec);
        $total_res=count($res);
        $counter=$start+1;
        $html='<form class="form-inline" style="margin: 10px 0 0 20px;">
                <div id="receivedCustomerSDateSearchTemp-div">
                    <label class="sr-only" for="receivedCustomerSDateSearchTemp">از تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerSDateSearchTemp" autocomplete="off" style="width:100px;" placeholder="از تاریخ" data-mdpersiandatetimepicker="" data-original-title="" title="">
                </div>
                <div id="receivedCustomerEDateSearchSENDED-div">
                    <label class="sr-only" for="receivedCustomerEDateSearch">تا تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerEDateSearchTemp" autocomplete="off" style="width:100px;" placeholder="تا تاریخ" data-mdpersiandatetimepicker="" data-original-title="" title="">
                </div>
                <div id="receivedCustomerRTypeSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRTypeSearchTemp">نوع دریافتی </label>
                    <select class="form-control form-control-sm mb-2 mr-sm-2 headsearch" style="width:120px" id="receivedCustomerRTypeSearchTemp">
                        <option value="-1">نوع دریافتی</option>
                        <option value="0">بابت فروش</option>
                        <option value="1">بابت چک برگشتی</option>
                    </select>
                </div>
                <div id="receivedCustomerRMethodSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRMethodSearchSended">روش دریافتی </label>
                    <select class="form-control form-control-sm mb-2 mr-sm-2 headsearch" style="width:120px" id="receivedCustomerRMethodSearchTemp">
                        <option value="-1">روش دریافتی</option><option value="1">واریزی به بانک ها</option>
                        <option value="2">pos</option><option value="3">چک</option>
                        <option value="4">نقدی</option>
                    </select>
                </div>
                <div id="receivedCustomerCNameSearchSended-div">
                    <label class="sr-only" for="receivedCustomerCNameSearch">نام مشتری</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerCNameSearchTemp" autocomplete="off" style="width:150px;" placeholder="نام مشتری">
                </div>
                <div id="receivedCustomerCNameSearchSended-div">
                    <label class="sr-only" for="receivedCustomerCNameSearch">شماره چک </label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerCheckSerialSearchTemp" autocomplete="off" style="width:150px;" placeholder="شماره چک ">
                </div>
                <div id="receivedCustomerRAmountSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRAmountSearchSended">مبلغ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerRAmountSearchTemp" autocomplete="off" style="width:150px;" placeholder="مبلغ">
                </div>
                <button type="button" class="btn btn-sm btn-secondary mb-2 mr-2" style="padding: 0.25rem 0.5rem;" onclick="getTempConfirmedCustomerRecieved();">جستجو&nbsp;&nbsp;<i class="fa fa-search"></i></button></form>';
        $html.="<table class='table table-bordered'>";
        ...
        $html.="<thead><tr>
                            ...
                            <th>آخرین وضعیت</th>
                            <th>	توضیحات	</th>";
                            if($acm->hasAccess('receivedCustomerChangeEdit')){
                            $html.="<th>	ویرایش	</th>";
                            }
                            $html.="
                            <th>	اطلاعات	</th>";
                            if($acm->hasAccess('receivedCustomerManageFinalConfirm')){
                                $html.="<th>	مدیریت مالی	</th>";
                            }
                                $html.="
                            <th>	ضمیمه ها</th>
                             <th>	 گردش کار</th>
       ...
       {
            $tr_class="bg_danger_custom";
            $changed=$this->get_changed_RC($row['RowID']);
            if($changed){
                $tr_class="bg_warning_custom";
            }
            $depositDate    =   $ut->greg_to_jal($row['depositDate']);
            ...
            // }
                ...
                if($acm->hasAccess('receivedCustomerChangeEdit')){
                    $html.="
                        <td><i onclick='editReceivedCustomer({$row['RowID']},1)' class='btn btn-info text-light fa fa-edit'></i></td>";
                }
                $html.="
                        <td><i onclick='ShowFinanceManage({$row['RowID']},1)' class='btn btn-info text-light fa fa-tv'></i></td>";
            if($acm->hasAccess('receivedCustomerManageFinalConfirm')){
                if($changed){
                    $html.="<td><i onclick='ShowTempFinanceManage({$row['RowID']})' class='btn btn-info text-light fa fa-check'></i></td>";
                }else{
                    $html.="<td>در حال بررسی..</td>";
                }
            }
                else{

                }
                 $html.=" <td><i onclick='ShowAttachFileReceivedCustomer({$row['RowID']})' class='btn btn-info text-light fa fa-link'></i></td>
                        <td><i onclick='getSendHistory({$row['RowID']})' class='btn btn-info text-light fa fa-history'></i></td>
                        </tr>";
                $counter++;
        }
    ...
    public function get_CR_row_temp_confirm($arr){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        $db = new DBi();
        extract ($arr);
        $sql="SELECT * FROM `customer_receive` WHERE `status`=4 AND `RowID`={$RowID}";
        $res=$db->ArrayQuery($sql);
        $row=$res[0];
        $html="<table class='table table-bordered'>";
        $html.="<tr >
                                <td style=\"font-family:IranSans\" class=\"bg-secondary text-light text-center\" colspan='2'><h5>روش  دریافتی  : {$this->get_received_method($row['receiveMethod'])}<h5></td>
                            </tr>";
                    $html.="<tr>
                                <td>شماره چک</td><td>{$row['checkSerial']}</td>
                            </tr>
                            <tr>
                                <td> بانک صادر کننده چک</td><td>{$row['check_issuing_bank']}</td>
                            </tr>
                            <tr>
                                <td>مبلغ چک</td>
                                <td>{$receiveAmount}</td>
                            ...
                            <tr>
                                <td> نام صادر کننده چک</td>
                                <td>{$row['checkOwner']}</td>
                            </tr>
                            <tr>
                            
                            <td>کد ملی صادر کننده چک</td>
                            <td>{$row['checkOwnerCode']}</td>
                            </tr>
                            <tr>
                            <td>چک صیادی شده است ؟</td>
                            <td>";
                            if($row['is_sayyad']==1){
                                $html.='<span class="text-success fa fa-check"></span>';
                            }else{
                                $html.='<span class=" text-danger fa fa-close"></span>';
                            }
                            $html.="</td>
                            <tr>
                                <td>تاریخ سررسید چک</td>
                                <td>{$ut->greg_to_jal($row['checkDate'])}</td>
                            </tr>
                            <tr>
                                <td>نام ارایه کننده چک</td>
                                <td>{$row['depositor']}</td>
                            </tr>
                            <tr>
                            <td> تعداد چک</td>
                            ...
                            <tr>
                                ...
                                if($row['is_deliverd']==1){
                                $html.="<tr><td colspan='2'><label><span class='mx-2'></span><span>اینجانب<span class='text-success padding:5px'>{$ut->get_user_fullname($row['user_deliver_check'])}</span> فیزیک چک به مشخصات فوق را ، از واحد وصول مطالبات تحویل گرفتم </span></label></td></tr>";
                                }
                                $html.="<tr><td colspan='2'><label><input type=\"checkbox\" id=\"lastConfirmAgain\"> مشکلات برگ چک به مشخصات فوق مرتفع  و تایید نهایی گردید </label></td></tr>";
                 $html.="<table>";               
    ...
    public function get_changed_RC($rc_id){
        $db=new DBi();
        $sql="SELECT `RowID` FROM `customer_receive_meta` WHERE `CRID`='{$rc_id}' ";
        $res=$db->ArrayQuery($sql);
        return count($res);
    }

    public function getSendedCustomerRecieved($arr){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        $start=($page-1)*20;
        $end=$strat+20;
        $js_func="getSendedCustomerRecieved";
        $section="sended";
        $w=["w.`sender`='{$_SESSION['userid']}'"];
        if(!empty($from_date) && !empty($to_date)){
            $g_from_date=$ut->jal_to_greg($from_date);
            $g_to_date=$ut->jal_to_greg($to_date);
            $w[]="c.`receiveDate` Between '{$g_from_date}' AND '{$g_to_date}' ";
        }
        if(!empty($receiveType) && $received_type >-1) {
            
            $w[]="c.`receiveType` = '{$received_type}' ";
        }
        if(!empty($recived_method) && $recived_method >-1) {
            
            $w[]="c.`receiveMethod` = '{$recived_method}' ";
        }
        if(!empty($customer_name)) {
            
            $w[]="c.`depositor` = '{$customer_name}' ";
        }
        if(intval($amount) >0) {
            
            $w[]="c.`receiveAmount` = '{$amount}' ";
        }
        $w[]="c.`status` IN(1,2) ";
        ...
        $total_count_sql="SELECT Count(c.RowID) as `count` FROM customer_receive as c INNER JOIN `customer_receive_workflow` as w on (c.`RowID`=w.`CRid` and c.`uid`=w.`sender`)  $where ";
        
        $count_res=$db->ArrayQuery($total_count_sql);
       
        $total_records=$total_res[0]['count'];
       
       
        $total_rec="SELECT c.* FROM `customer_receive` as c INNER JOIN `customer_receive_workflow` as w on(c.`RowID`=w.`CRid` and c.`uid`=w.`sender`)   $where ORDER BY c.`RowID` DESC  LIMIT {$start},{$end}";
        $res=$db->ArrayQuery($total_rec);
        
      
        $total_res=$count_res[0]['count'];
        $ut->fileRecorder('total_res::...........................'.$total_res);
        $ut->fileRecorder('total_res::...........................'.$total_count_sql);
        $counter=$start+1;
        if(intval($total_res)==0){
            return "<p class='text-danger'> هیچ مورد ارسالی وجود ندارد </p>";
        }
        $html='<form class="form-inline" style="margin: 10px 0 0 20px;">
                <div id="receivedCustomerSDateSearchSended-div">
                    <label class="sr-only" for="receivedCustomerSDateSearchSended">از تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerSDateSearchSended" autocomplete="off" style="width:100px;" placeholder="از تاریخ" data-mdpersiandatetimepicker="" data-original-title="" title="">
                </div>
                <div id="receivedCustomerEDateSearchSENDED-div">
                    <label class="sr-only" for="receivedCustomerEDateSearch">تا تاریخ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerEDateSearchSended" autocomplete="off" style="width:100px;" placeholder="تا تاریخ" data-mdpersiandatetimepicker="" data-original-title="" title="">
                </div>
                <div id="receivedCustomerRTypeSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRTypeSearchSended">نوع دریافتی </label>
                    <select class="form-control form-control-sm mb-2 mr-sm-2 headsearch" style="width:120px" id="receivedCustomerRTypeSearchSended">
                        <option value="-1">نوع دریافتی</option>
                        <option value="0">بابت فروش</option>
                        <option value="1">بابت چک برگشتی</option>
                    </select>
                ...
                <div id="receivedCustomerRMethodSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRMethodSearchSended">روش دریافتی </label>
                    <select class="form-control form-control-sm mb-2 mr-sm-2 headsearch" style="width:120px" id="receivedCustomerRMethodSearchSended">
                        <option value="-1">روش دریافتی</option><option value="1">واریزی به بانک ها</option>
                        <option value="2">pos</option><option value="3">چک</option>
                        <option value="4">نقدی</option>
                    </select>
                ...
                <div id="receivedCustomerCNameSearchSended-div">
                    <label class="sr-only" for="receivedCustomerCNameSearch">نام مشتری</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerCNameSearchSeneded" autocomplete="off" style="width:150px;" placeholder="نام مشتری">
                </div>
                <div id="receivedCustomerRAmountSearchSended-div">
                    <label class="sr-only" for="receivedCustomerRAmountSearchSended">مبلغ</label>
                    <input type="text" class="form-control form-control-sm mb-2 mr-sm-2 headsearch" id="receivedCustomerRAmountSearchSended" autocomplete="off" style="width:150px;" placeholder="مبلغ">
                </div>
                <button type="button" class="btn btn-sm btn-secondary mb-2 mr-2" style="padding: 0.25rem 0.5rem;" onclick="getSendedCustomerRecieved();">جستجو&nbsp;&nbsp;<i class="fa fa-search"></i></button></form>';
        $html.="<table class='table table-bordered'>";
        $html.="<thead><tr>
                            <th>    ردیف</th>
                            <th>    نوع دریافتی</th>
       ...
       foreach($res as  $row)
       {
        ...
        $html.="</tbody></table>";
        $pageinate=$ut->generatePagination($total_res, 20, $page, $js_func,$section, $max_pages = 5);
        $html.=$pageinate;
        return $html;
    }

    public function get_company_bank_name($id){
        $db=new DBi();
        $sql="SELECT `Name` from `company_banks` where `RowID`={$id}";
        $res=$db->ArrayQuery($sql);
        return $res[0]['Name'];
    }
   
    public function get_CR_row($arr){   
        $acm=new acm();
        $db=new DBi();
        $ut=new Utility();
        extract($arr);
        $sql="SELECT * FROM `customer_receive` where `status` IN (1,2,3,4) AND RowID={$RowID}";
       
        $res=$db->ArrayQuery($sql);
        $row=$res[0];
        $receiveAmount=(intval($row['receiveAmount'])>0?number_format($row['receiveAmount']). " ریال ":'');
        $temp_confirm_reason=!empty($row['temp_confirm_reason'])?" بدلیل  ".$row['temp_confirm_reason']:"";
        $tbl_class=($row['status']==1?'bg_danger_custom':($row['status']==2?'bg_warning_custom':($row['status']==3?'bg_success_custom':'bg_temp_custom')));
        $html="<table class='table table-bordered ".$tbl_class."'><tbody>";
            switch(intval($row['receiveMethod'])){
                case 1: //واریزی به بانکها`
                case 2: //pos
                case 4: // نقدی
                    $html.="<tr >
                                <td style=\"font-family:IranSans\" class=\"bg-secondary text-light text-center\" colspan='2'>روش   دریافتی  : {$this->get_received_method($row['receiveMethod'])}<h5></td>
                            </tr>";
                    $html.="<tr>
                                <td> تاریخ دریافت</td><td>{$ut->greg_to_jal($row['receiveDate'])}</td>
                            </tr>";
                            if(intval($row['receiveMethod'])==1){
                                $html.="<tr>
                                <td>تاریخ واریز</td><td>{$ut->greg_to_jal($row['depositDate'])}</td>
                                </tr>";
                                $html.="<tr>
                                    <td> بانک</td><td>{$this->get_company_bank_name($row['bankID'])}</td>
                                </tr>";
                            }
                            if(intval($row['receiveMethod'])==2){
                                
                                $html.="<tr>
                                    <td> نام دستگاه POS</td><td>{$this->get_company_pos($row['posID'])}</td>
                                </tr>";
                            }
                            $html.="
                            <tr>
                                <td>مبلغ (واریزی/نقدی /POS)</td>
                                <td>{$receiveAmount}</td>
                            </tr>
                            <tr>
                                <td> نام واریز کننده</td>
                                 <td>{$row['depositor']}</td>
                            </tr>
                            <tr>
                                <td>توضیحات</td>
                            ...
                            if($readOnly==1){
                                ...
                                if($row['status']==4){
                                    ...
                                    $html.="<tr><td colspan='2'><label><span class='mx-2'></span> دریافتی از مشتری  توسط <span class='text-success padding:5px'>{$ut->get_user_fullname($row['user_temp_confirm'],1)}</span> در تاریخ <span class='text-success padding:5px'>{$ut->greg_to_jal($row['temp_confirm_date'])}</span>  {$temp_confirm_reason}  تایید موقت گردید</span></label></td></tr>";
                                }
                                if($row['status']==3){
                                   
                                $html.="<tr><td colspan='2'><label><span class='mx-2'></span> دریافتی از مشتری  توسط <span class='text-success padding:5px'>{$ut->get_user_fullname($row['user_last_confirm'],1)}</span> در تاریخ <span class='text-success padding:5px'>{$ut->greg_to_jal($row['last_confirm_datetime'])}</span> تایید نهایی گردید</span></label></td></tr>";
                                }
                            }
                            else{
                            if($acm->hasAccess('receivedCustomerCheckManage')){
                                $deliver_date_arr=explode(" ",$row['deliver_date']);
                                $deliver_time=$deliver_date_arr[1];
                                $deliver_date=$ut->greg_to_jal($deliver_date_arr[0]);
                                $delivaer_datetime=$deliver_date." - ".$deliver_time;
                                if($row['is_deliverd']==1){
                                    $html.="<tr><td colspan='2'> <span>فیزیک چک به مشخصات فوق توسط  <span class='text-success padding:5px'>{$ut->get_user_fullname($row['user_deliver_check'],1)}</span > در تاریخ  <span class='text-success padding:5px'>{$delivaer_datetime}</span> در واحد مالی  دریافت شد</span></td></tr>";
                                }
                                // else{
                                //     $html.="<tr><td colspan='2'><label><span class='mx-2'><input id=\"ReceivePhysicalCheck\" type='checkbox' /></span><span>اینجانب<span class='text-success padding:5px'>{$ut->get_user_fullname($_SESSION['userid'])}</span> فیزیک چک به مشخصات فوق را ، از واحد وصول مطالبات تحویل گرفتم </span></label></td></tr>";
                                // }
                               
                            }
                        
                            if($acm->hasAccess('receivedCustomerManageFinalConfirm')){
                                $html.="<tr><td colspan='2'><label><span class='mx-2'><input id=\"CR_FinalConfirm\" type='checkbox' /></span> دریافتی از مشتری  در دفاتر مالی ثبت شده و تایید نهایی گردید</span></label></td></tr>";
                            }
                        }
                    break;
                case 3: //چک
                    ...
                    $html.="<tr>
                            ...
                            if($readOnly==1){
                                ...
                                if($row['status']==4){
                                    ...
                                    $html.="<tr><td colspan='2'><label><span class='mx-2'></span> دریافتی از مشتری( چک )  توسط <span class='text-success padding:5px'>{$ut->get_user_fullname($row['user_temp_confirm'],1)}</span> در تاریخ <span class='text-success padding:5px'>{$ut->greg_to_jal($row['temp_confirm_date'])}</span> {$temp_confirm_reason}  تایید موقت گردید</span></label></td></tr>";
                                    }
                                }
                            ...
                            if($acm->hasAccess('receivedCustomerManageFinalConfirm')){
                                ...
                                $html.="<tr><td >
                                <label class='p-2'><span>تایید موقت</span><input name=\"CRTempConfirm\" onclick=\"toggle_reason_box(this.value)\" type='radio' value='4'/></label>
                                <label class='p-2'><span>تایید نهایی</span><input name=\"CRTempConfirm\" onclick=\"toggle_reason_box(this.value)\" type='radio' value='3' /></label>
                                <td id=\"temp_confirm_reson_td\"></td></tr>";
                            }
                        }		
    ...
    public function doConfirmCR($arr){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        if(intval($ReceivePhysicalCheck)==1){
            $physic_check_deliver=1;
            $deliver_user=$_SESSION['userid'];
            $deliver_date=date("Y-m-d H:i:s");
            $sql_cond="`status`=2";
        }
        if(intval($CR_Confirm)==3){
            $sql_cond="`status`=3";
            $last_confirm_user=$_SESSION['userid'];
            $finalConfirm_date=date("Y-m-d H:i:s");
        }
        if(intval($CR_Confirm)==4){
            $sql_cond="`status`=4";
            $temp_confirm_user=$_SESSION['userid'];
            $temp_confirm_date=date("Y-m-d H:i:s");
          
        }


        $sql="UPDATE customer_receive SET 
            `is_deliverd`='{$physic_check_deliver}',
            `user_deliver_check`='{$deliver_user}',`deliver_date`='{$deliver_date}',
            `user_last_confirm`='{$last_confirm_user}',`last_confirm_datetime`='{$finalConfirm_date}',
            `user_temp_confirm`='{$temp_confirm_user}',`temp_confirm_date`='{$temp_confirm_date}',`temp_confirm_reason`='{$temp_confirm_reason}', {$sql_cond} where RowID='{$RowID}'";
        $res=$db->Query($sql);
        return $res;
            
    }

    public function createReceivedCustomer($arr){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        $depositDate = ($depositDate?$ut->jal_to_greg($depositDate):'NULL');
        $checkNumber = (intval($RMethod) !== 3 ? 'NULL' : 1);
        $customer_sql="SELECT `Name`,`codeTafzili`  FROM `depositors` where `RowID`='{$CstName}'";
        $res_customer=$db->ArrayQuery($customer_sql);
       
        $Customer_name=$res_customer[0]['Name'];
        $Customer_tafzoili=$res_customer[0]['codeTafzili'];

        $sql = "INSERT INTO `customer_receive` (`receiveType`,`receiveMethod`,`checkNumber`,`receiveAmount`,`receiveDate`,`depositDate`,`uid`,`depositor`,`codeTafzili`,`description`,`bankID`,`posID`,`checkDate`,`checkSerial`,`checkOwner`,`checkOwnerCode`,`check_issuing_bank`,`is_sayyad`,`did`,`lastReceiver`)
                VALUES ({$RType},{$RMethod},{$checkNumber},{$amount},'{$rDate}','{$depositDate}',{$_SESSION['userid']},'{$Customer_name}',{$Customer_tafzoili},'{$desc}',{$bank},{$pos},'{$chDate}','{$serial}','{$chOwner}','{$chOwnerC}','{$chBankIssuingBank}','{$is_sayyad}','{$CstName}','{$_SESSION['userid']}')";
        $ut->fileRecorder($sql);
        $res = $db->Query($sql);
        $ut->fileRecorder($sql);
        if (intval($res) > 0){
    ...
    public function delete_CR_row($arr){
        extract($arr);
        $db=new DBi();
        $sql="UPDATE `customer_receive` SET `status`=0 WHERE `RowID` ={$row_id}";
        $res=$db->Query($sql);
        return $res;


    }
    public function editReceivedCustomer($arr){
        $acm = new acm();
        if(!$acm->hasAccess('receivedCustomerManage')){
            die("access denied");
            exit;
        ...
        $Customer_name=$res_customer[0]['Name'];
        $Customer_tafzoili=$res_customer[0]['codeTafzili'];
        $ut->fileRecorder($customer_sql);
        $depositDate = ($depositDate?$ut->jal_to_greg($depositDate):'NULL');
        
        $sql = "UPDATE `customer_receive` SET `receiveType`={$RType},`receiveMethod`={$RMethod},
                `receiveAmount`={$amount},`receiveDate`='{$rDate}',`depositDate`='{$depositDate}',`depositor`='{$Customer_name}',
                `codeTafzili`={$Customer_tafzoili},`description`='{$desc}',`bankID`={$bank},`posID`={$pos},`checkDate`='{$ut->jal_to_greg($chDate)}',`check_issuing_bank`='{$chBankIssuingBank}',`is_sayyad`='{$is_sayyad}',`did`='{$CstName}',`checkSerial`='{$serial}' WHERE `RowID`={$rid}";
        $db->Query($sql);
       $ut->fileRecorder('update:'.$sql);
        ...
        $res = (($res == -1 || $res == 0) ? 0 : 1);
        if (intval($res)) {
            if($res==1 && $RC_Changed_data==1){
                $change_date = date("Y-m-d H:i:s");
                $sql2="INSERT INTO `customer_receive_meta` (`CRID`,`key`,`value`,`desc`) values ('{$rid}','user_id','{$_SESSION['userid']}','کاربر ویرایش کننده'),('{$rid}','change_date','{$change_date}','تاریخ تغییر')";
                $res=$db->Query($sql2);
    ...
    public function receivedCustomerInfo($reid){
        ...
        $checkDate = (strtotime($res[0]['checkDate']) > 0 ? $ut->greg_to_jal($res[0]['checkDate']) : '');
        $depositDate = (strtotime($res[0]['depositDate']) > 0 ? $ut->greg_to_jal($res[0]['depositDate']) : '');
        if(count($res) == 1){
            $res = array("receiveType"=>$res[0]['receiveType'],"receiveMethod"=>$res[0]['receiveMethod'],
                         ...
                         "check_issuing_bank"=>$res[0]['check_issuing_bank'],"is_sayyad"=>$res[0]['is_sayyad'],
                         "checkOwnerCode"=>$res[0]['checkOwnerCode'],'did'=>$res[0]['did']);
            return $res;
        }else{
            return false;
        }
    ...
    public function attachFileToContract($cid,$info,$files){
        $db = new DBi();
        $SFile = array();
        $allowedTypes = ['png','jpg','jpeg','pdf','xlsx'];
        if (isset($files) && !empty($files)) {
            $no_files = count($files['name']);
            for ($i = 0; $i < $no_files; $i++) {
                $filepath = $files["tmp_name"][$i];
...
/*        $ut = new Utility();
    ...
    public function editAccount($aid,$Name,$code,$an,$codeMelli,$role_array){
        ...
        $sqq = "SELECT `RowID`,`code` FROM `account` WHERE `RowID`={$aid}";
        ...
        if ($resqq[0]['code'] != $code) {
            ...
            if (in_array($code, $codes)) {
                ...
                {
                    if($code!=8){
                        $update_payComment="UPDATE pay_comment SET `accName`='{$Name}', `codeTafzili`='{$code}' where `accountID`='{$resqq[0]['RowID']}'";
    ...
    public function getDepositorNames(){
        $db = new DBi();
        $sql = "SELECT `Name`,`RowID`,`codeTafzili` FROM `depositors`";
        $res = $db->ArrayQuery($sql);
        $cnt = count($res);
    ...
    public function commentDepositHTM($pid,$deleteShow,$payOrReport,$sendMali,$confMali){
        $ut = new Utility();
        $db = new DBi();
        $sql = "SELECT `deposit`.`RowID` AS `did`,`dDate`,`depositor`,`dBank`,`dAmount`,`dDesc`,`fname`,`lname`,`status`,`maliRecord`,`accName`,`dAmount_payed_by_others` FROM `deposit` 
        ...
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 6%;">تاریخ واریز</th>';
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 6%;">ثبت کننده</th>';
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 11%;">طرف مقابل</th>';
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 11%;">واریزکننده</th>';
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 11%;">نام بانک</th>';
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 11%;">مبلغ واریزی</th>';
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 10%;">مبلغ واریزی توسط سایرین</th>';
        $htm .= '<th style="text-align: center;color: #ffc107;font-family: dubai-Bold;width: 19%;">توضیحات</th>';
        ...
        for ($i=0;$i<$cnt;$i++){
            ...
            $htm .= '<td style="text-align: center;font-family: dubai-Regular;padding: 10px;">'.number_format($res[$i]['dAmount_payed_by_others']).' ریال</td>';
        ...
        $sql5="SELECT dAmount_payed_by_others from deposit where pid={$pid}";
        $res5=$db->ArrayQuery($sql5);
        $others_pay=0;
        foreach($res5 as $k=>$value){
            $others_pay+=$value['dAmount_payed_by_others'];
        }


        $query = "SELECT `CashSection`,`NonCashSection`,`Amount`,`sendType` FROM `pay_comment` WHERE `RowID`={$pid}";
        $result = $db->ArrayQuery($query);
        $remainingNaghdi = (intval($result[0]['CashSection']) > 0 ? $result[0]['CashSection'] - $amount-$others_pay : 0);
        $remainingCheck = (intval($result[0]['NonCashSection']) > 0 ? $result[0]['NonCashSection'] - $checks : 0);
        $remainingFinal = ($result[0]['Amount'] + $rAmount + $fAmount) - ($amount + $checks + $fxAmount)-$others_pay;
        $colorNaghdi = (intval($remainingNaghdi) < 0 ? 'color: #db0000;' : '');
        $colorCheck = (intval($remainingCheck) < 0 ? 'color: #db0000;' : '');
        $colorFinal = (intval($remainingFinal) < 0 ? 'color: #db0000;' : '');
        $htm .= '</tbody>';
        ...
        $htm .= '<td colspan="4" style="text-align: center;font-family: dubai-bold;padding: 10px;">مبلغ واریز شده توسط سایرین :'.number_format($others_pay).' ریال</td>';
        $htm .= '<td colspan="4" style="'.$colorNaghdi.'text-align: center;font-family: dubai-bold;padding: 10px;">مانده نقدی : <span dir="ltr">'.number_format($remainingNaghdi).'</span> ریال</td>';
        $htm .= '</tr>';
        $htm .= '<tr class="table-info">';
        $htm .= '<td colspan="4"style="text-align: center;font-family: dubai-bold;padding: 10px;">جمع پرداختی چک : '.number_format($checks).' ریال</td>';
        $htm .= '<td colspan="4" style="'.$colorCheck.'text-align: center;font-family: dubai-bold;padding: 10px;">مانده چک : <span  dir="ltr">'.number_format($remainingCheck).'</span> ریال</td><td></td>';
        $htm .= '</tr>';
        $htm .='<tr></tr>';
        for ($x=0;$x<$cnt2;$x++) {
            $sqlbank = "SELECT `Name` FROM `company_banks` WHERE `RowID`={$rst[$x]['bankID']}";
       ...
       // $$receiverTime=date('H:i:s',strtotime("-1 hour"));
        ...
        if ((intval($totalAmount) + $fAmount) >= ($res[0]['Amount'] + $rAmount)){   // پرداختی مازاد یا برابر با مبلغ کل اظهارنظر بود
           ...
           // $sql1 = "SELECT `user_id` FROM `access_table` WHERE `item_id`=48";  // کاربری که به تاییدیه مالی دسترسی دارد
            ...
            // $rst1 = $db->ArrayQuery($sql1);
            // $receiver = $rst1[0]['user_id'];
            $receiver = $ut->get_full_access_users(29)[0];
      ...
      //  $sqq = "SELECT `Transactions`,`checkOutType`,`related_vat` FROM `pay_comment` WHERE `RowID`={$pid}";
        ...
        for ($i=0;$i<$cnt;$i++)
        {
            ...
            if($acm->hasAccess('ViewHistoryCommentManagement'))
            {
                if(!empty($res[$i]['view_history']))
                {
                    $htm .= '<td style="text-align: center;font-family: dubai-Regular;padding: 10px;'.$style.'"><span style="cursor:pointer" onclick="get_view_detailes('.$res[$i]['RowID'].')">'.$html_seen_sign.'</span></td>';
                }
                else{
                    $htm .= '<td style="text-align: center;font-family: dubai-Regular;padding: 10px;'.$style.'">'.$html_tick.'</td>';
                }
...
/*    public function getReportPayCommentManageList($unCode,$rcsDate,$rceDate,$rcaName,$rcToward,$rcAmount,$rcPaytype,$rcPaySend,$rcSortType,$rpcsDate,$rpceDate,$rpnsDate,$rpneDate,$dUnit,$mUnit,$layer1,$layer2,$layer3,$page=1){
        ...
        for($y=0;$y<$listCount;$y++){
            switch ($res[$y]['sendType']){
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $finalRes[$y]['payed_other']=number_format($res[$y]['Amount']);
                    break;
            }
            $finalRes[$y]['RowID'] = $res[$y]['pcid'];
    ...
    public function getTotalAmountPayComment($unCode,$rcsDate,$rceDate,$rcaName,$rcToward,$rcAmount,$rcPaytype,$rcPaySend,$rpcsDate,$rpceDate,$rpnsDate,$rpneDate,$dUnit,$mUnit,$c_type){
        ...
        $sqq3 = "SELECT `dAmount_payed_by_others` FROM `deposit` WHERE `pid` IN ({$rowIDs})";
        $ut->fileRecorder($sqq3);
        $rsq = $db->ArrayQuery($sqq);
        $rsq1 = $db->ArrayQuery($sqq1);
        $rsq2 = $db->ArrayQuery($sqq2);
        $rsq3 = $db->ArrayQuery($sqq3);
        ...
        for ($i=0;$i<$cntq3;$i++){
            $payed_by_others += $rsq3[$i]['dAmount_payed_by_others'];
        }

        $Mandeh = $TotalAmount - ($Naghdi + $Cheki + $sabtDarHesab+$payed_by_others);

        $send = array($TotalAmount,$Naghdi,$Cheki,$sabtDarHesab,$Mandeh,$payed_by_others);
        return $send;
    }
    ...
    public function getPrintReportPayCommentHtm($cid){
        ...
        for ($i=0;$i<$cnt;$i++) {
           ...
           // if ($rsig[$i]['sender'] == 27 || $rsig[$i]['sender'] == $this->finance_manager_user_arr[0]){  // مدیر مالی
            if ($rsig[$i]['sender'] == $this->finance_manager_user_arr[0]){  // مدیر مالی
        ...
        $htm .= '<tr>';
        $htm .= '<td style="padding: 3px 0;width: 50%;background-color: #fff;font-size: 30px;font-family: BNazanin;font-weight: bold;text-align: right;" class="pr-3">واحد مصرف کننده : <span style="font-size: 25px;font-family: BTitr;">'.$rst[0]['unitName'].'</span></td>';
        $htm .= '<td style="padding: 3px 0;width: 50%;background-color: #fff;font-size: 30px;font-family: BNazanin;font-weight: bold;text-align: right;">'.$creditor_info.'</td>';
        $htm .= '</tr>';
        $htm .= '</thead>';
        $htm .= '</table>';
        ...
        //------------------------------------------------ اطلاعات امضا-----------------------------------------------------
        $htm .= '<footer style="width: 100%;border: 2px solid #000;display:flex justify-content:center">';
        $htm .= '<span style="padding: 2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;">'.$beginner.$fbeginner.'</span>';
        $htm .= '<span style="padding: 2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;">'.$Sarparast.$fSarparast.'</span>';
        $htm .= '<span style="padding: 2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;">'.$Moavenat.$fMoavenat.'</span>';

        //************************ امضا حسابداری *************************
        $htm .= '<span style="padding:2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;">'.$Hesabdar.$fHesabdar.'</span>';
        $htm .= '<span style="padding:2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;">'.$RHesabdar.$fRHesabdar.'</span>';
        $htm .= '<span style="padding:2px;width: auto;background-color: #fff;font-size: 25px;font-family: BNazanin;font-weight: bold;text-align: center;">'.$MHesabdar.$fMHesabdar.'</span>';
        ...
        $htm .= '<p class="pl-3" style="padding: 50px 0 20px 0;width: 100%;background-color: #fff;font-size: 30px;font-family: BNazanin;font-weight: bold;text-align: center">'.$Modiriat.$fModiriat.'</p>';
        $htm .= '</footer>';
    ...
    public function getFinancialConfirmationList($fcsDate,$fceDate,$fcaName,$fcToward,$fcAmount,$page=1){
      ...
      //  $ut->fileRecorder($col_name ."*tesrrrrrrrrrrrrrrrrrrrrrrr*" .$sql);
        $res = $db->ArrayQuery($sql);
        $listCount = count($res);
        $finalRes = array();
        for($y=0;$y<$listCount;$y++){
    ...
    public  function toggle_comment_force_status($arr){
        $db=new DBi();
        $ut=new Utility();
        extract($arr);
        $sql="SELECT `priorityLevel` from  `pay_comment` where `RowID`='{$cid}'";
        $res=$db->ArrayQuery($sql);
        $pl=1;
        if($res[0]['priorityLevel']==1){
            $pl=0;
        }
        
        $sql2="UPDATE `pay_comment` set `priorityLevel` ='{$pl}' where `RowID`='{$cid}'";
        $res2=$db->Query($sql2);
        return $res2;

    }

    public function uploadAccountGroupFile($gsplist){
        $acm = new acm();
        ...
        // }
        $db = new DBi();
        $file_to_delete = str_replace('\\', '/', dirname(__DIR__)).'/pmlexcel/excAccounts.xlsx';
        unlink($file_to_delete);

        if (isset($gsplist) && intval($gsplist['size']) > 0) {
            // $sql = "DROP TABLE IF EXISTS `goodsalepricelist`";
            // $db->Query($sql);
            $gsplFile = "excAccounts.xlsx";
            ...
            if(!$upload){
                ...
                //     return false;
                // }
                return $this->insertAccountGroupDB();
            }
        }else{
            return false;
        }
    ...
    private function insertAccountGroupDB(){
        $inputFileName = '../pmlexcel/excAccounts.xlsx';
        ...
        $key_index_excel = array_keys($excell_array_data[1]);
        $field_name_excel = array_values($excell_array_data[1]);
        $key_index_sample=['A','B','C','D','E','F'];
        $field_name_excel_sample=['code','role','national_code','acc_num','bank'];
        $array_diff1=array_diff($key_index_sample,$key_index_excel);
        $array_diff2=array_diff($field_name_excel_sample,$field_name_excel);
        if(count($array_diff1)==0 && count($array_diff2)==0){
            unset($excell_array_data[1]);
            $values="";
            foreach($excell_array_data as $key=>$array){
                ...
                $national_code=$array["D"];
                $acc_num=$array["E"];
                if(empty($name)){
                    return array('notName',$code);
                }
                if(intval($code)==0){
                    return array('undefined',$code);
                }
                $sql_check="SELECT `code` From `account` where `code`='{$code}'";
                $res=$db->ArrayQuery($sql_check);
                if(count($res)>0){
                    return array('duplicate',$code);
                }
                if(!empty($acc_num)){

                
                    $bank=$array["F"]?$array["F"]:'ثبت نشده است';
                }
                else{
                    $acc_num=0;
                    $bank="ندارد";
                ...
                $bank=$array["F"];
                $uid=$_SESSION['userid'];
                $values.="('{$name}','{$code}','{$role}','{$national_code}','{$acc_num}','{$bank}','{$uid}'),";
            }

            
            $values=rtrim($values,",");
            $ut->fileRecorder($values);
            $sql_insert="INSERT INTO account (`Name`,`code`,`account_role`,`codeMelli`,`accountNumber`,`bankName`,`uid`) VALUES {$values}";
            ...
            return array('notMatch');
        }
    }

    public function update_sabt_dar_hesab(){
        $db=new DBi();
        $ut=new Utility();
        $get_sabt_dar_hesb="SELECT * from `pay_comment` where `isEnable`=1 AND `sendType`=3 and `accountID` <> `creditor_id`";
        $res=$db->ArrayQuery($get_sabt_dar_hesb);
        foreach($res as $key=>$value){
            $this->create_sabt_dar_hesab_pay_comment($value['RowID']);
        }
    }

public function getPayrolls(){
    $db = new DBi();
    $sql = "SELECT `RowID` ,`fullname` FROM `revolving_payrolls` WHERE `status`=1";
    $res = $db->ArrayQuery($sql);
    if(count($res)>0){
        return $res;
    }else{
