...
class Documents{
    ...
    public function getOrganizationalDocumentationManageHtm(){
        ...
        if($acm->hasAccess('warrantyManage')) {
            ...
            $headerSearch2[$a]['options'][1]["title"] = "آرشیو شده";
            $headerSearch2[$a]['options'][1]["value"] = "0";
            $headerSearch2[$a]['options'][1]["title"] = "عودت شده  ";
            $headerSearch2[$a]['options'][1]["value"] = "-1";
            $a++;
...
/*        $items[$c]['type'] = "text";
    ...
    public function editWarranty($cid,$w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_acc,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc){
        $acm = new acm();
        if(!$acm->hasAccess('WarrantyManage')){
            die("access denied");
            exit;
        ...
        $sql = "UPDATE `sale_warranty` SET `warranty_title`='{$w_title}',`warranty_type`='{$w_acc_type}',`account_id`='{$w_acc}',`doc_owner_name`='{$w_doc_owner}',`doc_owner_nat_id`='{$w_doc_owner_nat_id}',`doc_other_info`='{$w_doc_info}',`access_id`='{$w_access_id}',`start_date`='{$SDate}',`end_date`='{$EDate}',`desc`='{$desc}' where RowID={$cid} ";
        $ut->fileRecorder($sql);
        $db->Query($sql);
        $res = $db->AffectedRows();
        $res = (($res == -1 || $res == 0) ? 0 : 1);
    ...
    public function createWarranty($w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc){
        $acm = new acm();
        if(!$acm->hasAccess('WarrantyManage')){
            die("access denied");
            exit;
       ...
       $ut->fileRecorder($w_account_id);
        $sql = "INSERT INTO `sale_warranty` (`warranty_title`,`warranty_type`,`account_id`,`doc_owner_name`,`doc_owner_nat_id`,`doc_other_info`,`access_id`,`start_date`,`end_date`,`desc`) 
                                    VALUES ('{$w_title}','{$w_acc_type}','{$w_account_id}','{$w_doc_owner}','{$w_doc_owner_nat_id}','{$w_doc_info}','{$w_access_id}','{$SDate}','{$EDate}','{$desc}')";
        $ut->fileRecorder($sql);
        $res = $db->Query($sql);
        if (intval($res) > 0){
            return true;
