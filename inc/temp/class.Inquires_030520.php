...
class Inquires{
	...
	public function get_inquires($status,$page=1,$other_params=[]){
       ...
       // $ut->fileRecorder('status:'.$w);
        ...
        if($status!=-1)
        {
            ...
            {
                if ($status == 0) {
                    $table_header = ['ردیف', 'نام استعلام ', 'تاریخ استعلام', 'کد استعلام', 'آخرین دریافت کننده', 'وضعیت استعلام', 'مدیریت'];
                } elseif ($status == 1) {
                    $table_header = ['ردیف', 'نام استعلام ', 'تاریخ استعلام', 'کد استعلام', 'آخرین دریافت کننده', 'وضعیت استعلام', 'مدیریت'];
                } elseif ($status == 2) {
                    $table_header = ['ردیف', 'نام استعلام ', 'تاریخ استعلام', 'کد استعلام', 'آخرین دریافت کننده', 'وضعیت استعلام', 'مدیریت'];
    ...
    public function save_inquiry_detailes($inq_good_name,$inq_good_quantity_unit,$inq_good_quantity,
                                            $inq_good_buy_request_num,$inq_good_desc,$buy_base_desc,
                                          $export_inq_seller_name,$export_good_quantity,$export_unit_price,
                                          $export_pay_method,$export_pay_detailes,$export_rent_inside,$export_rent_outside,$seller_group_code,$inq_id,
                                          $export_cash_section,$export_term_payment,$export_description,$is_vat,$temp_seller,$good_time_deliver){

        $db=new DBi();
        $ut=new Utility();
        $group_code=rand(1000000,9999999);
     //   $ut->fileRecorder("is_vat:".$is_vat);
       ...
       if(!empty($seller_group_code))
       {
           ...
           if(!empty($export_pay_method)){
               $sql_update7="UPDATE `inquiries_meta` set `value` = '{$export_pay_method}' WHERE `key`='export_pay_method' AND group_code={$seller_group_code};";
           ...
           if(!empty($export_cash_section)){
               $sql_update12="UPDATE `inquiries_meta` set `value` = '{$export_cash_section}' ,`sub_key`='ریال'  WHERE `key`='export_cash_section' AND group_code={$seller_group_code};";
               $res10=$db->Query($sql_update12);
               if(!$res12){
                   $flag=false;
               }
           }
        ...
        if(!empty($good_time_deliver)){
            $sql_update15="UPDATE `inquiries_meta` set `value` = '{$good_time_deliver}'   WHERE `key`='good_deliver_time' AND group_code={$seller_group_code};";
            $res10=$db->Query($sql_update15);
            if(!$res15){
                $flag=false;
            }
        }
          ...
          // if($vat_amount){
           ...
           $values = "('{$inq_id}','{$inq_good_name}','inq_good_id','','{$inq_good_name}','کد کالا','{$group_code}'),
                     ...
                     ('{$inq_id}','{$inq_good_name}','good_deliver_time','','{$good_time_deliver}',' زمان تحویل کالا','{$group_code}'),
                     ...
                     ('{$inq_id}','{$inq_good_name}','export_pay_method','','{$export_pay_method}','روش پرداخت','{$group_code}'),
                     
                     ('{$inq_id}','{$inq_good_name}','export_rent_inside','ریال','{$export_rent_inside}','کرایه حمل داخلی','{$group_code}'),
                     ('{$inq_id}','{$inq_good_name}','export_rent_outside','ریال','{$export_rent_outside}','کرایه حمل خارجی','{$group_code}'),
                     ('{$inq_id}','{$inq_good_name}','export_cash_section','ریال','{$export_cash_section}','مبلغ پرداخت نقدی','{$group_code}'),
                     ('{$inq_id}','{$inq_good_name}','export_term_payment','ریال','{$export_term_payment}','مبلغ پرداخت به چک','{$group_code}'),
    ...
    public function delete_all_form_inq($inq_id){
        $db = new DBi();
        $ut = new Utility();
        $sql="UPDATE inquiries_meta set status=0 where `inquiry_id` ={$inq_id}";
       // $ut->fileRecorder($sql);
        $res=$db->Query($sql);
        if($res){
            return 1;
    ...
    public function  create_description_seller($inq_id){
        ...
        $ut->fileRecorder($temp_seller_id);
        foreach($final_array as $group_code=>$arr_value){
           ...
           $k++;
            ...
            foreach($arr_value as $arr_index=>$arr_value1){
                ...
                $export_cash_section="";
                $export_term_payment="";
                if(!empty($arr_value1['export_pay_method'])){
                    $export_pay_method=$arr_value1['export_pay_method']." روز";
                }
                if(!empty($arr_value1['export_cash_section'])){
                    $export_cash_section=$arr_value1['export_cash_section']." ریال";
                }
                if(!empty($arr_value1['export_term_payment'])){
                    $export_term_payment=$arr_value1['export_term_payment']." ریال";
                }
                // if(!empty($arr_value1['export_pay_method'])){
                //     $export_pay_method=$arr_value1['export_pay_method']." روز";
                // }
            ...
            $html.="<fieldset id='box_".$group_code."' style='border: 2px solid gray;border-radius: 10px;padding: 10px;width:100%;border:2px solid rgb(".rand(0,255).",".rand(0,255).",".rand(0,255).")'><legend style='color:green;font-size:1rem;width: auto;padding: 5px'>$good_name</legend>";
            $html.="<div>
                        ...
                        <button title='مدیریت پیوست ها' style='color:blue' class='btn bg-light' type='button' onclick='display_inq_attachments(".$arr_value1['group_code'].",".$arr_value1['inquiry_id'].")'>مدیریت پیوست ها </button>
                    </div>";
            $html.="<table class='table'>".$inner_htmlheader.$inner_html."</table>";
            $html.="</fieldset> ";
    ...
    public function print_inq($inq_id,$print_paraf){
        ...
        foreach($print_array as $seller_key=>$seller_value)
        {
            ...
            <div class="print_inq" style="font-family:IranSans;font-size:11px;width: 100% ;direction:rtl;padding: 0;margin: 0;border:2px solid #000;page-break-after:always">
                ...
                $html.='<table border="1"  class="table table-bordered" style="border-collapse:collapse"><thead><tr><th style="width: 20%;height:0.3cm">نام فروشنده</th><th style="width:5%;height:0.3cm">تعداد کالا </th><th style="width: 10%"> محاسبه قیمت با ارزش افزوده </th><th style="width: 10%">قیمت تمام شده </th><th style="width: 10%">مبلغ نقدی</th><th style="width: 10%">مبلغ چک </th><th style="width: 10%">مدت بازپرداخت</th><th style="width: 10%">کرایه حمل</th><th style="width: 20%">توضیحات</th></tr></</thead><tbody>';
                foreach($seller_value as $seller_info_key=>$seller_info){
                    ...
                    $html.="<tr>
                                ...
                                <td>".$seller_info['export_good_quantity']." ".$this->get_unit($seller_info['inq_good_quantity_unit'])."</td>
                                <td>".$sign."</td>
...
public function attach_inq_file($file,$row_id,$file_title,$group_code,$inq_id){
   ...
   // $ut=>fileRecorder('2');
    ...
    public function get_seller_name($seller_id){
        $db=new DBi();
        $ut=new Utility();
        $sql="SELECT `Name` FROM account where RowID ='{$seller_id}'";
        $res=$db->ArrayQuery($sql);
        return $res[0]['Name'];
    ...
    public function get_seller_data($inq_id,$good_id){
        ...
        //$ut->fileRecorder('selerctor_inq'.$select_inq);
        $res_inq=$db->ArrayQuery($select_inq);
        foreach($res_inq as $value){
           ...
           // $ut->fileRecorder('test:'.$sql);
            ...
            foreach($res as $seller_info) {
                $array_handler['group_code'] = $seller_info['group_code'];
                switch($seller_info['key'])
                {
                    case"export_inq_seller_name":
                         $array_handler['export_inq_seller_name'] = $seller_info['value'];
                        break;
                    case'export_good_quantity':
                        $array_handler['export_good_quantity']= $seller_info['value'];
                        break;
                    case "export_unit_price";
                        $array_handler['export_unit_price'] = $seller_info['value'];
                        break;
                    case "export_pay_method":
                        $array_handler['export_pay_method'] = $seller_info['value'];
                        break;
                    case "export_pay_detailes":
                        $array_handler['export_pay_detailes'] = $seller_info['value'];
                        break;
                    case "export_rent_inside":
                        $array_handler['export_rent_inside'] = $seller_info['value'];
                        break;
                    case "export_rent_outside":
                        $array_handler['export_rent_outside'] = $seller_info['value'];
                        break;
                    case "export_cash_section":
                        $array_handler['export_cash_section'] = $seller_info['value'];
                        break;
                    case "export_term_payment":
                        $array_handler['export_term_payment'] = $seller_info['value'];
                        break;
                    case "export_description":
                        $array_handler['export_description'] = $seller_info['value'];
                        break;
                    case "is_vat":
                        $array_handler['is_vat'] = $seller_info['value'];
                        break;
                    case "good_deliver_time":
                        $array_handler['good_deliver_time'] = $seller_info['value'];
                        break;
                    case "temp_seller_name":
                        $array_handler['temp_seller_name'] = $seller_info['value'];
                        break;
                }
