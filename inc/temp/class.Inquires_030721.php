...
class Inquires {
    ...
    public function do_send_inquiry($reciever,$comment_array,$inq_id){
        ...
        $ut->fileRecorder('selected_inq_detailes');
        $ut->fileRecorder($selected_inq_detailes);
        $selected_rowids=[];
        ...
        $res=$db->Query($sql);
        $update_sql="UPDATE `inquiries` set `last_receiver`={$reciever},`status`=3 where `RowID`={$inq_id} ";
	...
	public function get_inquires($status,$page=1,$other_params=[]){
        ...
        switch($status){
            ...
            case"2":
            case 2:
                $w.="    i.`last_receiver`={$_SESSION['userid']} AND i.status=3";
        ...
        if($status!=-1)
        {
            ...
            if (count($result) > 0) 
            {
                ...
                foreach ($result as $row) {
                  ...
                  //  if ($status != 0) {
                        $htm .= '<td><button class="btn btn-primary-outline" onclick="open_inq_manage(' . $row['RowID'] . ',' . $status . ')"><i class="fa fa-angle-left manage_inq"</i></button</td>';
                   // } else {
    ...
    public function inq_get_all_goods($good_name){ 
		$db=new DBi();
		$ut=new Utility();
        $sql="SELECT RowID,code,title FROM inquiry_good where title <>'' AND title like '%{$good_name}%'";
		$res=$db->ArrayQuery($sql);
		return $res;
	}
