...
class Salary{
    ...
    public function getPersonnelList($Pname, $Pfamily, $Pcode, $Punit, $RsDate, $ReDate, $TsAmount, $TeAmount, $Pname1, $Pfamily1, $Pcode1, $Punit1, $RsDate1, $ReDate1, $TsAmount1, $TeAmount1, $ability, $status, $page = 1){
        $acm = new acm();
        if (!$acm->hasAccess('salaryBenefitsManage')) {
            die("access denied");
            exit;
    ...
    public function getPersonnelListCountRows($Pname, $Pfamily, $Pcode, $Punit, $RsDate, $ReDate, $TsAmount, $TeAmount, $Pname1, $Pfamily1, $Pcode1, $Punit1, $RsDate1, $ReDate1, $TsAmount1, $TeAmount1, $ability, $status){
        $acm = new acm();
        if (!$acm->hasAccess('salaryBenefitsManage')) {
            die("access denied");
            exit;
    ...
    public function editAllPersonnel(){
        ...
        for ($i=0;$i<$cnts;$i++) {
            if (intval($rts[$i]['type']) == 1) {  // پرسنل عادی
                $OutOfList1 = ($rts[$i]['SalaryOutofList'] / 30) * 30.5;
                $responsibilityRight1 = ($rts[$i]['responsibility_right'] / 30) * 30.5;
                $jobRight1 = ($rts[$i]['job_right'] / 30) * 30.5;
                ...
                // حقوق کل داخل لیست ماهانه
                $SalaryInofList = intval($monthlyWages) + intval($rts[$i]['RightHousing']) + intval($ChildAllowance) + intval($rts[$i]['Grocery']) + intval($Shift);
                ...
                if (intval($rts[$i]['insuranceNo']) == 1) {
                    ...
                    // حق بیمه سهم کارگر ماهانه
                    $Worker = ((intval($monthlyWages) + intval($rts[$i]['Grocery']) + intval($rts[$i]['RightHousing']) + intval($Shift)) * 7) / 100;

                    // حق بیمه سهم کارفرما ماهانه
                    $EmployerSalane = (((intval($monthlyWages) + intval($rts[$i]['Grocery']) + intval($rts[$i]['RightHousing']) + intval($Shift)) * 23) / 100);
    ...
    public function getPersonnelAgreementHtm($pid){
        ...
        // page 1
        $htm .= '<p style="text-align: center;direction: rtl;font-size: 20px;font-family: BTitr;">به نام خدا</p>';
        $htm .= '<p style="text-align: center;direction: rtl;font-size: 20px;font-family: BTitr;">شرکت فورج فلزات رنگین پارسیان (سهامی خاص)</p>';
        $htm .= '<p style="text-align: center;direction: rtl;font-size: 20px;font-family: BTitr;">((قرارداد کار))</p>';
        $htm .= '<p style="text-align: left;direction: rtl;font-size: 20px;font-family: BTitr;line-height: 0px;">' . $res[0]['PersonnelCode'] . '</p>';
        $htm .= '<hr>';
        ...
        $htm .= '<p style="text-align: justify;direction: rtl;font-size: 19px;font-family: BLotus;"><span style="text-align: right;direction: rtl;font-size: 15px;font-family: BTitr;">تبصره 1 : </span>مدت قرارداد بابت دوره های متوالی نهایتا تا سی و یکم اردیبهشت ماه سال آینده، <span style="font-size:15px;font-weight:bold;">بصورت یکطرفه از طرف کارفرما</span>، قابل تمدید می باشد.</p>';
        ...
        $htm .= '<p style="text-align: justify;direction: rtl;font-size: 19px;font-family: BLotus;">حقوق و مزایای قانونی بر اساس آخرین مصوبات شورای عالی کار و در سال 1403 مطابق با جدول زیر خواهد بود.</p>';
