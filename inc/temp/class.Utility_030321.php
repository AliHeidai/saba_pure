    ...
    public function getHtmlOfDefaultManagementPage($pagename='',$pageIcon='',$contentId='',$buttons=array(),$headerSearch=array(),$inputs='',$changeableFields=array(),$hiddenContentId=''){
        ...
        if(count($buttons) > 0){
            ...
            for($b=0 ;$b<$Countbuttons;$b++){
                $htm .= '<button type="button" onclick="'.$buttons[$b]["jsf"].'()" class="btn btn-light" '.(isset($buttons[$b]['id']) ? $buttons[$b]['id'] : '').' '.(isset($buttons[$b]['style']) ? $buttons[$b]['style'] : '').' title="'.$buttons[$b]["title"].'"><i class="fas '.$buttons[$b]["icon"].'"></i>&nbsp;&nbsp;'.$buttons[$b]["title"].'</button>';
            }
            $htm .= '</div>';
        }
            ...
            for ($j = 0; $j < $manifold; $j++) {
                ...
                if (count($bottons) > 0) {
                    ...
                    for ($b = 0; $b < count($bottons[$j]); $b++) {
                        $htm .= '<button type="button" onclick="' . $bottons[$j][$b]["jsf"] . '()" class="btn btn-light" title="' . $bottons[$j][$b]["title"] . '" '.(isset($bottons[$j][$b]['id']) ? $bottons[$j][$b]['id'] : '').' ><i class="fas ' . $bottons[$j][$b]["icon"] . '"></i>&nbsp;&nbsp;' . $bottons[$j][$b]["title"] . '</button>';
                    }
                    $htm .= '</div>';
                }
...
// //***************************************************************************************************
	...
	public function createJsonFile($array=[]){
        ...
        $resArray=$resArray['userLogin'];
        //error_log(print_r($resArray,true));
        $resArray[$array['userID']]=array("ip"=>$array['ip']);
        $json = json_encode(array('userLogin' => $resArray));
