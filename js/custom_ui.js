    ...
    switch(attrs_array['type']){
        ...
        case "fieldset":
           //</legend></fieldset>`;
            ...
            var element=`<fieldset  ${attrs} ><legend style="padding:10px;width:auto;font-size:12px">`;
            element+=`${title}</legend></fieldset>`
...
function create_table(selector,data_array,style="",thead=0,border=1){
 ...
 function set_steps_form(selector_form,steps,current_step,callback_array,section_ides_array,active_steps=1){
    ...
    $(selector_form).children('section').each(function(index){
        ...
        $("#step_btn_"+index).on('click',async function(e){//-------------- for transfer to next step
            ...
            if(typeof callback_array[index] ==="function")
            {
                ...
                console.log('test section 3');
                console.log(res);

                if(!res|| typeof(res)=="undefined" || typeof res =="undefined" ){
                    custom_alerts('اطلاعات را کامل نمایید'),'e',0,'خطا';
                    return false;
                }
