...
function get_current_shamsi_today(){
   var d= new Date();
   var y=d.getFullYear();
   var m=d.getMonth()+1;
   var d=d.getDate();

  var s=farvardin.gregorianToSolar(y , m , d , "string").replaceAll('-','/')
  return s
}

async function get_selected_inqs(selected_array){
   var action = get_selected_inqs;
   var params={action:action,selected_array:selected_array}
   var res= await manageAjaxRequestCustom(params);

}

async function do_inq_primary_confirm(inq_id){
    var action = "do_inq_primary_confirm";
    var inq_primary = $('input[name="inq_primary"]:checked').val();
    if(typeof(inq_primary)=="undefined"){
        swal_custom('','لطفا موافقت یا عدم موافقت با ثبت اظهارنظر را مشخص نمایید','w')
    }
    var res=await manageAjaxRequestCustom({action:action,inq_id:inq_id,inq_primary_confirm:inq_primary});
    if(res.res=="true"){
        menu_item_display(goToPurchaseInquiry,'PurchaseInquiry');
        swal_custom('',res.data,'s')

    }
    else{
        swal_custom('',res.data,'w')
        return false;
...
function addSubtraceDays (date, days) {
    const newDate = new Date(date);
    newDate.setDate(date.getDate() + days);
    return newDate;
}

function set_persian_date(element,status=-1){
    ...
    var date_j=$(element).MdPersianDateTimePicker(date_json);
    // var todayDate=new Date();
    // var disable_date=todayDate
    // if(origin_day!=0){
    //     disable_date =addSubtraceDays(todayDate, origin_day)
    // }
    ...
    //         date_json.disableAfterDate=new Date(disable_date)
    //     }
    //     if(status==1){
    //         date_json.disableBeforDate= new Date(disable_date)
    //     }
    // }
   // var date_j=$(element).MdPersianDateTimePicker(date_json);
 ...
 async function swal_custom(title,text,type,abort_btn_text="انصراف",confirm_btn_text="بله"){
    var msg_type="";
    switch(type)
    {
        case "e":
...
//       res = data;
//     //    if(res.trim()=="access_denied"){
//     //     window.location.href="frp-saba.local/login.php";
//     //    }
...
function display_inq_counts(){
    ...
    $(".inq_count").hide();
    console.log('res',res);
    if(parseInt(res.temp)>0){
        $("#inq_count_temp").text(res.temp).show();
    }
    if(parseInt(res.ready)>0){
        $("#inq_count_ready").text(res.ready).show();
    }
    if(parseInt(res.sended)>0){
        $("#inq_count_sadereh").text(res.sended).show();
    }
    if(parseInt(res.received)>0){
        $("#inq_count_varede").text(res.received).show();
    }
    if(parseInt(res.confirmed)>0){
        $("#inq_count_confirmed").text(res.confirmed).show();
    }
}
...
async function goToPurchaseInquiry(){
    ...
    `<ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item" role="presentation" style="position:relative">
            ...
            <span class="inq_count" id="inq_count_temp">0</span>
        </li>
        <li class="nav-item" role="presentation" style="position:relative">
            ...
            <span class="inq_count" id="inq_count_ready">0</span>
        </li>
        <li class="nav-item" role="presentation" style="position:relative">
            ...
            <span class="inq_count" id="inq_count_varede">0</span>
        </li>
        <li class="nav-item" role="presentation" style="position:relative">
            ...
            <span class="inq_count" id="inq_count_sadereh">0</span>
        </li>
        <li class="nav-item" role="presentation" style="position:relative">
            <button class="nav-link inq_tab" id="confirmed_inqs-tab" data-bs-toggle="tab" data-bs-target="#saderh_inqs" type="button" role="tab" aria-controls="saderh_inqs"  onclick="get_inquires(this,11,'temp_inq_detailes')""aria-selected="false">استعلام های در انتظار تایید</button>
            <span class="inq_count" id="inq_count_awaiting_confirmation">0</span>
        </li>
         <li class="nav-item" role="presentation" style="position:relative">
            <button class="nav-link inq_tab" id="confirmed_inqs-tab" data-bs-toggle="tab" data-bs-target="#saderh_inqs" type="button" role="tab" aria-controls="saderh_inqs"  onclick="get_inquires(this,20,'temp_inq_detailes')""aria-selected="false">استعلام های تایید شده</button>
            <span class="inq_count" id="inq_count_confirmed">0</span>
        </li>
...
function create_new_inquiry(){
    ...
    $("#inq_date").MdPersianDateTimePicker({
        disableBeforeDate:addSubtraceDays(currentdate,-2),
        disableAfterDate:currentdate
    })
    $("#inq_date").val(get_current_shamsi_today());
    $("#create_inq").modal({backdrop:'static',keyboard:false},'show');

}

 async function do_create_inquiry(){
...
function add_inq_pay_row(input){
...
function get_inquires(input,status,selector,page=1,other_params={},is_search=0){
...
function search_inquiry(input,status)
{
    ...
    if(status==0){
        tab_input=$("#temp_inqs-tab")
    }
    if(status==1){
        tab_input=$("#ready_inqs-tab")
    }
    if(status==2){
        tab_input=$("#varede_inqs-tab")
    }
    if(status==3){
        tab_input=$("#sadereh_inqs-tab")
    }
    if((inq_to=="" && inq_from=="") ||(inq_to && inq_from)){
       
        get_inquires(tab_input,status,'temp_inq_detailes',1,other_params,1)
...
async function add_row_inq_table(added_goods,edit=0,inq_id){
    console.log('added_goods',added_goods);
    ...
    var td_content=`<div >
                ...
                <fieldset class="form-group-custom">
                    ...
                    <p class="text-center">${added_goods['desc']}</p>
                </fieldset>
            </div>`
    ...
    var seller_td="";
        $("#inq_table").find("th.inq_detailes_th").each(function(){
            ...
            seller_td+=`<td class="inq_detaile_td" style="min-width:300px;min-height:300px">
                <div class="detailes_box" style="min-height:300px;height:auto">
                    <button test="1" id="more_seller_data_${seller_id}" test="3"  onclick="register_inq_detailes(this,'${seller_name}','${seller_id}')" class="btn btn-success">ثبت/ویرایش جزییات</button>
                </div>
            </td>`
        });

        var last_inq_html  = await last_inq_data(good_id,inq_id);
        
        return `<tr style="border-bottom:4px solid gray" good_quantity="${added_goods['quantity']}" good_id="${good_id}" good_name="${added_goods['good_name']}" class="good_detailes" id="good_detailes_${good_id}">
        <td >
            ${td_content}
        ...
        <td id="last_inq_result_${good_id}">
            <div>
            ${last_inq_html}
            <input type="hidden" value="${added_goods['quantity']}" id="quantity_hidden_${added_goods['good_id']}">
            </div>
        </td>
...
async  function last_inq_data(good_id,inq_id){
    var action='get_last_inq_data'
    var html='<div id="last_inq_result">';
    var params={inq_id:inq_id,good_id:good_id,action:action}
    var res=await manageAjaxRequestCustom(params);
    console.log('res_parent_id',res);
    if(res.res=='true'){
        ...
        if(Object.keys(res['data']).length){
            if(res['data']['parent_id']){
                if(res['data']['parent_id']==-1){
                    ...
                    html+=`<div><p class="text-center color-secondary"> &#10004;  &nbsp;خرید برای اولین بار</p></div>`
                }
            ...
            else
            {
                ...
                html+=`<table class="inq_inner_table">
                    <tr>
                        <td>فروشنده :</td>
                        <td>${res['data']['Name']}</td>
                    </tr>
                    <tr>
                        <td> قیمت :</td>
                        <td>${res['data']['ultimate_price']} ریال</td>
                    </tr>
                    <tr>
                        <td> مدت باز پرداخت :</td>
                        <td>${res['data']['pay_term']} روز</td>
                    </tr>
                    <tr>
                        <td> تاریخ خرید :</td>
                        <td>${res['data']['inq_date']}</td>
                    </tr>
                </table>
                
                <!--<button class="btn btn-success">مشاهده جزییات</button>-->`
            }
            html+=`<input type="hidden" good_id="${good_id}"  id="last_inq_hidden_${good_id}" value="1"/>`
        }
        else{
            html+=
            `<div id="last_inq_result">
                <label > <input type="checkbox" id="first_inq_${good_id}" onclick="toggle_add_last_inq(this,${good_id})"> خرید اولین بار کالا </label>
                <button  id="last_inq_btn_${good_id}" onclick="add_last_inq(${good_id})" class="btn btn-dark">افزودن آخرین  استعلام</button>
                <input type="hidden" good_id="${good_id}"  id="last_inq_hidden_${good_id}" value="-1"/>
            </div>`
...
async function toggle_add_last_inq(input,good_id){
    var checked=$(input).is(":checked")?1:0
    if(checked==1){
        $("#last_inq_btn_"+good_id).hide();
        var conf=await swal_custom('','استعلام این کالا برای اولین  بار می باشد و قبلا خریدی در خصوص این کالا صورت نگرفته است ','c')
        if(conf){
            var action="set_first_inq";
            var inq_id=$("#inq_id_hidden").val();
            var params={action:action,good_id:good_id,inq_id:inq_id}
            var res= await manageAjaxRequestCustom(params);
            if(res.res=="true"){
                $("td#last_inq_result_"+good_id).html(`<div><p class="text-center color-secondary"> &#10004;  &nbsp;خرید برای اولین بار</p></div>`);
                $("#last_inq_hidden_"+good_id).val(0)
            }
            else{
                swal_custom('',res.data,'w');
                $("#last_inq_btn_"+good_id).show(); 
                $("#last_inq_hidden_"+good_id).val(-1)
...
async function add_last_inq(good_id){
    var seller_array = get_sellers();
    var p_objects = get_all_citeis();
    var checked=$("#first_inq_"+good_id).is(":checked")?1:0;
    if(checked==1){
        return false
    }
    ...
    if($("#last_inq_modal")){
        $("#last_inq_modal").remove();
    }
    var modal=`<div class="modal" tabindex="-1" role="dialog" id="last_inq_modal">
                <div class="modal-dialog" role="document" style="max-width:950px">
                    <div class="modal-content">
                    <div class="modal-header text-light bg-primary">
                        <h5 class="modal-title">ثبت آخرین خرید</h5>
                    </div>
                    <div class="modal-body">
                    <h6 class="p-4"><span class="ml-4"> شرح کالا :</span><span style="color:green"> ${get_all_goods(good_id)['title']}</span></h6>
                      <form id="inq_detailes_form" style="">
                            <div class="form-group row">
                                <div class="col-sm-6">
                                    <div class="col-sm-12">
                                        <label for="staticEmail" class="col-sm-4 col-form-label">تاریخ استعلام</label>
                                        <div class="col-sm-8">
                                            <input type="text" class="form-control" id="last_inq_date">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="col-sm-12">
                                        <label for="staticEmail" class="col-sm-4 col-form-label">مقدار درخواستی</label>
                                        <div class="col-sm-8">
                                            <input onkeyup="set_separator(this)"  type="text" class="form-control" id="last_good_amount">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-sm-6">
                                <div class="col-sm-12">
                                    <label for="staticEmail" class="col-sm-4 col-form-label">مبنای درخواست  </label>
                                    <div class="col-sm-8">
                                        <select id="last_base_inq">${base_options}</select>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <label for="staticEmail" class="col-sm-4 col-form-label"> شماره درخواست خرید </label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="last_purchase_number">
                                </div>
                            </div>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-6">
                                <label for="staticEmail" class="col-sm-4 col-form-label">انتخاب تامین کننده </label>
                                <div class="col-sm-8">
                                    <select class="form-control" onchange="get_seller_detailes(this,1)"  id="last_inq_seller_selcect">${seller_options}</select>
                                </div>
                            </div>
                            <div class="col-sm-6">
                            <div class="col-sm-12">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="last_seller_phon">
                                </div>
                            </div>
                        </div>
                        </div>
                            <div class="form-group row">
                            <input type="hidden" id="last_inq_good_id" value="${good_id}">
                                <div class="col-sm-6">
                                    <label for="staticEmail" class="col-sm-4 col-form-label">استان</label>
                                    <div class="col-sm-8">
                                        <select class="form-control" id="last_pri_origin_of_send" onchange="get_citeis(this,'last')" >${option_privonce}</select>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <label for="staticEmail" class="col-sm-4 col-form-label">شهرستان </label>
                                    <div class="col-sm-8">
                                        <select class="form-control" onchange="" id="last_city_origin_of_send"></select>
                                    </div>
                                </div>
                            </div>
                       </div>        
                    <div id="last_inq_detailes">
                        <div class="row form-group">
                            <div class="col-sm-6">
                                <label class="col-sm-6">ارائه فاکتور با ارزش افزوده</label>
                                <div class="col-sm-6">
                                        <label> بله<input name="last_is_vat" type="radio" value="1" onchange="set_ultimat_price()"></label>
                                        <label> خیر<input name="last_is_vat" type="radio" value="-1" onchange="set_ultimat_price()"></label>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6" >
                                ...
                                <div class="col-sm-6">
                                        <label> درصدی<input name="last_rent_percent" type="radio" value="1" onchange="set_ultimat_price()"></label>
                                        <label> عادی<input name="last_rent_percent" type="radio" value="-1" onchange="set_ultimat_price()"></label>
                                </div>
                            </div>
                        </div>
                        <div class="row form-group" id="last_tax_risks_box">
                            <div class="col-sm-12">
                                <label class="col-sm-3">ریسک های مالیاتی</label>
                                <div class="col-sm-9">
                                    <select onchange="set_ultimat_price()" id="last_tax_risks" class="last_risk_select" multiple data-live-search="true">
                                        
                                    ${await get_risks('tax_risk')}
                                    </select>
                                </div>
                            </div>
                        </div>

                        <div class="row form-group" id="last_tamin_risks_box">
                            <div class="col-sm-12">
                                <label class="col-sm-3">ریسک های تامین اجتماعی</label>
                                <div class="col-sm-9">
                                    <select onchange="set_ultimat_price()" id="last_tamin_risks" class="last_risk_select" multiple data-live-search="true">
                                        <!--<option value="25">حد بالای عدم ارائه کد ملی</option>
                                        <option value="18">حد پایین عدم ارائه کد ملی</option>-->
                                         ${await get_risks('tamin_risk')}
                        ...
                        <div class="row form-group">    
                            <div class="col-sm-6">
                                <label class="col-sm-6">قیمت واحد (ریال)</label>
                                <div class="col-sm-6">
                                    <input class="form-control" onkeyup="set_separator(this)" onchange="set_ultimat_price()" type="text" id="last_unit_price">
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <label class="col-sm-6">مدت بازپرداخت (روز)</label>
                                <div class="col-sm-6">
                                    <input class="form-control"  type="text" id="last_pay_term">
                                </div>
                            </div>
                        </div>
                        <div class="row form-group">
                            <div class="col-sm-6">
                                <label class="col-sm-6">(ساعت)اعتبار قیمت</label>
                                <div class="col-sm-6">
                                    <input class="form-control" onkeyup="set_separator(this)"  type="text" id="last_price_valid">
                                </div>
                            </div>

                            <div class="col-sm-6" id="rent_div">
                                <label id="last_rent_lbl" class="col-sm-6">کرایه حمل (درصدی )</label>
                                <div class="col-sm-6">
                                    <input class="form-control" onkeyup="set_separator(this)" onchange="set_ultimat_price()"  type="text" id="last_rent">
                                </div>
                            </div>
                        </div>
                        <div class="row form-group">
                            <div class="col-sm-6">
                                ...
                                <div class="col-sm-6">
                                    <input onkeyup="set_separator(this)" class="form-control"  type="text" id="last_ultimate_price">
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <label class="col-sm-6"> توضیحات</label>
                                <div class="col-sm-6">
                                    <textarea id="last_description" class="form-control"></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="row form-group last_ultimate_price" style="display:flex;justify-content:center;margin:0 auto">
                    ...
                    <div class="modal-footer">
                        <button type="button" onclick="save_last_inq(${good_id})" class="btn btn-primary">تایید </button>
                        <button type="button" class="btn btn-danger" onclick="close_modal('last_inq_modal')" data-dismiss="modal">انصراف</button>
                    </div>
                    </div>
                </div>
                </div>`
    ...
    $("body").append(modal)
    $("#last_tax_risks").selectpicker();
    $("#last_tamin_risks").selectpicker();
    $(".last_risk_select").css('width','100%')
    $("#last_inq_modal").modal({backdrop:'static',keyboard:false},'show')
    $("input[name='last_is_vat']").on('click',function(){
       
        if($(this).val()==1){
            $("#last_tax_risks_box").hide();
          
        }
        if($(this).val()==-1){
            $("#last_tax_risks_box").show();
           
        }
    });
    var vat_input=$("input[name='last_is_vat']")
    if($(vat_input).prop('checked')){
        if($(vat_input).is(":checked").val()==1){
            $("#last_tax_risks_box").show();
        }
        if($(vat_input).is(":checked").val()==-1){
            $("#last_tax_risks_box").hide();
    ...
    select_data_picker("#last_inq_seller_selcect")
   // select_data_picker("#last_inq_seller_selcect")
    select_data_picker(".last_risk_select")
    set_persian_date("#last_inq_date");
    
}

function set_ultimat_price(return_value=0,last=1){
    if(last==1){
        var vat =$("input[name=last_is_vat]:checked").val()??0;
        var rent_type=$("input[name=last_rent_percent]:checked").val()??0
        var tax_risk=0;
        var unit_price=$("#last_unit_price").val()?$("#last_unit_price").val().replaceAll(',',''):0
        var tamin_risk=0;

        var last_rent=$("#last_rent").val()?$("#last_rent").val().replaceAll(',',''):0
        if(rent_type==1){
            last_rent=parseFloat(unit_price*last_rent/100)??0
            $("#last_rent_lbl").text('کرایه حمل(درصدی)');
        }
        else{
            $("#last_rent_lbl").text('کرایه حمل(عادی)');
        }
        if(vat==-1){
            tax_risk=parseFloat(unit_price)*($("#last_tax_risks").val())/100??0
        }
        tamin_risk=unit_price*parseInt($("#last_tamin_risks").val())/100??0
       
        var last_price=parseFloat(unit_price)+parseFloat(last_rent)+parseFloat(tax_risk)+parseFloat(tamin_risk)
        $(".last_ultimate_price").html('');
        if(last_price && last_price>0){
            $(".last_ultimate_price").html(`<span class="p-4"> قیمت تمام شده نهایی (محاسبه سیستم)</span><span class="py-4">:</span >  <span class="p-4" id="ultimate_price">${Number(last_price).toLocaleString()}  ریال </span>`);
        
        }
    }
    if(last==0){
        var vat =$("input[name=is_vat]:checked").val()??0;
        var rent_type=$("input[name=rent_percent]:checked").val()??0
        var tax_risk=0;
        var unit_price=$("#unit_price").val()?$("#unit_price").val().replaceAll(',',''):0
        var money_rent=0;

        var rent=$("#rent").val()?$("#rent").val().replaceAll(',',''):0
        if(rent_type==1){
            rent=parseFloat(unit_price*rent/100)??0
        }
        if(vat==-1){
            tax_risk=parseFloat(unit_price)*($("#inq_tax_risks").val())/100??0
        }
        var tamin_risk=unit_price * parseFloat($("#inq_tamin_risks").val())/100??0
        var price=parseFloat(unit_price)+parseFloat(rent)+parseFloat(tax_risk)+parseFloat(tamin_risk)
        var money_rent_array=[];
        
        if($("#pay_method_box_tbl").length){
            $("#pay_method_box_tbl tbody tr").each(function(){
                var pay_type=$(this).find("select[id^='pay_type']").val()
                var pay_amount=manage_number_format($(this).find("input[id^='pay_amount']").val(),0)
                var dat_diff=manage_number_format($(this).find("input[id^='pay_amount']").val(),0)
                
            })
        }
        $(".last_ultimate_price").html('');
        if(price && price>0){
            $(".ultimate_price").html(`<span class="p-4"> قیمت تمام شده نهایی (محاسبه سیستم)</span><span class="py-4">:</span >  <span class="p-4" id="ultimate_price">${Number(price).toLocaleString()}  ریال </span>`);
...
function manage_number_format(input,has_seprator=0){
   var input_type=typeof input;
   var returned_value="";
    switch(input_type){
       case "number":
       case "string":
            returned_value=input  
            if(has_seprator==1 && parseInt(input)>0){
            ...
            break;
        case "object":
            returned_value=input.value
            if(has_seprator==1){
                return Number(input).toLocaleString();
            }
            input.value=returned_value
            
            break;
    }
}

async function get_risks(risk_type){
    var action="get_risks";
    var res= await manageAjaxRequestCustom({action:action,risk_type:risk_type});
    var options=``;
    ...
    if(res.res=="true"){
        ...
        for(k in res_arr){
            options+=`<option value="${res_arr[k]['code']}">${res_arr[k]['description']}</option>`
        }
    }
    return options;
}

function select_checked_radio(radio_name)
{
    var radios=document.getElementsByName(radio_name)
    var check_value=''
    for(var i=0;i<radios.length;i++){
        if(radios[i].checked){
            check_value=radios[i].value
        }
    }
    return check_value;
}

async function save_last_inq(good_id){
    var inq_id=$("#inq_id_hidden").val();
    var seller = $("#last_inq_seller_selcect").val();
    var seller_phone = $("#last_seller_phon").val();
    var seller_ostan = $("#last_pri_origin_of_send").val();
    var seller_city = $("#last_city_origin_of_send").val();
    var is_vat = select_checked_radio('last_is_vat')??0;
    var rent_percent = select_checked_radio('last_rent_percent')??0
    var tax_risks = $("#last_tax_risks").val()??0;
    var tamin_risks = $("#last_tamin_risks").val();
    var unit_price = manage_number_format($("#last_unit_price").val());//numberformat($("#last_unit_price").val(),1);
    var pay_term = manage_number_format($("#last_pay_term").val());
    var price_valid = manage_number_format($("#last_price_valid").val());
    var pay_term = manage_number_format($("#last_pay_term").val());
    var rent = manage_number_format($("#last_rent").val());
    var desc = $("#last_description").val();
    var amount=$("#last_good_amount").val();
    var purchase_num=$("#last_purchase_number").val();
    var base_inq=$("#last_base_inq").val();
    var action="save_last_inq"
    var last_ultimat_price_user = manage_number_format($("#last_ultimate_price").val())
    var last_inq_date = $("#last_inq_date").val()
    if(seller==0){
        swal_custom('','تامین کننده انتخاب نشده است','w');
        return false;
    }
    if(seller_phone=="" || seller_phone.trim()=="" ){
        swal_custom('','شماره همراه تامین کننده وارد نشده است','w');
        return false;
    }
    if(seller_phone=="" ){
        swal_custom('','تامین کننده انتخاب نشده است','w');
    ...
    if(seller_ostan==0){
        swal_custom('','استان مبدا  انتخاب نشده است','w');
        return false;
    }
    if(seller_city==0){
        swal_custom('',' شهر مبدا  انتخاب نشده است','w');
        return false;
    }
    if(is_vat==0){
        swal_custom('',' وضعیت مالیات بر ارزش افزوده  مشخص نشده است','w');
        return false;
    }
    if(rent_percent==0){
        swal_custom('','وضعیت کرایه حمل مشخص نشده است ','w');
        return false;
    }
    if(seller==0){
        swal_custom('','تامین کننده انتخاب نشده است','w');
    ...
    if(is_vat==-1){
        if(tax_risks.length==0){
            swal_custom('','ریسک های مالیاتی مشخص نشده است','w');
            return false;
        }
    }
    if(unit_price==0 || unit_price==""){
        swal_custom('','قیمت واحد مشخص نشده است','w');
        return false;
    }

    if(pay_term==""){
        swal_custom('','مدت بازپرداخت مشخص نشده است','w');
        return false;
    }

    if(price_valid==""){
        swal_custom('','  مدت اعتبار قیمت مشخص نشده است','w');
        return false;
    }

    // if(rent==""){
    //     swal_custom('','  کرایه حمل مشخص نشده است','w');
    //     return false;
    // }
    var params={
        seller : seller,
        seller_phone : seller_phone,
        seller_ostan : seller_ostan,
        seller_city : seller_city,
        is_vat :is_vat,
        rent_percent : rent_percent ,
        tax_risks :tax_risks,
        tamin_risks : tamin_risks,
        ...
        desc : desc,
        action:action,
        last_ultimat_price_system:set_ultimat_price(1,1),
        last_ultimat_price_user:last_ultimat_price_user,
        good_id:good_id,
        inq_id:inq_id,
        amount:amount,
        purchase_num:purchase_num,
        base_inq:base_inq,
        inq_date:last_inq_date
    }
    var res=await manageAjaxRequestCustom(params)

    close_modal('last_inq_modal');
    open_inq_manage(inq_id,0)
   
}

// function isValidNumber(number) {
//     const regex = /^\d+$/;  // Adjust as needed
//     const test_res= regex.test(number);
//     if(!test_res){
...
async function get_seller_detailes(input,status=0){
    var action="get_seller_detailes";
    var seller_id=$(input).val();
    var res=await manageAjaxRequestCustom({action:action,seller_id:seller_id})
    var input_array={};
    if(res.res=="true"){
   
        if(status==0){
            input_array={ostan:'#pri_origin_of_send',city:'#city_origin_of_send',phone:'#seller_phon'};

        }
        if(status==1)
        {
            input_array={ostan:'#last_pri_origin_of_send',city:'#last_city_origin_of_send',phone:'#last_seller_phon'};
        }
        $(input_array.ostan).val(res.data.ostan)
        get_citeis($(input_array.ostan),'last')
        get_citeis($(input_array.ostan))
        $(input_array.city).val(res.data.city)
        $(input_array.phone).val(res.data.phone)
        // for(k in input_array){
        //     if($(input_array[k]).val()){

        //         $(input_array[k]).attr('disabled',true) 
        //     }
        //     else{
        //         $(input_array[k]).attr('disabled',false)   
        //     }
        // }
...
async function open_inq_manage(id,status,last_confirm=0){
   ...
   //-----------------------------------------------
    add_loading(action);
//    var footer=res.data[1]['footer_html'];
//    console.log('footer',footer);
   //-----------------------------------------------
    ...
    if(added_goods && added_goods.length>0){
       disable="";
        ...
        for(k in added_goods){
            ...
            good_td+=await add_row_inq_table(added_goods[k],0,id);
    ...
    switch(status){
        ...
        case 20:
        case "20":
        title=" استعلام تایید شده"
        break;
        case 11:
        case "11":
        title=" استعلام  در انتظار تایید"
        break;
            
    }
   // var add_good_btn=`<button id="add_good_row" onclick="add_good_inq(this)" class="btn btn-link mx-2 col-md-1">انتخاب  کالا</button>`
    var add_seting_btn=`<li class="inq_drop_down" style="position:relative"><a id="add_good_row"  class="nav-link text-primary"><i class="mx-1 fa fa-cogs"></i>اطلاعات اولیه</a>
        <ul class="inq_drop_down_content">
            <li class="p-2">
                <a  class="add_new_inq_data" id="add_new_good_link" onclick="add_new_seller_inq(this)"> <i class="mx-1 fa fa-cog"></i>ثبت طرف حساب جدید</a>
            </li>
            <li  class="p-2">
                <a class="add_new_inq_data" id="add_new_seller_link" onclick="add_new_good_inq(this)"> <i class="mx-1 fa fa-cog"></i>ثبت کالا/خدمت جدید</a>
            </li>
        </ul>
    </li>`
    var add_good_btn=`<li><a id="add_good_row" onclick="add_good_inq(this)" class="nav-link text-warning"><i class="mx-1 fa fa-calendar-check"></i>انتخاب کالا </a></li>`
   // var add_seller_btn=` <button ${disable} onclick="add_seller()" id="add_seller_btn"  title="انتخاب تامین کنندگان " class="btn btn-light col-md-1 mx-2">انتخاب تامین کنندگان </button>`
    var add_seller_btn=` <li ${disable}><a onclick="add_seller()" id="add_seller_btn"  title="انتخاب تامین کنندگان " class="nav-link text-primary"><i class="mx-1 fa fa-commenting"></i>انتخاب تامین کنندگان </a></li>`
    var inq_print_btn=` <li ${disable}><a onclick="inqiury_print(${id})" id="print_btn"  title="پرینت استعلام " class="nav-link text-info"><i class="mx-1 fa fa-print"> </i>پرینت استعلام</a></li>`
    var send_inq_btn=`<li ${disable}><a onclick="send_inq(${id},${status})" id="send_inq_btn"  title="گردش کار/ارسال" class="nav-link text-primary"><i class="mx-1 fa fa-paper-plane"></i>گردش کار/ارسال</a></li>`
    var ready_to_send_btn=`<li ${disable}><a onclick="send_to_ready()" id="send_to_ready_btn"  title="تایید و ادامه" class="nav-link text-success"><i class="mx-1 fa fa-check"></i>تایید و ادامه</a></li>`
    var inq_archive_btn=`<li ${disable}><a onclick="archive_inq(${id})" id="archive_inq_btn"  title="بایگانی و توقف" class="nav-link text-warning"><i class="mx-1 fa fa-archive"></i>  بایگانی و توقف  استعلام </a></li>`
    var inq_create_pay_comment_btn=`<li> <a  onclick="get_all_payComments(${id})" id="archive_inq_btn"  title="ثبت اظهارنظر " class="nav-link text-success"><i class="mx-1 fa fa-registered"></i>ثبت اظهارنظر </a></li>`
    var inq_create_inq_paraf_btn=`<li><a  onclick="inq_create_paraf(${id})" id="paraf_inq_btn"  title="مدیریت پاراف" class="nav-link text-danger"><i class="mx-1 fa fa-commenting"> </i>مدیریت پاراف </a></li>`
    var inq_final_confirm_btn=`<li><a  onclick="do_inq_last_confirm(${id})" id="last_confirm_inq_btn"  title=" تایید نهایی" class="nav-link text-success"><i class="mx-1 fa fa-check-square"> </i> تایید نهایی </a></li>`
    var inq_primary_confirm_btn=`<a class="btn btn-primary text-light" onclick="do_inq_primary_confirm(${id})" id="confirm_temp_inq_btn">ادامه</a>`
    var modal=`
    <div id="inq_detailes" >
    <ul class="inq_breadcrumb">
        <li style="list-style:none"><a href="#" onclick="menu_item_display(goToPurchaseInquiry,'PurchaseInquiry')">استعلام بها</a></li>
        <li style="list-style:none"><a href="#">${title}</a></li>  
    </ul>
    <div class="inq_main_menu">
    <ul class="inq_navbar">`
    var inq_temp_comment="";
            if(status==0){
                modal+=`${add_good_btn} ${add_seller_btn} ${ready_to_send_btn} ${add_seting_btn}`        
            }
            else
            {
                ...
                if(status==1){
                    ...
                    modal+=`${inq_create_inq_paraf_btn}`
                }
                if(status==2 && show_confirm==1){
                    modal+=`${inq_final_confirm_btn}`
                }
                if(status==2){
                     modal+=`${send_inq_btn}`
                     modal+=`${inq_create_inq_paraf_btn}`
                ...
                if(status==20 ){
                    modal+=`${inq_create_pay_comment_btn}`
                }
                if(status==11 ){
                    //modal+=`${inq_primary_confirm_btn}`
                    var inq_temp_comment=`
                    <div style="background:#3cd76c26;width:100%">
                    <div class="temp_container" style="width:50%;display:flex;gap:10px;padding:10px;margin:0 auto">
                        <div class="text-info"> مبنای ثبت این استعلام   در خواست شفاهی جنابعالی  (مدیریت محترم عامل) می باشد  </div>
                        <div>
                            <label  class="text-secondary"><input type="radio" value="1" name="inq_primary">بله</label>&nbsp&nbsp
                            <label class="text-secondary" ><input type="radio" value="0" name="inq_primary">خیر</label>
                        </div>
                        <div>${inq_primary_confirm_btn}</div>
                    </div>
                    </div>`;
                }
            ...
            modal+=`${inq_print_btn}</ul>`   
       modal+= `<div class="return_box"><i class="text-danger h5 fa fa-undo return_btn" aria-hidden="true" titile="بازگشت به لیست استعلامات" onclick="menu_item_display(goToPurchaseInquiry,'PurchaseInquiry')"></i></div>
        
       </div><input type="hidden" value="${id}" id="inq_id_hidden" >
...
for(var i=0;i<selected_sellers.length;i++){
    ...
    var td_text=`
    <div class="container">
        <div class="custom_row2 py-2">
            <h6 style="min-height:100%;height:100%" class="text-right id="selle_name_${index}" >
                <span>نام تامین کننده:</span>
                <span>${selected_sellers[i]['Name']} </span>
            </h6>
           <h6 class="text-right" id="origin_${index}">
                
            </h6>
            <h6 class="text-right" id="phone_${index}">
    ...
    appended_tbody_td=`<td class="inq_detaile_td" style='min-width:300px;min-height:300px'>
                            <div class="detailes_box" style="min-height:300px;height:auto">
                                ...
                                <div seller_id="${selected_sellers[i]['seller_id']}" s_rowID="${selected_sellers[i]['RowID']}" class="seller_summary" >
                               
                                   
                                </div>
                                  <button test="2" class="btn btn-success more_seller_data" id="more_seller_data_${selected_sellers[i]['seller_id']}" 
                                onclick="register_inq_detailes(this,'${selected_sellers[i]['Name']}','${selected_sellers[i]['seller_id']}','${selected_sellers[i]['RowID']}')" class='btn btn-success'>ثبت/ویرایش جزییات</button>
                            </div>
                        </td>`
    ...
    $("#origin_"+index).html(`<span>مبدا ارسال :</span><span>${await get_ostan_city(selected_sellers[i]['ostan_id'])??"----"} - ${await get_ostan_city(selected_sellers[i]['city_id'])??"----"}</span>`);
    $("#phone_"+index).html(`<span> تلفن :</span><span> ${selected_sellers[i]['phone']??"----"} </span>`);
    $("#origin_data_"+selected_sellers[i]['seller_id'])
}
//------------------------------------------------
    if(show_flag){
        $("#inq_tbl_main").show();
    ...
    if(status !=0){
        ...
        $(".more_seller_data").text('مشاهده جزییات');
        $(".more_seller_data").addClass('btn-primary').removeClass('btn-success');
        // $('#seller_summary_'+rand).append(`<p>test-${rand}</p>`);
        // console.log('rand',rand);
       
    }
    else{
        $(".more_seller_data").text('مدیریت جزییات');
        $(".more_seller_data").removeClass('btn-primary').addClass('btn-success');
        
    }
    $("div[id^='seller_summary_']").each(async function(){
       
        var good_id=$(this).parents('tr').attr('good_id');
        var seller_id=$(this).attr('seller_id');
        var RowID=$(this).attr('s_rowID');
        var htm= await show_summary_suggests(good_id,seller_id,RowID,1,show_confirm)
       $(this).html(htm)
       
    })
    // $("#inq_tbl_main #inq_table").append(`<tr class="paraf_inq_tr"><td colspan="10">${footer}</td></tr>`);
    
    // $(".paraf_box").css('width',100/parseFloat(res.data[1]['footer_box_count'])+"%")
    get_inq_footer(id);
   
}

async function get_inq_footer(inq_id){
    var action="get_inq_footer";
    var res=await manageAjaxRequestCustom({action:action,inq_id:inq_id});
    console.log('res_footer',res);
    if(res.res=="true"){
        if( $("#inq_tbl_main #inq_table tr.paraf_inq_tr").length){
            $("#inq_tbl_main #inq_table tr.paraf_inq_tr").remove();
        }
        $("#inq_tbl_main #inq_table").append(`<tr class="paraf_inq_tr"><td colspan="10">${res.data['footer_html']}</td></tr>`);
        $(".paraf_box").css('width',100/parseFloat(res.data['footer_box_count'])+"%")
    }

}


async function add_new_seller_inq(){
    var modal_id="new_seller_modal";
    if($("#"+modal_id).length){
        $("#"+modal_id).remove();
    }
    var modal=`

<div  id="${modal_id}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">افزودن تامین کننده جدید</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
      ...
      <div class="modal-body">
        <form>
            <div class="form-group row">
                <label for="staticEmail" class="col-sm-4 col-form-label">نام ونام خانوادگی</label>
                <div class="col-sm-6">
                    <input type="text"  class="form-control" id="seller_fullname" value="">
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword" class="col-sm-4 col-form-label">کد تفضیلی</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="code_tafzili" />
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword" class="col-sm-4 col-form-label">کد ملی</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="code_melli" />
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword" class="col-sm-4 col-form-label">شماره همراه </label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="mobile" />
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword" class="col-sm-4 col-form-label">شماره ثابت </label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="phone" />
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword" class="col-sm-4 col-form-label"> آدرس </label>
                <div class="col-sm-6">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="add_new_seller()">تایید</button>
      </div>
    </div>
  </div>
</div>`;
...
async function add_new_seller(){
    var seller_name=$("#seller_fullname").val();
    var tafzili=$("#code_tafzili").val();
    var code_melli=$("#code_melli").val();
    var mobile=$("#mobile").val();
    var seller_phon=phone=$("#phone").val();
    var seller_address=$("#addres").val();
   
    if(seller_name.trim()=="" || typeof seller_name.trim()=="undefined"){
        custom_alerts('نام فروشنده را وارد نمایید','e',0,'خطا');
        return false;
    }
     if(mobile.trim()=="" || typeof seller_phon.trim()=="undefined"){
        custom_alerts('شماره همراه  فروشنده را وارد نمایید','e',0,'خطا');
        return false;
    }
    if(phone.trim()=="" || typeof seller_phon.trim()=="undefined"){
    ...
    var params={action:action,seller_name:seller_name,phone:phone,seller_address:seller_address,mobile:mobile,tafzili:tafzili,code_melli:code_melli}
    var res=await manageAjaxRequestCustom(params)
...
async function add_new_good_inq(){
    
}

async function save_inq_paraf(inq_id,paraf_id=0){
    var action="save_inq_paraf";
    var paraf_text=$("#inq_paraf_txt").val();
    paraf_id=$("#inq_paraf_id").val()??0
    if(paraf_text.trim()==""){
        swal_custom('','متن پاراف خالی می باشد','w')
        return false;
    }
    var params={action:action,paraf_text:paraf_text,inq_id:inq_id,paraf_id:paraf_id}
    var res=await manageAjaxRequestCustom(params)
    if(res.res=="true"){
        swal_custom('',res.data,'s')
        $("#inq_paraf_txt").val('');
    ...
    await get_inq_parafs(inq_id)
    await get_inq_footer(inq_id)
}

async function edit_paraf(paraf_id,inq_id){
    var action = "edit_paraf"
    var param = {action:action,paraf_id:paraf_id,inq_id:inq_id}
    $("#inq_paraf_id").val(paraf_id);
    var res = await manageAjaxRequestCustom(param)
    if(res.res == 'true'){

        $("#inq_paraf_txt").val(res['data']['paraf_text'])
    }
    else{
        swal_custom('',res.data,'w')
    }
...
async function delete_paraf(paraf_id,inq_id){
    var conf=await swal_custom('','پاراف حذف شود؟','c')
    if(!conf){
        return false;
    }
    var action = "delete_paraf"
    var param = {action:action,paraf_id:paraf_id,inq_id:inq_id}
    var res = await manageAjaxRequestCustom(param)
    if(res.res=='true'){
        swal_custom('',res.data,'s')
...
async function get_inq_parafs(inq_id){
    var action="get_inq_parafs";
    var params={action:action,inq_id:inq_id}
    var res=await manageAjaxRequestCustom(params)
    if(res.res=="true"){
       $("#paraf_lists").html('')
       $("#paraf_lists").html(res.data)
    }
}
 
async function inq_create_paraf(inq_id){
    var id="inq_paraf_"+inq_id
    if($("#"+id).length){
        $("#"+id).remove();
    }
    var modal=`
...
<div class="modal fade" id="${id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document" style="max-width:900px">
    <div class="modal-content" >
      <div class="modal-header text-light bg-primary">
        <h5  class=" modal-title" id="exampleModalLabel">مدیریت  پاراف </h5>
        <button type="button" class="close"  aria-label="Close">
          <span class="text-danger h3" onclick="close_modal('${id}')" aria-hidden="true">X</span>
        </button>
      </div>
      <div class="modal-body">
        <button class="btn btn-info" id="add_new_paraf_btn" onclick="add_new_inq_paraf(this,${inq_id})">ثبت پاراف جدید</button>
        <div id="inq_paraf_container"></div>
        
         
        <div id="paraf_lists"></div>
      </div>
...
$('body').append(modal);
$("#"+id).modal({keyboard:false,backdrop:"static"},'show')
await get_inq_parafs(inq_id)
}

function add_inq_paraf_form_html(input,inq_id){
    $("#inq_paraf_container").html('');
    $("#inq_paraf_container").html(`<form id="inq_paraf_form">
     
          
        <div class="inq_paraf_content"><div class="form-group  row">
            <label for="message-text" class="col-form-label col-md-4" style="font-size:10px;font-family:IranSans">متن پاراف:</label>
            <div class="col-md-8">
                <textarea class="form-control" id="inq_paraf_txt" style="min-height:90px;font-size:10px;font-family:IranSans;background:transparent"></textarea>
                <input type="hidden" value="0" id="inq_paraf_id" />
            </div>
          </div>
         <div class="inq_paraf_btn_box">
           <button type="button" class="btn btn-success" onclick="save_inq_paraf(${inq_id})">ذخیره </button>
           <button type="button" class="btn btn-danger" onclick="abort_inq_paraf()">انصراف </button>
           </div></div></form>`);
           return 1;
}
function add_new_inq_paraf(input,inq_id){
   add_inq_paraf_form_html(input,inq_id);

        $("#add_new_paraf_btn").hide();



}

function abort_inq_paraf(){
    $("#inq_paraf_container").html('');
    $("#add_new_paraf_btn").show();
}
...
async function send_to_ready(){
    ...
    var last_inq_flag=0;
    var inq_good_is_first_time=[]
    var inq_id=$("#inq_id_hidden").val();
    $("#inq_table tbody tr.good_detailes").each(function(){
    
    var checked=$(this).find('input[type="hidden"]').val();
    if(parseInt(checked)){
    ...
    $("#inq_table tbody input[id^='last_inq_hidden_'").each(function(){
        if($(this).val()==-1){
            last_inq_flag=1;
        }
    ...
    if(last_inq_flag==1){
        swal_custom('خطا','برای یک یا چند  کالا اطلاعات  آخرین خرید مشخص نشده است','w');
        return false;
    }

    if(good_rows_count>0){// && check_select_inq_detailes.length==good_rows_count){

        var conf2=await swal_custom('هشدار','در صورت تایید  قادر به ویرایش استعلام نخواهید بود  ادامه می دهید؟','c');
        if(!conf2){
            return  false;
        }
        var inq_id=$("#inq_id_hidden").val();
...
async function show_summary_suggests(good_id,seller_id,RowID,summary_type=0,last_confirm=0){

    var action="show_summary_suggests";
    var inq_id=$("#inq_id_hidden").val();
    var params={action:action,seller_id:seller_id,RowID:RowID,good_id:good_id,inq_id:inq_id}
    var res=await manageAjaxRequestCustom(params);
    var htm="";
   
    var rand= create_random_code();
    
    var thead=`<tr><td>قیمت واحد</td><td>مدت بازپرداخت</td><td>اعتبار قیمت</td><td>قیمت نهایی</td><td>توضیحات</td></tr>`
    // if(summary_type==1){
    //     thead=`<tr><th style="min-width:80px !important">قیمت واحد</th><th style="min-width:80px !important">مدت بازپرداخت</th><th style="min-width:80px !important">اعتبار قیمت</th><th style="min-width:80px !important" >توضیحات</th></tr>`
    // }
    var table=`<table  class="inq_inner_table">${thead}`
    var class_name="";
    var tbody='';
    if(res.data.length){
       if(summary_type==0){
            for(k in res.data){
                tbody+=`<tr>
                            <td>
                                ...
                                <span>${res.data[k]['unit_price']} ریال</span>
                            </td>
                            <td>
                            
                                <span>${res.data[k]['pay_term']}</span>
                            </td>
                            <td>
                                <input type="hidden" name="select_${good_id}" value="${res.data[k]['RowID']}">
                                <span>${res.data[k]['price_valid']}</span>
                            </td>
                            <td>
                                <span>${res.data[k]['ultimate_price']} ریال </span> 
                            </td>
                            <td>
                                <p style="width: 150px;overflow-wrap: break-word">${res.data[k]['description']}</p>
                            </td>
                        </tr>`
            }
       }
       ...
       {
        class_name='inq_inner_table'
        var obj_key=['ultimate_price','pay_term','price_valid','description'];
        
        var obj_key_fa=['قیمت واحد','مدت بازپرداخت','اعتبار قیمت','توضیحات'];
        if(last_confirm==1){
            obj_key_fa=['قیمت واحد','مدت بازپرداخت','اعتبار قیمت','توضیحات','انتخاب'];
            obj_key=['ultimate_price','pay_term','price_valid','description','select_inq_suggest'];
        }
        ...
        for(k in obj_key){
            table+=
            `<tr>
                <td style="width:25%">${obj_key_fa[k]}</td>`
               
            for(m in res.data){
                console.log('status',res.data[m].status)
                var post_fix="";
                var td_value="";
                td_value=res.data[m][obj_key[k]]
                if(obj_key[k]=='ultimate_price'){
                     post_fix=" ریال"
                    td_value=manage_number_format(res.data[m][obj_key[k]],1)
                }
                if(obj_key[k]=='pay_term'){
                    post_fix=" روز"
                    td_value=manage_number_format(res.data[m][obj_key[k]],1)
                    
                }
                if(obj_key[k]=='price_valid'){
                    post_fix=" ساعت"
                    td_value=manage_number_format(res.data[m][obj_key[k]],1)
                    
                }
                if(obj_key[k]=='select_inq_suggest'){
                   
                   
                    td_value=`<input status="${res.data[m]['status']}" onclick="select_inq_suggest(this)" sid="${RowID}" type="checkbox" id="suggest_${RowID}" name="suggest_${good_id}">`
                    
                }
                
                table+=`<td>${td_value} ${post_fix}</td>`
            }
            table+=`</tr>`
        ...
        htm+=`<table class="${class_name}">${table}</table>`
    }
    else{
...
function select_inq_suggest (input) {
   
    var row_id = $(input).attr('sid');
    var check_name= $(input).attr('name')
...
async function do_inq_last_confirm(inq_id){
    var good_rows=$("#inq_table td.good_detailes").length;
    var selected_suggest=[];
    $("#inq_table td input[id^='suggest']").each(function(){
        if($(this).prop('checked')){
            selected_suggest.push($(this).attr('sid')); 
        }
        
    });
    // if(selected_suggest.length!=good_rows){
    //     swal_custom('','از هر ردیف کالا باید یک پیشنهاد استعلام انتخاب شود','w');
    //     return false;
    // }

    var selected_suggest_stat=JSON.stringify(selected_suggest);
    var action="do_inq_last_confirm";
    var params={inq_id:inq_id,selected_suggest_stat:selected_suggest_stat,action:action}
    var res= await manageAjaxRequestCustom(params);
    if(res.res=="true"){
        swal_custom('',res.data,'s');
        menu_item_display(goToPurchaseInquiry,'PurchaseInquiry')
...
function add_seller(){
    ...
    if(!$("tr.good_detailes").length){
        swal_custom('','کالایی برای استعلام انتخاب نشده است','w');
        return false;
    }
    ...
    <div class="modal-dialog" role="document" style="max-width:900px">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">انتخاب تامین کننده </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span class="h3 text-danger" aria-hidden="true" onclick="close_modal('select_sellers_modal')">X</span>
                </button>
            </div>
        <div class="modal-body">
            <div id="seller_detailes_box">
            </div>
            <div class="row">
                <button class="btn btn-primary w-100 m-4" id="add_seller_detailes_btn" onclick="create_seller_form()">+</button>
            </div>
...
$("#add_seller_detailes_btn").show();

}

function create_seller_form(){
    var seller_array = get_sellers();
    var p_objects = get_all_citeis();
    var option_privonce=`<option value="0"> استان را انتخاب نمایید</option>`;
    var province = p_objects['data'];
    ...
    var form=`<form id="form_seller">
                <div class="form-group row">
                    <div class="col-sm-12">
                        <label for="staticEmail" class="col-sm-4 col-form-label">انتخاب تامین کننده </label>
                        <div class="col-sm-8">
                            <select class="form-control"  id="inq_seller_selcect" onchange="get_seller_detailes(this,0)" >${seller_options}</select>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    <div class="col-sm-12">
                        <label for="staticEmail" class="col-sm-4 col-form-label">استان</label>
                        <div class="col-sm-8">
                            <select class="form-control" id="pri_origin_of_send" onchange="get_citeis(this)" id="ostan_inq_seller_selcect">${option_privonce}</select>
                        </div>
                    </div>
                </div>
                <div class="form-group row">
    ...
    $("#seller_detailes_box").html(form)
    $("#add_seller_detailes_btn").hide();
...
async function add_good_inq(btn_input){
    ...
    //--------------------------------------------------------
      var modal=
      `<div id="select_goods_modal" class="modal" tabindex="-1"  aria-hidden="true" enter role="dialog">
...
function check_inserted_duplicate_good(input){
 ...
 function edit_create_good_row(input,good_id,inq_id){
            ...
            <div class="modal-body">
                 <form>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                        <select id="inq_base_inquiry" onchange="toggle_inq_file(this)" class="form-control">${base_options}</select>
                        </div>
                    </div>
                     <div class="form-group row">
                    ...
                    <div class="attach_written_order_box">
    ...
    get_inq_attachment_list(good_rows[0]['good_id'],inq_id);


 }

 function toggle_inq_file(base){
    var inq_id=$("#inq_id_hidden").val()
    if($("#attach_written_order").length){
       
        $(".attach_written_order_box").html('');
    }
    var base_value=0
    ...
    if(base_value==3){
        var html=`<div class="form-group row">
                    <label for="attach_written_order" class="col-sm-4 col-form-label">آپلود  دستور کتبی</label>
                    <div class="col-sm-8 row">
                        <div class="col-sm-9">
                            <input type="file" id="attach_written_order"/>
                        </div>
                        <div class="col-sm-3">
                            <button type="button" class="btn btn-success" onclick="uploadFile(${inq_id},'attach_written_order','دستور کتبی خرید')">+</button>
                        </div>
                    </div>
                </div>`
        $(".attach_written_order_box").append(html);
    }
 }
...
async function save_good_row(RowID=0){
    ...
    console.log('inq_number',inq_number);
    ...
    if(inq_base==1 || inq_base==2){
        if(inq_number==null || inq_number.trim()=="" ){
            swal_custom('خطا','شماره درخواست خرید  وارد نشده است','w');
            return false; 
        }
    }
    if(inq_base==3){
        if(!$("#inq_files_count").length){
            
            swal_custom('خطا','فایل  دستور کتبی  خرید آپلود نشده است','w');
            return false; 
        }
        else{
           if($("#inq_files_count").val()==0){
                swal_custom('خطا','فایل  دستور کتبی  خرید آپلود نشده است','w');
                return false; 
           }
        }
    }
    console.log('inq_number2',inq_number);
    var params={
        action:action,
        inq_id:inq_id,
        good_name:good_name,
    ...
    if(res.res=="true"){
        ...
        if(!$("inq_tbl thead th.inq_detailes_th").length){
            if(RowID==0){
                var td = await add_row_inq_table(res.data[0][0],0,inq_id);
                $("#inq_table tbody").append(td)
            }
            else{
                var td = await add_row_inq_table(res.data[0][0],1,inq_id);
        ...
        //await attach_inq_file(inq_id,$("#attach_written_order"),'دستور کتبی خرید')
        open_inq_manage(inq_id,0);
        $(".modal-backdrop").hide();
        if(!$("#quantity_hidden_"+good_name)){
            $("#inq_table").append(`<input type="hidden" id="quantity_hidden_${good_name}" value="${good_quantity}"> `);
        }
        else{
            $("#quantity_hidden_"+good_name).val($good_quantity)
        }
...
async function saveSelectedSeller()
{
    ...
    if($(chk).val()!==-1){
        ...
        $('#seller_detailes_box').html('');
        $("#add_seller_detailes_btn").show();
        await get_all_inq_sellers(inq_id);
    }
    else{
    ...
    if(count_th>2){
        $("#inq_table thead tr th").each(function(){
            if($(this).index()>1){
                ...
                <div class="detailes_box" style="min-height:300px;height:auto">
                     <button test="3" id="more_seller_data_${seller_id}" onclick="register_inq_detailes(this,'${seller_name}')" class='btn btn-success'>ثبت/ویرایش جزییات</button>
                </div>
            </td>`
            }
        })
...
function get_citeis(input,perfix=""){
    ...
    if(perfix=='last'){
        $("#last_city_origin_of_send").html(city_option);
    }
    if(perfix==""){
...
async function insert_select_sellers(seller_id,seller_name,ostan_id,city_id,phone){
    ...
    var option_privonce="";
    var province = p_objects['data'];
    var ostan_name=await get_ostan_city(ostan_id);
    var city_name=await get_ostan_city(city_id);
    for(p in province){
        option_privonce+=`<option value="${province[p].RowID}">${province[p].title}</option>`
    }
    ...
    //for(var i=0;i<selected_sellers.length;i++){
      ...
      // var index=seller_id;
        ...
        <div class="container">
            <div class="custom_row2 py-2">
                <h6 style="min-height:100%;height:100%" class="text-right "><span> نام تامین کننده:</span> <span>${seller_name}</span> </h6>
                <h6 class="text-right "><span>مبدا ارسال :</span>${ostan_name}  - <span>${city_name}</span></h6>
                 <h6 class="text-right "><span> تلفن :</span> <span>${phone}</span></h6>
            </div>
            <div class="row form-group">
        ...
        appended_tbody_td=`<td class="inq_detaile_td" style='min-width:300px;min-height:300px'>
                                <div class="detailes_box" style="min-height:300px;height:auto">
                                    <button test="4"class="btn btn-success more_seller_data" onclick="register_inq_detailes(this,'${seller_name}','${seller_id}','${seller_inq_RowID}')" class='btn btn-success'>ثبت/ویرایش جزییات</button>
...
function get_all_goods(good_id=0){
    var goods_params = {action:'inq_get_all_goods',good_id:good_id}
...
async function do_delete_seller_column(RowID,seller_id,inq_id){
 ...
 function getRandomColor() {
  ...
  async function toggle_form(quantity,detailes_id=0){
    ...
    var form_html=`<form id="inq_detailes_form" style="display:none">
                        ...
                        <div class="row form-group">
                            <div class="col-sm-6">
                                <label class="col-sm-6">ارائه فاکتور با ارزش افزوده</label>
                                <div class="col-sm-6">
                                        <label> دارد<input name="is_vat" onchange="set_ultimat_price(0,0)" type="radio" value="1"></label>
                                        <label> ندارد<input name="is_vat" type="radio" onchange="set_ultimat_price(0,0)"  value="-1"></label>
                                </div>
                            </div>
                            <div class="col-sm-6" >
                                <label class="col-sm-6">محاسبه کرایه حمل</label>
                                <div class="col-sm-6">
                                        <label> درصدی<input name="rent_percent" onchange="set_ultimat_price(0,0)" type="radio" value="1"></label>
                                        <label> عادی<input name="rent_percent" onchange="set_ultimat_price(0,0)" type="radio" value="-1"></label>
                                </div>
                            </div>
                        </div>
                        
                        <div class="row form-group" id="tax_risks_box">
                            <div class="col-sm-12">
                                <label class="col-sm-3">ریسک های مالیاتی</label>
                                <div class="col-sm-9">
                                    <select onchange="set_ultimat_price(0,0)" id="inq_tax_risks" class="risk_select" data-live-search="true">
                        ...
                        <div class="row form-group" id="last_tamin_risks_box">
                            <div class="col-sm-12">
                                <label class="col-sm-3">ریسک های تامین اجتماعی</label>
                                <div class="col-sm-9">
                                    <select onchange="set_ultimat_price(0,0)" id="inq_tamin_risks" class="risk_select"  data-live-search="true">
                        ...
                        <div class="row form-group">    
                            <div class="col-sm-6">
                                <label class="col-sm-6"> قیمت واحد (ریال)</label>
                                <div class="col-sm-6">
                                    <input class="form-control" onkeyup="set_separator(this)" type="text" id="unit_price">
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6" id="rent_div">
                                ...
                                <div class="col-sm-6">
                                    <input class="form-control" onchange="set_ultimat_price(0,0)" type="text" id="rent">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row form-group">
                            <div class="col-sm-6">
                                <label class="col-sm-6"> اعتبار قیمت (ساعت) </label>
                                <div class="col-sm-6">
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-sm-6">
                                ...
                                <label class="col-sm-6">قیمت تمام شده نهایی هر واحد(ریال)</label>
                                <div class="col-sm-6">
                                    <input onkeyup="set_separator(this)" class="form-control"  type="text" id="ultimate_price">
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <label class="col-sm-6"> توضیحات</label>
                        ...
                        <div class="ultimate_price"></div>
                        <button class="col-sm-12 btn btn-success" id="add_pay_method_btn" type="button" onclick="add_pay_method(${good_id},[],${quantity})" title="جزییات پرداخت">افزودن ردیف  پرداخت</button>
                         <div class="row form-group pay_type_box p-4">
                            
                        </div>
                        <div class="modal-footer">
                            <button type="button" onclick="save_seller_info(this,'${good_id}','${RowID}','${seller_id}','${summary_id}',${quantity},${detailes_id})" class="btn btn-primary" >تایید</button>
    ...
    if(detailes_id>0){
        var result=await get_inq_detailes_info(detailes_id)
      ...
      if(res.rent_percent==1){
        $("#rent").val(res.unit_price/res.ultimate_rent)
      }
      else{
      ...
      $("#inq_tamin_risks").val(res.tamin_risk);
      $("#inq_tax_risks").val(res.tax_risk);

    }
    $("#inq_detailes_form").show(1000);
    $("#toggle_form_btn").hide(1000);
    select_data_picker(".risk_select");
    $("input[name='is_vat']").on('click',function(){
    ...
    if(detailes_id>0){
        add_pay_method(good_id,res2,quantity)
    }
   
   // get_inq_pay_rows(res2)
    ...
    var set_int=setInterval(function(){
        if($("#inq_detailes_form").length){
            set_ultimat_price(0,0);
        }
        else{
  ...
  function get_inq_pay_rows(res){

    var tbody_row=`<table> `;
    for(k in res){
        var row_index=parseInt(k)+1
        tbody_row+=`<tr>
            <td>${row_index}</td>
            <td>
                <select id="pay_method_${row_index}">
                    <option value="0">روش پرداخت را انتخاب نمایید</option>
            ...
            <td>
                <select id="pay_type_${row_index}">
                    <option value="0">نوع پرداخت راانتخاب نمایید</option>
                    <option value="1">پیش پرداخت</option>
                    <option value="2"> علی الحساب</option>
                    <option value="3"> تسویه حساب</option>
                </select>
            </td>
            <td>
                <input type="text" class="pay_amount"  onkeyup="check_remind(this,${good_id})" value="${res[k]['pay_amount']}" id="pay_amount_${row_index}">
            </td>
            <td>
                <input type="text" onchange=set_date_diff(this) class="pay_date" value="${res[k]['pay_date']}" id="pay_date_${row_index}">
            </td>
            <td>
                <textarea style="height:30px;" id="pay_description_${row_index}">${res[k]['descriptiom']}</textarea>
            </td>
            <td>
                <button type="button" onclick="remove_pay_row(this)" class="btn btn-danger"><i class="fa fa-trash"></i></button>
            </td>
        </tr>`;
    }
    $(".pay_type_box").html(tbody_row+"</table>");
  }
  ...
  async function get_inq_good_detailes(inq_id,good_id){
    var action="get_inq_good_detailes";
    var params={action:action,inq_id:inq_id,good_id:good_id}
    var res=await manageAjaxRequestCustom(params)
    console.log('res',res);
    if(res.res=="true"){
...
async function register_inq_detailes(input,seller_name,seller_id,RowID=0){
    ...
    var inq_good_detailes=await get_inq_good_detailes(inq_id,good_id);
    
   // var good_name= $("#"+parent_id).attr('good_name');
    var good_name= inq_good_detailes['good_name'];
    var good_unit= inq_good_detailes['unit'];
    var good_quantity= inq_good_detailes['quantity'];
  ...
  //-------------------------------------------------------------
    ...
    if(sessionStorage.getItem('first')==1){
        ...
        var html=await show_summary_suggests(good_id,seller_id,RowID,1)
    ...
    else
    {
        ...
        <div id="seller_inq_detailes" class="modal custom_modal"  tabindex="-1" role="dialog">
            ...
            <div class="modal-dialog" style="max-width:60vw" role="document">
                ...
                <h5 class="modal-title row justify-content-between px-6"><span> نام طرف حساب :${seller_name}</span> <span>شرح کالا : ${good_name}</span> 
                <span> تعداد/مقدار درخواستی : ${good_quantity} ${good_unit}</span></h5>
        ...
        if(status==0){
            modal+=`<div class="row">
                        <button class="btn btn-success w-100" id="toggle_form_btn" onclick="toggle_form(${good_quantity})"> + </button>
                    </div>`
        }
                    
        modal+=`<div id="inq_detailes_box">
                </div>
                </div>
        ...
        var rows=await get_inq_seller_detailes(inq_id,good_id,seller_id,RowID);
...
async function save_seller_info (input,good_id="",RowID="",seller_id="",summary_id="",quantity,inq_detailes_id=0){
    ...
    var unit_price=numberformat2($("#unit_price").val());
    var pay_term=$("#pay_term").val();
    var price_valid=$("#price_valid").val();
    var ultimate_rent=0;
    var rent=numberformat2($("#rent").val());
    var good_id=$("#good_id").val();
    var seller_tbl_id=$("#seller_tbl_id").val();
    var ultimate_price=numberformat2($("#ultimate_price").val());
    var description=$("#description").val()?$("#description").val():'';
    var tamin_risk=$("#inq_tamin_risks").val();
    var tax_risk=$("#inq_tax_risks").val();
    if(rent_percent==1){
    ...
    var fillable=[is_vat,unit_price,pay_term,price_valid,ultimate_price]
    var fa_alias=[' وضعیت ارزش افزوده ',' قیمت واحد ','  مدت بازپرداخت',' اعتبار قیمت ',' قیمت نهایی ']
    for(k in fillable){
    ...
    $("#pay_method_box_tbl tbody tr").each(function(){
        var pay_type=$(this).find('select[id^="pay_type_"]').val()
        var pay_method=$(this).find('select[id^="pay_method_"]').val();
        var pay_amount=numberformat2($(this).find('input[id^="pay_amount_"]').val());
        var pay_date=$(this).find('input[id^="pay_date_"]').val();
        var pay_desc=$(this).find('textarea[id^="pay_description_"]').val();
        total_pay_amount=parseInt(total_pay_amount)+parseInt(pay_amount)
      
        if(parseInt(pay_type)==0){
            error_flag=1;
            return false;
        }
        if(parseInt(pay_method)==0){
            error_flag=2;
            return false;
        }
        if(parseInt(pay_amount)==0){
            error_flag=3;
            return false;
        }
        if(pay_date=="" || typeof(pay_date)=="undefined"){
            error_flag=4;
            return false;
        }
        var obj={};
        obj['pay_type'] = pay_type//$(this).find('select[id^="pay_type_"]').val();
        obj['pay_method'] = pay_method//$(this).find('select[id^="pay_method_"]').val();
        obj['pay_amount'] = pay_amount;
        obj['pay_date'] = pay_date;
        obj['pay_description'] = pay_desc;
        pay_method_array.push(obj);
    })
    if(error_flag==1){
        swal_custom('','روش پرداخت وارد نشده است','w')
        return false;
    }
    if(error_flag==2){
        swal_custom('','نوع پرداخت وارد نشده است','w')
        return false;
    }
    if(error_flag==3){
        swal_custom('','مبلغ پرداخت وارد نشده است','w')
        return false;
    }
    if(error_flag==4){
        swal_custom('','تاریخ پرداخت وارد نشده است','w')
        return false;
    }

    if(total_pay_amount!=parseInt(ultimate_price)*parseInt(quantity)){
        swal_custom('','مجموع ردیف پرداخت های ثبت شده با مبلغ کل قیمت تمام شده برابر نمی باشد','w');
        return false;
    }
    var params={
        action:action,
        ...
        inq_id:inq_id,
        tax_risk:tax_risk,
        tamin_risk:tamin_risk,
        pay_method_array:JSON.stringify(pay_method_array)
    }
    ...
    if(res['res']=='true'){
        ...
        await create_inq_seller_detailes_rows(rows);
        if(summary_id){
            var res=await show_summary_suggests(good_id,seller_id,RowID,1)
            $("#"+summary_id).html(res);
        }
...
async function get_inq_seller_detailes(inq_id,good_id,seller_id,RowID){
    var action="get_inq_seller_detailes";
    var params={action:action,good_id:good_id,seller_id:seller_id,RowID:RowID,inq_id:inq_id}
    var res=await manageAjaxRequestCustom(params);
...
function create_inq_seller_detailes_rows(res){
    ...
    for(k in res_array){
        tbody_content+=`
            <tr>
                <td>${parseInt(k)+1}</td>
                <td>${numberformat2(res_array[k]['unit_price'],1)} ریال</td>
                <td>${res_array[k]['pay_term']} روز</td>
                <td>${res_array[k]['price_valid']} ساعت</td>
                <td>${numberformat2(res_array[k]['ultimate_price'],1)} ریال</td>
                <td>${numberformat2(res_array[k]['ultimate_rent'],1)} ریال</td>
                <td>${res_array[k]['description']}</td>`
        if(status==0){
            tbody_content+=`<td>
                    <button class="btn btn-info" onclick="toggle_form(${res_array[k]['quantity']},${res_array[k]['RowID']})">
...
async function get_all_payComments(inq_id){
    var action = 'create_pay_comment_modal';
    var account_options=`<option value="-1">--------</option>`;
    var res = ajaxHandler({action:action,inq_id:inq_id});
...
function remove_pay_row(input){
    var table=$(input).parents('table');
    var tr=$(table).find('tbody tr').length
    if(tr==1){
        $(".pay_method_box").remove();
       
    }
    else{
        $(input).parents('tr').remove();
    }
    $("#add_pay_method_btn").show();
}

function create_inq_table_row(good_id,res_array=[],quantity){
  
    if(parseInt(res_array.length)==0)
    {
        var row_count = $("table#pay_method_box_tbl tbody tr").length??0;
        var row_index = parseInt(row_count)+1;
        var ultimate_price =  manage_number_format($("#ultimate_price").val())??0;
        var unit_price = manage_number_format($("#unit_price").val())??0;
        var assigned = 0;
        if(row_index>0){
            $("#pay_method_box_tbl tbody tr").each(function(){
                assigned=parseInt(assigned)+parseInt(manage_number_format($(this).find('input[id^="pay_amount_"]').val()))
            })

        }

        if(unit_price == 0){
            swal_custom('خطا','قیمت واحد وارد نشده است','w');
            return false;
        }
        if(ultimate_price == 0){
            swal_custom('خطا','قیمت تمام نشده نهایی  وارد نشده است','w');
            return false;
        }

    
        var total_price = parseInt(ultimate_price)*parseInt(quantity);
        var added_price = 0;
        if($("table#pay_method_box tbody tr .pay_amount").length){
            $("table#pay_method_box tbody tr .pay_amount").each(function(){
                added_price = parseInt(added_price)+parseInt(manage_number_format($(this).val()))
            })
        }

 
        var remind =row_count==0?total_price: parseInt(total_price)-parseInt(assigned);
        remind = manage_number_format(remind,1);
        var tbody_row=` 
        <tr>
            <td>${row_index}</td>
            <td>
            ...
            <td>
                <input type="text" class="pay_amount"  onkeyup="check_remind(this,${good_id})" value="${remind}" id="pay_amount_${row_index}">
            </td>
            <td>
                <input type="text" onchange=set_date_diff(this) class="pay_date" id="pay_date_${row_index}">
            </td>
            <td>
                <textarea style="height:30px;" id="pay_description_${row_index}"></textarea>
            </td>
            <td>
                <button type="button" onclick="remove_pay_row(this)" class="btn btn-danger"><i class="fa fa-trash"></i></button>
            </td>
        ...
        return {html_content:tbody_row,remind:remind,row_count:row_count}
    }
    else
    {
        ...
        { 
            ...
            var pay_date=res_array[k]['pay_date'];
            tbody_row+=`<tr>
                <td>${row_index}</td>
                <td>
                    <select selected="${res_array[k]['pay_method']}" id="pay_method_${row_index}">
                        <option value="0">روش پرداخت را انتخاب نمایید</option>
                        <option value="1">نقدی</option>
                        <option value="2"> چک</option>
                        <option value="3"> اعتباری</option>
                ...
                <td>
                    <select selected="${res_array[k]['pay_type']}"  id="pay_type_${row_index}">
                        <option value="0">نوع پرداخت راانتخاب نمایید</option>
                        <option value="1">پیش پرداخت</option>
                        <option value="2"> علی الحساب</option>
                        <option value="3"> تسویه حساب</option>
                ...
                <td>
                    <input type="text" class="pay_amount"  onkeyup="check_remind(this,${good_id})" value="${res_array[k]['pay_amount']}" id="pay_amount_${row_index}">
                </td>
                <td>
                    <input type="text" onchange=set_date_diff(this) class="pay_date" value="${pay_date}" id="pay_date_${row_index}">
                </td>
                <td>
                    <textarea style="height:30px;" id="pay_description_${row_index}">${res_array[k]['description']}</textarea>
                </td>
                <td>
                    <button type="button" onclick="remove_pay_row(this)" class="btn btn-danger"><i class="fa fa-trash"></i></button>
                </td>
        ...
        return {html_content:tbody_row,remind:0,row_count:row_index}
    }
}

function toEnglishNumber(strNum) {
   var num_arr=strNum.split('');
   var num=''
   for(k in num_arr){
     switch(num_arr[k]){
        case "٠"://arabic
   ...
   var num_array_text= num_arr.join();
   return parseInt(num_array_text.replaceAll(',',''))
...
function set_date_diff(input){
    var date_arr=($(input).val()).split("/");
    var y=toEnglishNumber(date_arr[0])
    var m=toEnglishNumber(date_arr[1])
    var d=toEnglishNumber(date_arr[2])
    var date2=farvardin.solarToGregorian(y , m, d)
    var date1=new Date();
    var date_diff_days=date_diff(date1,date2);
    // if(parseInt(date_diff_days)<0){
    //     $(input).val('')
    //     swal_custom('خطا','تاریخ سررسید پرداخت نمی تواند قبل از تاریخ روز جاری باشد','w');
    //     return false;
    // }
}

function add_pay_method(good_id,res_array=[],quantity){
    var arr=res_array;
    var tbody_row_obj=create_inq_table_row(good_id,arr,quantity)
    var tbody_row=tbody_row_obj.html_content
    var remind=manage_number_format(tbody_row_obj.remind);
    console.log('remind',remind);
    if(parseInt(res_array.length)==0 && remind==0){
        swal_custom('خطا',' مجموع ردیف پرداخت برابر  قیمت تمام شده کل  می باشد','w');
        return false;
    }
    console.log('res_array000:',res_array);
    var row_count=tbody_row_obj.row_count
    if(parseInt($(".pay_method_box #pay_method_box_tbl").length)==0){
        var html=`
            <div class="pay_method_box m-auto">
            <div class="row">
                <p> جزییات پرداخت</p>
            
            </div>
                <table border="1" id="pay_method_box_tbl" class="table-striped table-borderd">
                    <thead>
                        <tr>
            ...
            `;
            $(".pay_type_box").html(html+`<input type="hidden" id="last_remind" value=${remind}`);
            $("#pay_method_box_tbl tbody tr").each(function(){
                var pay_date=$(this).find('input[id^="pay_date_"]')
                
                var p_date_id=$(pay_date).attr('id');
                set_persian_date("#"+p_date_id,1);
              
            })
            for(k in res_array){
                var index=parseInt(k)+1
                $("#pay_method_"+index).val(res_array[k]['pay_method']);
                $("#pay_type_"+index).val(res_array[k]['pay_type']);
            }
           // set_persian_date("#pay_date_1",1);

    }
    else{
        
        $("#pay_method_box_tbl tbody").append(tbody_row);
        set_persian_date("#pay_date_"+parseInt(row_count+1),1);
    }
   
       
}

// function add_pay_method1(good_id){

//     var row_count=$("table#pay_method_box tbody tr").length??0;
//     var row_index=parseInt(row_count)+1;
//     var unit_price=manage_number_format($("#unit_price").val())??0;
//     if(unit_price==0){
//         swal_custom('خظا','قیمت واحد وارد نشده است','w');
//         return false;
//     }
//     console.log('unit_price',unit_price);
//     var quantity=manage_number_format($("#quantity_hidden_"+good_id).val());
//     var total_price=parseInt(unit_price)*parseInt(quantity);
//     var added_price=0;
//     if($("table#pay_method_box tbody tr .pay_amount").length){
//         $("table#pay_method_box tbody tr .pay_amount").each(function(){
//             added_price=parseInt(added_price)+parseInt(manage_number_format($(this).val()))
...
//         </td>
//         <td>
//             <input type="text" class="pay_date" id="pay_date_${row_index}">
//         </td>
//         <td>
//             <textarea id="pay_description_${row_index}"></textarea>
//         </td>
//         <td>
//             <button type="button" onclick="remove_pay_row(this)" class="btn btn-danger"><i class="fa fa-trash"></i></button>
//         </td>
...
//     var remind=parseInt(total_price)-parseInt(added_price);

//     if(remind==0){
//         swal_custom('خطا','مجموع مقادیر پرداخت  وارد شده است','w');
//         $("#add_pay_method_btn").hide();
//         return false;
//     }
//     else{
//         $("#add_pay_method_btn").show();
...
//         </td>
//     </tr>`;
//     if($("table#pay_method_box").length){
//         $("table#pay_method_box tbody").append(tbody_row);
//     }
//     else
//     {
//         var html=`
...
//         </div>
//             <table border="1" id="pay_method_box" class="table table-striped table-borderd mt-1">
//                 <thead>
//                     <tr>
//                         <td class="tbl_header">ردیف</td>
//                         <td class="tbl_header">روش پرداخت</td>
...
//     set_persian_date("#pay_date_"+row_index)
        
// }

function check_remind( input,good_id){
    set_separator(input)
    var pay_amount=manage_number_format($(input).val());
    //var row_count=$("table#pay_method_box tbody tr").length??0;
    if($(input).val()==0){
    ...
    var remind=parseInt(total_price)-parseInt(added_price);
    console.log('remind',remind);
    if(parseInt(pay_amount)>parseInt(remind)){
        $(input).val('');
        return false;
    }
}
...
async function get_inq_attachment_list(group_code,inq_id){
    ...
    var params={action:action,good_id:group_code,inq_id:inq_id}
    var res= await manageAjaxRequestCustom(params);
...
function display_inq_attachments(group_code,inq_id){
    ...
    $('body').append(`
<div class="modal fade" id="inq_attachment_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="background:rgba(128,128,128,0.5)">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-footer">
        ...
        <button type="button" type="button" class="btn btn-secondary" data-dismiss="modal">انصراف</button>
        </div>
      </div>
      <div id="inq_modal_grid"></div>
    </div>
...
async function uploadFile(inq_id,file_input_id,file_title) {
    let good_id=$("#inq_good").val()
    let formData = new FormData(); 
    let file_input=document.getElementById(file_input_id)   
    if(inq_id=="" || inq_id==null ){
        swal_custom('','شناسه استعلام دریافت نشده است','w'); 
        return false;
    }
    if($(file_input).val() == '' || $(file_input)[0].files.length == 0){
        swal_custom('','فایل انتخاب نشده است ','w'); 
        return false;
    }
    if(file_title=="" ||file_title=="" ){
        swal_custom('','عنوان فایل  دریافت نشده است','w'); 
        return false;
    }
    if(good_id=="" ||good_id==0 ){
        swal_custom('','  کالایی جهت استعلام  انتخاب نشده است','w'); 
        return false;
    }
    
    formData.append("file", file_input.files[0]);
    formData.append("inq_id",inq_id);
    formData.append("good_id",good_id);
    formData.append("file_title",file_title);
    formData.append("action",'attach_inq_file');
    try{
        ...
        if (!response.ok) {
            ...
            swal_custom('','فایل با موفقیت  بارگزاری شد','s'); 
            get_inq_attachment_list(good_id,inq_id);
           $(file_input).val('')
        }
    }
    catch(error){
...
//  function attach_inq_file(inq_id,file_input_id,file_title ){
...
//     xhr.open('POST', 'php/inq_manage_proccess.php',true);
//     xhr.onload = function () {
//         if (xhr.status === 200) {
              ...
              <div class="modal-dialog modal-lg" role="document" style="max-width:1024px" >
                <div class="modal-content">
                  ...
                  <div id="send_form_body" >
                        <div class="modal-body inq_send_body">
                            <form>
                           <div class="form-group row">
                                ...
                                <div class="form-group row" >
                                    <label for="inq_select_users" class="col-sm-2 col-form-label">توضیحات</label>
                                    <div class="col-sm-10 p-0">
                                        <textarea id="inq_send_comment" style="min-height:7rem" class="form-control"></textarea>
                                    </div>
                    ...
                    <div class="modal-footer">
                            <button style="" type="button"  id="send_inquiry" onclick="do_send_inquiry(${tab_status})" class="btn btn-primary">ارسال </button>
...
async function do_send_inquiry(status,selected_suggest){
    var action = "do_send_inquiry";
    var final_confirm=1;
    var inq_id = $("#inq_id_hidden").val();
    var reciever = $("#inq_select_users").val();
    var desc = $("#inq_send_comment").val();
    if(status!=1){

        if(reciever==0){
            swal_custom('خطا',' گیرنده را انتخاب نمایید','w');
            return false;
        }
        final_confirm=0;
    }

    var params={action:action,reciever:reciever,inq_id:inq_id,desc:desc,selected_suggest:selected_suggest}
    var res=manageAjaxRequestCustom(params)
    menu_item_display(goToPurchaseInquiry,'PurchaseInquiry')
    close_modal('send_modal_box')
}
...
async function inqiury_print(inq_id){

    var print_paraf=1;
    var paraf_conf=await swal_custom('print',"نسخه خروجی به همراه پاراف ها  چاپ شود ؟",'c');
    if(!paraf_conf){
        print_paraf=0
    }
    var action="inqiury_print"
    var params={action:action,inq_id:inq_id,print_paraf:print_paraf}
    var res =await manageAjaxRequestCustom(params)
    if($(".print_inq")){
        $(".print_inq").remove();
