...
async function add_row_inq_table(added_goods,edit=0,inq_id){
   ...
   if(edit==0){
    var seller_td="";
        ...
        return `<tr style="border-bottom:4px solid gray" good_quantity="${added_goods['quantity']}" good_id="${added_goods['RowID']}" good_name="${added_goods['good_name']}" class="good_detailes" id="good_detailes_${good_id}">
        <td >
            ${td_content}
...
async  function last_inq_data(good_id,inq_id){
    ...
    if(res.res=='true'){
        ...
        if(Object.keys(res['data']).length){
            html+=`<table class="table bg-light">
                    <tr>
                        <td>فروشنده</td>
                        <td>${res['data']['Name']}</td>
                    </tr>
...
function set_ultimat_price(return_value=0,last=1){
    ...
    // var tamin_risk=parseFloat(unit_price*$("#last_tamin_risks").val()/100)??0
        var last_rent=$("#last_rent").val()?$("#last_rent").val().replaceAll(',',''):0
        if(rent_type==1){
            last_rent=parseFloat(unit_price*last_rent/100)??0
        }
        if(vat==-1){
            var tax_risk=parseFloat(unit_price)*($("#last_tax_risks").val())/100??0
        }
        var tamin_risk=unit_price*$("#last_tamin_risks").val()/100??0
...
async function show_summary_suggests(good_id,seller_id,RowID){
    ...
    var rand= create_random_code();
    if(res.data.length){
        htm+=`<table  border="1" class="bg-light table table-borderd"><tr><!--<td>انتخاب</td>--><td>قیمت واحد</td><td>مدت بازپرداخت</td><td>اعتبار قیمت</td><td>قیمت نهایی</td><td>توضیحات</td></tr>`
        for(k in res.data){
            htm+=`<tr>
                        <td>
...
async function do_delete_seller_column(RowID,seller_id,inq_id){
 ...
 function getRandomColor() {
  ...
  async function toggle_form(detailes_id=0){
    ...
    //-----------------------------------------------
    // var form_html=`<div class="modal" tabindex="-1" role="dialog" id="last_inq_modal">
    //             <div class="modal-dialog" role="document" style="max-width:950px">
    //                 <div class="modal-content">
    //                 <div class="modal-header text-light bg-primary">
    //                     <h5 class="modal-title">ثبت آخرین خرید</h5>
    ...
    var form_html=`<form id="inq_detailes_form" style="display:none">
                        ...
                        <div class="row form-group" id="tax_risks_box">
                            <div class="col-sm-12">
                                <label class="col-sm-3">ریسک های مالیاتی</label>
                                <div class="col-sm-9">
                                    <select onchange="set_ultimat_price(0,0)" id="last_tax_risks" class="risk_select" multiple data-live-search="true">
                        ...
                        <div class="row form-group" id="last_tamin_risks_box">
                            <div class="col-sm-12">
                                <label class="col-sm-3">ریسک های تامین اجتماعی</label>
                                <div class="col-sm-9">
                                    <select onchange="set_ultimat_price(0,0)" id="last_tamin_risks" class="risk_select" multiple data-live-search="true">
                        ...
                        <div class="row form-group">    
                            ...
                            <div class="col-sm-6">
                                ...
                                <div class="col-sm-6 row">
                                        ...
                                        <input class="col-sm-10 form-control"  type="text" id="pay_term">
                                        <button class="col-sm-2 btn btn-success" id="add_pay_method_btn" type="button" onclick="add_pay_method(${good_id})"  alt="جزییات پرداخت">+</button>
                                </div>-->
                                <label class="col-sm-6">قیمت تمام شده نهایی(ریال)</label>
                                <div class="col-sm-6">
                        ...
                        <button class="col-sm-12 btn btn-success" id="add_pay_method_btn" type="button" onclick="add_pay_method(${good_id})" title="جزییات پرداخت">مدیریت جزییات پرداخت</button>
...
function create_inq_table_row(good_id){
    ...
    if(row_index>0){
        for(var i=1;i<row_index;i++){
            assigned=parseInt(assigned)+parseInt($("#pay_amount_"+i).val())
        }
    }
    if(unit_price == 0){
...
function add_pay_method(good_id){
    ...
    var remind=tbody_row_obj.remind
...
//     var selection_count=inq_detailes.length
...
//                     <button type="button" class="close"  data-dismiss="modal" aria-label="Close" onclick="close_modal('send_modal_box')">
...
//                         <div class="modal-footer">
//                             <button style="" type="button"  id="send_inquiry" onclick="do_send_inquiry(${selection_count},${tab_status})" class="btn btn-primary">ارسال </button>
                ...
                <div class="modal-content">
                  ...
                  <div id="send_form_body" >
                       ...
                       // }
                        ...
                        <div class="modal-footer">
                            <button style="" type="button"  id="send_inquiry" onclick="do_send_inquiry(${selection_count})" class="btn btn-primary">ارسال </button>
                            ${final_confirm_btn}
                            <button style="" type="button "  id="close_inquiry_modal" class="btn btn-secondary" >بستن</button>
                        </div>
                    </div>
