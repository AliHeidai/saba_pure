   ...
   if(edit==0){
    var seller_td="";
        ...
        <td id="last_inq_result_${good_id}">
            <div>
            ${last_inq_html}
            <input type="hidden" value="${added_goods['quantity']}" id="quantity_hidden_${added_goods['RowID']}">
            </div>
        </td>
...
function toggle_add_last_inq(input,good_id){
...
//    console.log('footer',footer);
   //-----------------------------------------------
    ...
    var add_good_btn=`<button id="add_good_row" onclick="add_good_inq(this)" class="btn btn-success mx-2 col-md-1">انتخاب  کالا</button>`
    var add_seller_btn=` <button ${disable} onclick="add_seller()" id="add_seller_btn"  title="انتخاب تامین کنندگان " class="btn btn-info col-md-1 mx-2">انتخاب تامین کنندگان </button>`
    ...
    var send_inq_btn=`<button ${disable} onclick="send_inq(${id},${status})" id="send_inq_btn"  title="گردش کار/ارسال" class="btn btn-primary col-md-1 mx-2">گردش کار/ارسال</button>`
    ...
    var inq_create_pay_comment_btn=`<button  onclick="get_all_payComments(${id})" id="archive_inq_btn"  title="ثبت اظهارنظر " class="btn btn-warning col-md-1 mx-2">ثبت اظهارنظر </button>`
     var inq_create_inq_paraf_btn=`<button  onclick="inq_create_paraf(${id})" id="paraf_inq_btn"  title="مدیریت پاراف" class="btn btn-warning col-md-1 mx-2">مدیریت پاراف </button>`
    var modal=`
    <div id="inq_detailes" >
    ...
    else
    {
        ...
        if(status==2 || status==3 || status==20){
             modal+=`${send_inq_btn}`
             modal+=`${inq_create_inq_paraf_btn}`
             if(status==20){
...
//------------------------------------------------
    ...
    $("div[id^='seller_summary_']").each(async function(){
        ...
        var htm= await show_summary_suggests(good_id,seller_id,RowID,1)
       $(this).html(htm)
    })
...
async function show_summary_suggests(good_id,seller_id,RowID,summary_type=0){
...
function check_inserted_duplicate_good(input){
 ...
 function edit_create_good_row(input,good_id,inq_id){
    ...
    toggle_inq_file(good_rows[0]['inq_base'])
...
async function save_good_row(RowID=0){
    ...
    if(inq_base==1 || inq_base==2){
        if(inq_number="" || inq_number==null || inq_number.trim()=="" ){
 ...
 function getRandomColor() {
  ...
  async function toggle_form(detailes_id=0){
    ...
    var form_html=`<form id="inq_detailes_form" style="display:none">
                        ...
                        <button class="col-sm-12 btn btn-success" id="add_pay_method_btn" type="button" onclick="add_pay_method(${good_id})" title="جزییات پرداخت">افزودن ردیف  پرداخت</button>
...
function create_inq_table_row(good_id){
    ...
    if(row_index>0){
        for(var i=1;i<row_index;i++){
            assigned=parseInt(assigned)+parseInt(manage_number_format($("#pay_amount_"+i).val()))
        }
    }
    ...
    var remind=parseInt(quantity*ultimate_price)-parseInt(assigned);
...
function add_pay_method(good_id){
    var tbody_row_obj=create_inq_table_row(good_id)
...
async function send_inq(row_id,tab_status) {
    ...
    // var inq_detailes = res.data.inq_detailes;
    // var selection_count=inq_detailes.length??0
    
    var grid_html=res.data.grid_html;
    var final_confirm_status=res.data.show_confirm_btn
    var final_confirm_btn="";
    if(final_confirm_status==1){
        //final_confirm_btn=`<button class="btn btn-info" onclick="do_send_inquiry(${selection_count},1)">تایید نهایی و توقف استعلام </button>`
    }
    var options = "<option value='0'>انتخاب نمایید</option>";
    for (k in users) {
        options += `<option value="${users[k]['RowID']}">${users[k]['fname'] + " " + users[k]['lname']}</option>`
    }
    // for(i in inq_detailes)
    // {
    //     comment_html+=`<fieldset style="border: 2px solid rgba(128,128,128,0.3);padding: 10px;border-radius: 10px;"><legend style="width: auto;font-size: 16px;padding: 10px;color: gray;"> ${inq_detailes[i]['good_name']}</legend>`
    //     var inq=inq_detailes[i]['inq'];
    //     var inq_tbl=`<table class="table table-borderd table-striped">
    //                     <tr>
    //                         <th>ردیف</th>
    //                         <th>انتخاب</th>
    //                         <th>نام طرف حساب</th>
    ...
    //     for(t in inq)
    //     {
    //         var index=inq[t]['RowID']
    //         comment_html+=` <input type="hidden" id="row_id_${index}" value="${index}"/>
    //                         <input type="hidden" id="seller_name_${index}" value="${inq[t]['Name']}"/>
    //                         <input type="hidden" id="good_name_${index}" value="${inq_detailes[i]['good_name']}"/>
    //                         <input type="hidden" id="is_vat_${index}" value="${inq[t]['is_vat']}"/>
    //                         <input type="hidden" id="pay_term_${index}" value="${inq[t]['pay_term']}"/>
    //                         <input type="hidden" id="price_valid_${index}" value="${inq[t]['price_valid']}"/>
    //                         <input type="hidden" id="ultimate_price_${index}" value="${inq[t]['ultimate_price']}"/>
    //                         <input type="hidden" id="description_${index}" value="${inq[t]['description']}"/>
    //                         `
           
    //         var is_vat_sign=inq[t]['is_vat']==1?'<i class="fa fa-check text-success" aria-hidden="true"></i>':'<i class="fa fa-close text-danger" aria-hidden="true"></i>'
    //         inq_tbl+=`<tr>
    //                         <td>${counter}</td>
    //                         <td><input type="radio" row_index="${index}" good_id="${inq_detailes[i]['good_id']}" name="select_${inq_detailes[i]['good_id']}"></td>
    //                         <td>${inq[t]['Name']}"</td>
    //                         <td> 
    //                             ${is_vat_sign}
                                
    //                         </td>
    //                         <td>${inq[t]['pay_term']}</td>
    //                         <td>${inq[t]['price_valid']}</td>
    //                         <td>${inq[t]['ultimate_price']}</td>
    //                         <td>${inq[t]['description']} </td>
    //                     </tr>`
    //                     counter++;
    //     }
    //     inq_tbl+=`</table>`
    //     comment_html+=`${inq_tbl}<div class="form-group row">
    //         <label class="col-md-2">توضیحات    </label>
    //         <div class="col-md-10">
    //             <textarea class="form-control" id="more_description_${inq_detailes[i]['good_id']}" ></textarea>
    //         </div>
    //     </div>
    //     </fieldset>`
    // }
              ...
              <div class="modal-dialog modal-lg" role="document" style="max-width:1024px" >
                <div class="modal-content">
                  ...
                  <div id="send_form_body" >
                    ...
                    <div class="modal-footer">
                            <button style="" type="button"  id="send_inquiry" onclick="do_send_inquiry()" class="btn btn-primary">ارسال </button>
                            ${final_confirm_btn}
...
async function do_send_inquiry(section_count,status){
    var action = "do_send_inquiry";
    var final_confirm=1;
    var inq_id = $("#inq_id_hidden").val();
    var reciever = $("#inq_select_users").val();
    var selected_inqs = $("#send_modal_box").find("input[type='radio']:checked")
    var selected_inqs_length = $("#send_modal_box").find("input[type='radio']:checked").length;
    if(selected_inqs_length < section_count){
        swal_custom('خطا','از هر کالا یک ردیف استعلام باید انتخاب شود','w');
        return false;
    }
    if(status!=1){
    ...
    var comment_array=[];
    $(selected_inqs).each(function(){
        c_obj={}
        var index=$(this).attr('row_index');
        var good_id=$(this).attr('good_id')
        c_obj=
        {
            row_id:index,
            is_vat:$("#is_vat_"+index).val(),
            pay_term:$("#pay_term_"+index).val(),
            price_valid:$("#price_valid_"+index).val(),
            ultimate_price:$("#ultimate_price_"+index).val(),
            description:$("#description_"+index).val(),
            description:$("#description_"+index).val(),
            good_name:$("#good_name_"+index).val(),
            more_description:$("#more_description_"+good_id).val(),
            good_id:good_id,
            final_confirm:final_confirm
        }
        comment_array.push(c_obj);
    })
    var params={action:action,reciever:reciever,inq_id:inq_id,inq_detailes:JSON.stringify(comment_array)}
    var res=manageAjaxRequestCustom(params)
   // send_inq(inq_id,status)
  //  get_inquires($("#ready_inqs-tab"),1,'temp_inq_detailes')
