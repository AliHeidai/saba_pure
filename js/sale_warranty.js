...
function select_warranty_row(input){
    var checked = $(input).prop('checked');
    var row_id = $(input).attr('rid');
    $(input).parents('table').find('.warrenty_select_row[type="checkbox"]').each(function(){
...
function editWarranty() {
    ...
    else if(res['status']==1 && res['has_access_manage']==2)
        {
            $('#warrantyManageModal').modal('show');
            $("#warrantyManageHiddenCid").val(res['cid']);
            $("#warrantyManageTitle").val(res['warranty_title']);
            ...
            if(res['status']==1){
                var msg=" سند تایید نهایی شده است و قابل ویرایش نمی باشد"
            }
            else if(res['status']==3){
                var msg="مدت تضمین به پایان رسیده و سند عودت داده شده است"
            }
            else if(res['status']==2){
                var msg="'سند بایگانی  شده است و شما قادر به ویرایش آن نمی باشید'";
            }
            Swal.fire({
                icon: "warning",
                title: "خطا",
        ...
        if(!$("#display_sale_warranty_modal").length)
        {
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                ...
                <div class="modal-header bg-success text-light">
                    <h5 class="modal-title" id="exampleModalLongTitle">تضامین فروش </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span class="text-danger" aria-hidden="true">X</span>
                    </button>
                </div>
...
function doEditCreateWarranty() {
    ...
    var param = {action:action,w_title:w_title,w_id:w_id,w_acc_type:w_acc_type,w_doc_owner:w_doc_owner,w_doc_owner_nat_id:w_doc_owner_nat_id,w_doc_info:w_doc_info,w_access_id:accID,w_start_date:w_start_date,
                    w_end_date:w_end_date,desc:desc,w_account_id:w_account_id[0],w_type:w_type,warranty_cost:warranty_cost};
    var res = manageAjaxRequest(param);
    if (res != false) {
