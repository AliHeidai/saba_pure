...
async function editcreatePropertyManage(id = 0) {
  ...
  console.log('p_all_info',p_all_info);
  ...
  if (p_all_info) {
    ...
    if(p_row.damage_desc){
      $("#damage_desc").val(p_row.damage_desc);
    }
    if(p_row.tool_status==0 ){
      $("#damage_desc_div").show();
    }
    else{
      $("#damage_desc_div").hide();
    }
    if (p_row.transaction_type) {
      $("#transaction_type").val(p_row.transaction_type);
    }
    if (p_row.prop_num) {
...
async function editcreatePropertyTools(id = 0) {
  ut.showModal({
    title: "ایجاد /ویرایش ابزار/ اموال",
    body: await editCreatePrpertyToolsFrom(),
  });
  var p_all_info = await get_property_tools(id);
  console.log('p_all_info',p_all_info);

  if (p_all_info) {
    var p_row = p_all_info;
    if (p_row.tool_name) {
      $("#tool_name").val(p_row.tool_name);
    }
    if (p_row.status) {
      $("#tool_status").val(p_row.status);
    }
    if(p_row.description){
      $("#damage_desc").val(p_row.description);
    }
    if(p_row.status==0 ){
      $("#damage_desc_div").show();
    }
    ...
    if (p_row.property_num) {
      $("#property_num").val(p_row.property_num);
    }
    if (p_row.last_receiver) {
      $("#t_receiver").val(p_row.last_receiver);
    }
  
    $("#t_id").val(p_row.RowID);
  }
 
  select_data_picker("#t_receiver");
...
async function get_property_tools(id){
  var action = "get_property_tools";
  var param = { action: action, id: id };
  var res = await manageAjaxRequestCustom(param);
  return res.data;
}

async function get_tools_status_list(){
  var action = "get_tools_status_list";
  var param = { action: action };
  var res = await manageAjaxRequestCustom(param);
  return res.data;
}
...
async function editCreatePrpertyToolsFrom(values = {}) {
    ...
    // var transaction_type = await get_transaction_type_list();
    // console.log('transaction_type',transaction_type);
    // var tt_op = `<option value='-1'>نوع تراکنش را انتخاب نمایید</option>`;
    // for (var k in transaction_type) {
    //   var tt_op_status = transaction_type[k];
    //   tt_op += `<option value="${tt_op_status["value"]}">${tt_op_status["desc"]}</option>`;
    // }
  
    var tool_status = await get_tools_status_list();
    ...
    var per_op_deliver = `<option value='-1'> آخرین تحویل  گیرنده را انتخاب نمایید</option>`;
   // var per_op_receiver = `<option value='-1'> تحویل دهنده را انتخاب نمایید</option>`;
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-8">
        <input type="text" id="tool_name" class="form-control">
    ...
    <div class="form-group row">
      <label for="receiver" class="col-sm-4 col-form-label">آخرین تحویل  گیرنده.</label>
      <div class="col-sm-8">
        <select class="form-control" id="t_receiver">
        ${per_op_deliver}
     ...
     <div class="form-group row">
      ...
      <div class="col-sm-8">
        <select class="form-control" onchange="manage_tools_status(this.value)" id="tool_status">${status_op}</select>
      </div>
    </div>
    <div id="damage_desc_div" class="form-group row" style="display:none">
      <label for="receiver" class="col-sm-4 col-form-label">شرح آسیب های وارده</label>
      <div class="col-sm-8">
        <textarea class="form-control" id="damage_desc" name="damage_desc" placeholder="شرح آسیب های وارده"></textarea>
      </div>
    </div>
    ...
    <!--<div class="form-group row">
      ...
      <div class="col-sm-8">
       ...
       <input type="hidden" value="0" id="t_id">
      <button type="button" class="btn btn-primary" id="doEditCreatePropertyTools_btn" onclick="doEditCreatePropertyTools()">تایید</button>
...
async function editCreatePrpertyManageFrom(values = {}) {
  ...
  var tool_status = await get_tools_status_list();
  console.log('tool_status',tool_status);
  var status_op = `<option value='-1'> وضعیت ابزار /اموال را انتخاب نمایید</option>`;
  for (var k in tool_status) {
    var status = tool_status[k];
    status_op += `<option value="${status["value"]}">${status["desc"]}</option>`;
  ...
  <div class="form-group row">
    <label for="receiver" class="col-sm-4 col-form-label"> نوع تراکنش</label>
    <div class="col-sm-8">
      <select class="form-control" id="transaction_type">${tt_op}</select>
    </div>
  </div>
...
function manage_tools_status(status){
  if(status==0){
    $("#damage_desc_div").show();
  }
  else{
...
async function doEditCreatePropertyManage() {
  ...
  var desc = $("#desc").val();
  var damage_desc = $("#damage_desc").val();
  var pid= $("#p_id").val()
  ...
  if(tool_status==0){
    if(damage_desc.trim() == null || damage_desc.trim() == ""){
      notice1Sec("شرح آسیب های وارده را وارد نمایید", "yellow");
      return false;
    }
  }
  if(tool_status==-2 && transaction_type!=5){
    notice1Sec(`
        در صورتی که وضعیت ابزار  کالای فروش رفته انتخاب گردد 
      نوع تراکنش باید حتما فروش باشد 
      `, "yellow",5000);
    return false;
  }
  ...
  if($("#transaction_type").val() == -1){
    notice1Sec("نوع تراکنش را انتخاب نمایید", "yellow");
    return false;
  }else{
    ...
    if(transaction_type == 5){
      if(parseInt(tool_status)!=-1) {
        notice1Sec("در صورتی که نوع تراکنش فروش باشد  در قسمت وضعیت محصول باید حتما کالای فروش رفته انتخاب گردد", "yellow",5000);
        return false;
      }
    }
  }
  ...
  formData.append("damage_desc", damage_desc);
...
async function doEditCreatePropertyTools() {
  var action = "doEditCreatePropertyTools";
  var prop_num = $("#property_num").val();
  var t_receiver = $("#t_receiver").val();
  ...
  formData.append("action", action);
  formData.append("prop_num", prop_num);
  formData.append("t_receiver", t_receiver);
...
async function showPropertyManageList(page = 1) {
  ...
  var res = await manageAjaxRequestCustom(formData);
  var selector='PropertyManageBody';
  add_property_tab(selector);
  $("#transactions").html(res.data);
  select_data_picker("#tools_id_s");
  select_data_picker("#receiver_s");
  select_data_picker("#deliver_s");
 
  set_persian_date_new("#fdate_s");
  set_persian_date_new("#edate_s");
...
async function get_tools_info(tool_id) {
  ...
  var res = await manageAjaxRequestCustom(param);
  if (res.res == "true") {
    $("#deliver").val(res.data.last_receiver);
    $("#deliver").attr("disabled", true);
    $("#property_num").val(res.data.property_num);
    select_data_picker("#deliver");
  }
}

async function get_transaction_type_list() {
  var action = "get_transaction_type_list";
  var param = { action: action };
  var res = await manageAjaxRequestCustom(param);
  return res.data;
}
...
function add_property_tab(selector, activeDefault = 0) {
  var contents = "";
  var home_active = "";
  var send_active = "";
  var temp_conf = "";
  var archive_active = "";
  if (activeDefault == 0) {
    contents = $("#" + selector).html();
    home_active = "active";
  }
  if (activeDefault == 1) {
    contents = `<p> مدیریت تراکنش ها</p>`;
    send_active = "active";
  }
  if (activeDefault == 2) {
    contents = `<p> مدیریت ابزار/اموال</p>`;
    temp_conf = "active";
  }
  if (activeDefault == 3) {
    contents = `<p>گزارشات</p>`;
    archive_active = "active";
  }

  var tabs = `
<ul class="nav nav-tabs property_menu">
  <li class="nav-item">
    <a class="recived_tab nav-link ${home_active}" href="#" onclick="showPropertyManageList()"> مدیریت تراکنش ها</a>
  </li>
  <li class="nav-item">
    <a id="tools_manage_link" class="recived_tab nav-link ${send_active}" href="#" onclick="showPropertyToolsList()"> مدیریت ابزار/اموال</a>
  </li>
   <!--<li class="nav-item">
    <a class="recived_tab nav-link ${send_active}" href="#" onclick="get_received_tabs(this,'temp_conf')">اطلاعات اولیه</a>
  </li>
  <li class="nav-item">
    <a class="recived_tab nav-link ${send_active}" href="#" onclick="get_received_tabs(this,'archived')"> گزارشات</a>
  </li>-->
</ul>
<section class="trans_manage" id="transactions" style="padding:1rem;background:#fff" >
  ${contents}
</section>
<!--<section class="trans_manage" id="tools" style="padding:1rem;background:#fff" >
  
</section>
<section class="trans_manage" id="archived" style="padding:1rem;background:#fff" >
 
</section>
<section class="trans_manage" id="temp_conf" style="padding:1rem;background:#fff" >-->
 
</section>
`;
  $("#PropertyManageBody-body").html(tabs);
}

async function showPropertyToolsList(page=1){
  
  add_loading('showPropertyToolsList');
  var action = "showPropertyToolsList";
  var form = document.getElementById("frm_search_tools");
  if (form && form.length) {
    var formData = new FormData(form);
  } else {
    var formData = new FormData();
  ...
  select_data_picker("#receiver");
  // select_data_picker("#deliver");
 
  // set_persian_date_new("#fdate");
  // set_persian_date_new("#edate");
  remove_loading('showPropertyToolsList');

  
  $(".property_menu li a").removeClass('active');
  $("#tools_manage_link").addClass('active');
}
