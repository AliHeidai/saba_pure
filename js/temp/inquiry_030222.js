...
// async function manageAjaxRequest(formData,loading=0,atype,url){
...
function get_inquires(status,selector,page=1,other_params={}){
  ...
  //  var res=manageAjaxRequest(param);
    ...
    $(selector).html(res);
    if(status==0){
        var btn_create_array = {'type':'button',"class":"btn btn-primary inqury_btn",'parent':"1", "title":"استعلام جدید","onclick":"create_inquiry(this)", 'id':"create_new_inq_btn",'style':'float:left;position:relative;right:90%'}
        var paragraph_array = {'type':'txt_info',"class":"", "text":"لیست استعلام های درحال ثبت", 'id':"export_inq_header",'style':'float:right'}
        create_form_element(".inq_list_header",btn_create_array);
        create_form_element(".inq_list_header",paragraph_array);

    }
    var inq_from_date_search = new AMIB.persianCalendar('inq_from_date_search');
    var inq_to_date_search = new AMIB.persianCalendar('inq_to_date_search');
}
...
async function  create_inquiry(input,id=0){
    ...
    var inq_table_header_step2=[[" کد استعلام","<span id='inq_uncode_disable'></span>","تاریخ استعلام ","<span id=\"inq_date_disable_td\"></span>","کد درخواست خرید","<span id=\"inq_buy_code_disable_td\"></span>"]] ;
    //-------------------------------------input for section 2-----------------------------------------------------------
...
async function add_inq_row_good(input,inq_id=0){
    ...
    if(sessionStorage.getItem('inq_id')>0) {
        ...
        var res_goods =await manageAjaxRequestCustom(params);
       

        // if(res_goods.res=="true"){
        //     add_good_display_btn_flag=1;
        // }
    ...
    var add_good_display_btn_flag=1;
    $("input[id^='inq_good_name_']").each(function(){
        if($(this).val()==''){
            add_good_display_btn_flag=0;
        }
    })
    if(add_good_display_btn_flag==0){
        $("#add_good").hide();
    }
    else{
        $("#add_good").show(); 
...
async function add_datalist(input){
    ...
    var final_html =
        `
        <div class="form-group p-2"><input style="background: #FFFFE0"  target="${id}_list" id="search_bar" onkeyup="search_goods(this)" class="form-control" placeholder="search.."></div>
        <hr>
...
async function search_goods(input){
    ...
    var all_rows="";
    for(k in res.data){
        all_rows+=`<tr> <td>${res.data[k]['title']}</td></tr>`
    }
        ...
        $('body').append(`
                ...
                <div class="modal-dialog" role="document">
                    ...
                    <div style="display: flex;justify-content: space-between; width:100%;padding: 10px;">
                     <button style="width: 80px" type="button"   id="set_inq_comment" onclick="function_test()" class="btn btn-primary">تایید</button>
 ...
 async function function_test ()
{
    ...
    $("#inq_comment").modal('hide');
    console.log(selected_radio_inq);
    console.log($(input));
...
async function add_seller_buyer_save(input,seller_form_index,good_form_index,inq_id=0){
    ...
    if($("vat_status_"+seller_form_index).val()==1){
        ...
        var t2=numberformat2(export_cash_section)+numberformat2(export_term_payment);
        if(t1!=t2){
...
async function open_send_modal(row_id,tab_status) {
    ...
    var action = "get_all_users_access_inq";
    var res = await manageAjaxRequestCustom({action: action,row_id:row_id})
    // if(res.res=="false") {
    //     custom_alerts(res.data,'e',0,'عدم ارجاع');
    //     return false;
    // }
    var users = res.data;
    ...
    {
        $('body').append(`
            <div class="modal fade" id="send_modal_box" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
              <div class="modal-dialog modal-lg" role="document" >
                ...
                <form>
                      ...
                      <div class="form-group row">
                         <label for="inq_send_comment" class="col-sm-2 col-form-label">توضیحات</label>
                            <div class="col-sm-10">
                                <textarea  class="form-control" id="inq_send_comment" cols="50" rows="5"></textarea>
                            </div>
                        </div>
                        <div>
                            <div class="form-group row p-4">
                                <div class="col-sm-12">
                                     <button style="width: 100%" type="button"  id="send_inquiry" class="btn btn-primary">ارسال </button>
                                </div>
                            </div>
                        </div>
                    </form>
                  ...
                  <div class="inq_send_data_grid"></div>
                  <div class="modal-footer">
                    <button style="width: 100%" type="button "  id="close_inquiry_modal" class="btn btn-danger" >بستن</button
                  </div>
                </div>
    ...
    if(tab_status==3){
        $("#send_modal_box").find('form').remove();
    }
    $("#send_modal_box").modal('show');
    $("#inq_select_users").selectpicker()
    var html=await get_all_inq_sends(row_id)
    ...
    $("#send_inquiry").on('click',async function(){
        ...
        var status="";
        var inq_send_comment=$("#inq_send_comment").val();
        ...
        if(status==0){
            if(inq_send_comment.trim()==""){
                custom_alerts('در صورت عدم تایید ثبت توضیحات الزامی می باشد','e',0,'')
                return false;
            }
        }
        var params={action:action,reciever:reciever,status:status,inq_send_comment:inq_send_comment,inquiry_id:row_id}
        var res=await manageAjaxRequestCustom(params)
        var input="";
