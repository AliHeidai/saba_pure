...
async function save_good_row(RowID=0){
    ...
    if(inq_base==3){
        console.log($("#attach_written_order"));
        if($("#attach_written_order")[0].files.length == 0){
    ...
    if(res.res=="true"){
        ...
        $(".modal-backdrop").hide();
         uploadFile(inq_id,'attach_written_order','دستور کتبی خرید');
    }
    else{
        swal_custom('خطا',res.data,'w'); 
        return;
...
async function uploadFile(inq_id,file_input_id,file_title) {
    ...
    if($('#'+file_input_id).val() != '' || $('#'+file_input_id)[0].files.length != 0){
        var fileSelect = document.getElementById(file_input_id);
    ...
    // formData.append("file", file_data.files[0]);
    // formData.append('inq_id',inq_id) ;
    // formData.append('action','attach_inq_file') ;
    // formData.append('file_title',file_title) ;
    // await fetch('/php/inq_manage_proccess.php', {
    //   method: "POST", 
    //   body: formData
    // });    
