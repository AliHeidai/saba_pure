...
function getExcelZamanSanji(){
    ...
    $hdarray = array('نام قطعه','کد قطعه','فورج','ماشین کاری','پرداخت','آب کاری','رنگ','PVD','خط شیلنگ','خط لوله','تزریق پلاستیک',
    ...
    $fieldNames = array('pName','pCode','Forging_timing','Machining_timing','Polishing_timing','Plating_timing',
                        'Paint_timing','PVD_timing','Hose_timing','Pipe_timing','PlasticInjection_timing',
...
function showFundListManageList(){
    ...
    $list = new Listview();
    $ut = new Utility();
    list($name,$code,$amount,$page) = $ut->varCleanInput(array('name','code','amount','page'));
    $amount = str_replace(',','',$amount);
    $res = $Comment->getFundListManageList($name,$code,$amount,$page);
    if($page == 1){
        $_SESSION['calcFundManage'] = $Comment->getFundListManageListCountRows($name,$code,$amount);
    }
...
function showOvertimeLunchManageList(){
    ...
    $totalRows = $_SESSION['calcOvertimeLunch']+ $_SESSION['calcOvertimeWhitoutLunch'];
    ...
    $pagerType = 1;
    $listTitle = '<sapn style="color:green"> تعداد ناهار ها : '.$_SESSION['calcOvertimeLunch']." عدد ".' &nbsp &nbsp &nbsp &nbsp' .' <sapn style="color:red">تعداد بدون ناهار : '.$_SESSION['calcOvertimeWhitoutLunch'].' عدد </span>' ;
...
function doCreateOvertimeLunchPersonnel(){
    ...
    $uti = new Utility();
    $security = new Security();
    list($personnel,$olDate,$unit,$meal) = $uti->varCleanInput(array('personnel','olDate','unit','meal'));
    $res = $security->createOvertimeLunchPersonnel($personnel,$olDate,$unit,$meal);
    if(intval($res) == -1){
...
function doCreateOvertimeLunchGuest(){
    ...
    $uti = new Utility();
    $security = new Security();
    list($olDate,$unit,$fname,$lname,$desc) = $uti->varCleanInput(array('olDate','unit','fname','lname','desc'));
    $res = $security->createOvertimeLunchGuest($olDate,$unit,$fname,$lname,$desc);
    if(intval($res) == -1){
        $res = "شخص تکراری انتخاب شده است !";
        $out = "false";
        response($res,$out);
...
function doAttachFileToBudget(){
    $uti = new Utility();
    $budget = new Budget();
    list($bid,$info) = $uti->varCleanInput(array('bid','info'));
    $files = $_FILES['files'];
    $res = $budget->attachFileToBudget($bid,$info,$files);
    if (intval($res) == -1){
        $res = "فایل ها مشکل دارند !";
        $out = "false";
        response($res,$out);
