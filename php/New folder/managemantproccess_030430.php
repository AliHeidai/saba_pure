...
function get_sms__clients_info(){
    $acm = new acm();
    if(!$acm->hasAccess('saba_admin')){
        die("access denied");
        exit;
    }
    $sms = new SMS();
    
    $res=$sms->get_sms_clients_info();
    if($res) {
        $out = "true";
        response($res, $out);
        exit;
...
function do_send_simple_sms(){
    ...
    $res=$sms->do_send_simple_sms($_POST['receptions'],$_POST['from_number'],$_POST['msg']);
    if($res) {
        $out = "true";
        response($res, $out);
        exit;
