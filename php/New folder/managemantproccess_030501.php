...
function doEditCreateWarranty(){
    ...
    $uti = new Utility();
    $documents = new Documents();
    list($w_id,$w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc) = $uti->varCleanInput(array('w_id','w_title','w_acc_type','w_doc_owner','w_doc_owner_nat_id','w_account_id','w_doc_info','w_access_id','w_start_date','w_end_date','desc'));
    if(intval($w_id) > 0){  //edit
        $res = $documents->editWarranty($w_id,$w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc);
        if($res){
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res,$out);
        ...
        $res = $documents->createWarranty($w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc);
        if($res){
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res,$out);
