...
function doSendPayComment(){
    ...
    if(intval($res) == -1) {
       ...
       // $send_sms=in_array($receiver,$users_send_sms)?1:0;
        if($send_sms==1){
            $ut->fileRecorder('testestes');
            $sql="SELECT `phone` from `users` where RowID={$receiver}";
            $res=$db->ArrayQuery($sql);
            $phone=$res[0]['phone'];
            $sms=new SMS();
            $res_s=$sms->send_by_pattern(array($phone),'cjtp5in07dul99f',array('cartable'=>' اظهارنظرها '));
            $ut->fileRecorder($phone."-".$res_s);
        }
...
function doEditCreateWarranty(){
    ...
    $uti = new Utility();
    $documents = new Documents();
    list($w_id,$w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc,$warranty_cost) = $uti->varCleanInput(array('w_id','w_title','w_acc_type','w_doc_owner','w_doc_owner_nat_id','w_account_id','w_doc_info','w_access_id','w_start_date','w_end_date','desc','warranty_cost'));
    if(intval($w_id) > 0){  //edit
        $res = $documents->editWarranty($w_id,$w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc,$warranty_cost);
        if($res){
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res,$out);
        ...
        $res = $documents->createWarranty($w_title,$w_acc_type,$w_doc_owner,$w_doc_owner_nat_id,$w_account_id,$w_doc_info,$w_access_id,$w_start_date,$w_end_date,$desc,$warranty_cost);
        if($res){
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res,$out);
