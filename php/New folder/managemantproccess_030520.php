...
function doEditCreateComment(){
    ...
    list($cid,$pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type) = $uti->varCleanInput(array('cid','pMethod','pType','Unit','consumerUnit','Type','Toward','totalAmount','Amount','AccNum','CashSection','MaturityCash','NonCashSection','MaturityCheck','RequestSource','RequestNumbers','Desc','billID','payID','layer1','layer2','cFund','cForv','code','ContractNum','CheckNumber','CheckDate','DeliveryDate','CheckCarcass','CardNumber','layer3','goodLoan','checkOutType','related_project','related_vat','is_drivers_fare','is_drivers_fare_type'));
    $files = $_FILES['files'];
    $Amount = str_replace(',','',$Amount);
    ...
    if(intval($cid) > 0){  //edit
        $res = $Comment->editPayComment($cid,$pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$files,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type);
        if(intval($res) == -1){
...
function select_receivers_id_related_barname(){
    $ut = new Utility();
    $Comment = new Comment();
    $res = $Comment->select_receivers_id_related_barname($_POST['pid'],$_POST['meta_data']);
    $out = "true";
    response($res,$out);
    exit;
}
...
function inq_get_all_goods()
{
    ...
    //     exit;
    // } 
    $result = $inq->inq_get_all_goods($_POST['good_name']);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function delete_all_seller_buyer_row()
{
    ...
    $result = $inq->delete_all_form_inq($_POST['inq_id']);
...
function add_seller_buyer_save()
{
    ...
    $export_pay_method = $_POST['export_pay_method'];
    $export_pay_detailes = $_POST['export_pay_detailes'];
    ...
    $inq_good_deliver_time=$_POST['inq_good_deliver_time'];
    $result = $inq->save_inquiry_detailes($inq_good_name, $inq_good_quantity_unit, $inq_good_quantity,$inq_good_buy_request_num,$inq_good_desc,$buy_base_desc,
        ...
        $export_pay_method, $export_pay_detailes, $export_rent_inside,
        $export_rent_outside, $seller_group_code,$inq_id,$export_cash_section,$export_term_payment,$export_description,$is_vat,$temp_seller_name,$inq_good_deliver_time
    );
