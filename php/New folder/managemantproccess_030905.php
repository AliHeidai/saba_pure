...
function doEditCreateComment(){
    ...
    list($cid,$pMethod,$pType,$Unit,$consumerUnit,$Type,$Toward,$totalAmount,$Amount,$AccNum,$CashSection,$MaturityCash,$NonCashSection,$MaturityCheck,$RequestSource,$RequestNumbers,$Desc,$billID,$payID,$layer1,$layer2,$cFund,$cForv,$code,$ContractNum,$CheckNumber,$CheckDate,$DeliveryDate,$CheckCarcass,$CardNumber,$layer3,$goodLoan,$checkOutType,$related_project,$related_vat,$is_drivers_fare,$is_drivers_fare_type,$PropertyNumber) = $uti->varCleanInput(array('cid','pMethod','pType','Unit','consumerUnit','Type','Toward','totalAmount','Amount','AccNum','CashSection','MaturityCash','NonCashSection','MaturityCheck','RequestSource','RequestNumbers','Desc','billID','payID','layer1','layer2','cFund','cForv','code','ContractNum','CheckNumber','CheckDate','DeliveryDate','CheckCarcass','CardNumber','layer3','goodLoan','checkOutType','related_project','related_vat','is_drivers_fare','is_drivers_fare_type','PropertyNumber'));
    $files = $_FILES['files'];
    $Amount = str_replace(',','',$Amount);
