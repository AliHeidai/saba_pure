...
function do_inq_primary_confirm(){
    $ut = new Utility();
    $inq = new Inquires();
    $acm = new acm();
    ...
    $res=$inq->do_inq_primary_confirm($_POST);
    if($res){
        response('عملیات با موفقیت انجام شد', 'true');
    }
    else{
        response('خطا در انجام در عملیات', 'false');
    }
...
function do_inq_last_confirm(){
   ...
   $res=$inq->do_inq_last_confirm($_POST);
   if($res)
   {
        response('استعلام با موفقیت تایید نهایی گردید', 'true');
    }
    else{
        response("خطا در انجام عملیات", 'false');
    } 
...
function create_pay_comment_modal(){
    ...
    $result=$inq->create_pay_comment_modal($_POST['inq_id']);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function save_seller_info(){
   ...
   // $_POST['pay_method_array']=json_decode($_POST['pay_method_array'],true);
    $ut->fileRecorder('post...');
    $ut->fileRecorder($_POST);
    $result = $inq->save_seller_info($_POST);
    // $_POST['is_vat'],
    // $_POST['rent_percent'],
    // $_POST['unit_price'],
    // $_POST['pay_term'],
    ...
    //json_decode($_POST['pay_method_array'],true)
...
function get_inq_seller_detailes(){
    ...
    $result=$inq->get_inq_seller_detailes($_POST['good_id'],$_POST['seller_id'],$_POST['RowID'],$_POST['inq_id']);
    $res=$result;
    $out = "true";
    response($res, $out);
    exit;
...
function show_summary_suggests(){
    ...
    $result=$inq->show_summary_suggests($_POST['inq_id'],$_POST['good_id'],$_POST['seller_id'],$_POST['RowID']);
    $res=$result;
    $out = "true";
    response($res, $out);
    exit;
...
function add_new_seller(){
    ...
    $result = $inq->add_new_seller($_POST);
    // $res = $result;
    if ($result) {
        $out = "true";
        $res = $result;
...
function inqiury_print(){
    ...
    $result = $inq->inqiury_print($_POST['inq_id'],$_POST['print_paraf']);
    // $res = $result;
    if ($result) {
        $out = "true";
        $res = $result;
