...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++
require_once('inq_manage_proccess.php');
require_once('saba_setting.php');
require_once('fetch_from_paya.php');
check_user_status();
$is_user_active = check_active_user();
if ($is_user_active == false) {
    die("access denied");
    exit;
}

if (!isset($_SESSION['username']) || strlen(trim($_SESSION['username'])) == 0 || !intval($_SESSION['userid'])) {
    die("access denied");
    exit;
} else {
    $action = $_POST['action'];
...
//     if($k=="userid")
//         $ut->fileRecorder($_SESSION[]);
// }
function get_saba_users()
{
    $db = new DBi();

    $sql = "SELECT RowID,fname,lname from users where IsEnable=1";
...
function changeUser()
{
    ...
    $sql = "INSERT INTO `sessions` (`session_id`,`user_id`,`created_at`) VALUES('{$sess_id}','{$_SESSION['userid']}','{$curr_date}')";
    $res = $db->Query($sql);
...
{
    $db = new DBi();
    $sql = "SELECT * FROM `users` WHERE `RowID`='" . $_SESSION['userid'] . "' AND `isEnable`=1";
    $res = $db->ArrayQuery($sql);
    if (!count($res)) {
        return false;
    }
...
function check_user_status()
{
    ...
    $temp_restriced_users = $ut->get_full_access_users(26);
    if (in_array($user, $temp_restriced_users)) {
        $_SESSION['user_restric'] = 1;
        die("access denied");
        exit;
    }
}
...
function calculateAddendum_legal_conf_rows()
{
    ...
    $event = new Events();
    $res = $event->calculateAddendumLegalConfirmAlaram();
    if ($htm >= 0) {
...
function calculateAddendum_final_conf_rows()
{
    ...
    $event = new Events();
    $htm = $event->calculateAddendumLFinalConfirmAlaram();
    if ($htm != false) {
...
function calculate_hugging_end_date_alarm()
{
    $acm = new acm();
    if (!$acm->hasAccess('eventsManage')) {
        die("false");
    ...
    $event = new Events();
    $htm = $event->calculate_hugging_end_date_alarm();
    if ($htm != false) {
...
function calculate_sale_warranty_alarm()
{
    ...
    $event = new Events();
    $htm = $event->showWarrantyAlarmModal();
    if ($htm != false) {
...
function calculate_pending_sale_warranty_alarm()
{
    ...
    $event = new Events();
    $htm = $event->showPendingWarrantyAlarmModal();
    if ($htm != false) {
        $out = "true";
        response($htm, $out);
        exit;
...
function  copy_user_access()
{
    ...
    if (!empty($to_user) && !empty($from_user)) {
        $user = new User();
        $res = $user->copy_user_access($from_user, $to_user);
        if ($res) {
            $out = "true";
            response(" دسترسی ها با موفقیت کپی شد", $out);
        } else {
            $out = "false";
            response("تغییری اعمال  نشد", $out);
        }
    } else {
        $out = "false";
        response(" کاربر مقصد یا کاربر مبدا انتخاب نشده است", $out);
    }
}
...
function createMasterListPieceExcel()
{
    ...
    $fieldNames = array(
        ...
        'material',
        'ChangingHow_supply',
        'first_stage_construction',
        'Custom_dimensions',
        'RawMaterialCode',
        'external_size_bullion',
        'System_weight',
        'Weight_materials',
        'Weight_Machining',
        'Weight_Final'
...
function get_pieces_product_bom_update_history()
{
    $ut = new Utility();
    $res = $ut->get_pieces_product_bom_update_history($_POST);
    if ($res != false) {
        $out = "true";
        response($res, $out);
        exit;
...
function display_eng_update_error()
{
    $ut = new Utility();
    $acm = new acm();
    if ($acm->hasAccess('GetFullUpdateHistory')) {
        $res = $ut->display_eng_update_error($_POST);
        if ($res != false) {
            $out = "true";
            response($res, $out);
            exit;
...
function showGoodManageList()
{
    ...
    $list = new Listview();
    $ut = new Utility();
    list($page, $gName, $gCode, $parvaneh, $hCode) = $ut->varCleanInput(array('page', 'gName', 'gCode', 'parvaneh', 'hCode'));
    $res = $Good->getGoodList($gName, $gCode, $parvaneh, $hCode, $page);
    if ($page == 1) {
    ...
    $feilds[$c]['title'] = "کد مهندسی محصول";
    $feilds[$c]['width'] = "8%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "gCode";
    $c++;
...
function createMasterListGoodExcel()
{
    ...
    $hdarray = array(
        ...
        'کارتن مادر',
        'کارتن بچه',
        'تعداد محصول در کارتن بچه',
        'تعداد کارتن بچه در کارتن مادر',
        'پروانه بهره برداری',
        'وزن کارتن مادر انبار (کیلوگرم)',
        'وزن کارتن مادر BOM (کیلوگرم)',
        'توضیحات'
    );
    $fieldNames = array(
        'gCode',
...
function showPersonnelManageList()
{
    ...
    $list = new Listview();
    $ut = new Utility();
    list($page, $Pname, $Pfamily, $Pcode, $Punit, $RsDate, $ReDate, $TsAmount, $TeAmount, $Pname1, $Pfamily1, $Pcode1, $Punit1, $RsDate1, $ReDate1, $TsAmount1, $TeAmount1, $ability, $status, $endContractDate) = $ut->varCleanInput(array('page', 'Pname', 'Pfamily', 'Pcode', 'Punit', 'RsDate', 'ReDate', 'TsAmount', 'TeAmount', 'Pname1', 'Pfamily1', 'Pcode1', 'Punit1', 'RsDate1', 'ReDate1', 'TsAmount1', 'TeAmount1', 'ability', 'status', 'endContractDate'));
    $TsAmount = str_replace(',', '', $TsAmount);
    $TeAmount = str_replace(',', '', $TeAmount);
    $res = $salary->getPersonnelList($Pname, $Pfamily, $Pcode, $Punit, $RsDate, $ReDate, $TsAmount, $TeAmount, $Pname1, $Pfamily1, $Pcode1, $Punit1, $RsDate1, $ReDate1, $TsAmount1, $TeAmount1, $ability, $status, $endContractDate, $page);
    if ($page == 1) {
        $_SESSION['calcPersonnel'] = $salary->getPersonnelListCountRows($Pname, $Pfamily, $Pcode, $Punit, $RsDate, $ReDate, $TsAmount, $TeAmount, $Pname1, $Pfamily1, $Pcode1, $Punit1, $RsDate1, $ReDate1, $TsAmount1, $TeAmount1, $ability, $status, $endContractDate);
        $t = $salary->getTotalAmountPersonnel($Pname, $Pfamily, $Pcode, $Punit, $RsDate, $ReDate, $TsAmount, $TeAmount, $Pname1, $Pfamily1, $Pcode1, $Punit1, $RsDate1, $ReDate1, $TsAmount1, $TeAmount1, $ability, $status, $endContractDate);
...
function finalPricePersonnelExcel()
{
    ...
    $hdarray = array(
        ...
        'سنوات',
        'هزینه سرویس',
        'حدود ساعت اضافه کار در ماه',
        'فوق العاده اضافه کار',
        'جمع قیمت تمام شده ماهیانه',
        'جمع قیمت تمام شده سالیانه'
    );
    $fieldNames = array(
        'Fname',
        'PersonnelCode',
...
function doEditCreatePersonnel()
{
    ...
    $personnelRightMarry = str_replace(',', '', $personnelRightMarry);
    if (intval($pid) > 0) { //edit
        $leaveDate = $_POST['leaveDate'];

        $res = $Sy->editPersonnel(
            $pid,
            $Fname,
            $Lname,
            $fatherName,
...
function doEditDocumentPersonnel()
{
    ...
    list(
        ...
        $RecognizanceDesc,
        $NationalCardDesc,
        $NationalCardDependantsDesc,
        $InsuranceBookletDesc,
        $CardMilitaryDesc,
        $InsuranceRecordsDesc,
        $DegreeEducationDesc,
        $PhotoDesc,
        $CertificateDesc,
        $VerificationServiceRecordsDesc,
        $LackBackgroundDesc,
        $AccountNumberDesc,
        $CheckPromissoryNoteDesc,
        $ExperimentsDesc
    ) = $uti->varCleanInput(array(
        'pid',
        'Questionnaire',
    ...
    $res = $Sy->createPersonnelDocument(
        $pid,
        $Questionnaire,
        $Recognizance,
        $NationalCard,
...
function refresh_select_options()
{
    ...
    $select_alias_name = $_POST['select_alias_name'];
    $meta_data = $_POST['meta_data'];
    switch ($select_alias_name) {
        case "commentManagmentOneLayer":
            $Comment = new Comment();
            $res = $Comment->refresh_select_options($meta_data);

            break;
        case "commentManagmentTwoLayer":
            $Comment = new Comment();
            $res = $Comment->refresh_select_options($meta_data, 1);
...
function get_related_to_barnameh_pay_comment()
{
    $acm = new acm();
    if (!$acm->hasAccess('commentManagement')) {
        die("access denied");
    ...
    $Comment = new Comment();
    $ut = new Utility();
    $res = $Comment->get_related_to_barnameh_pay_comment($_POST['cid']);
    $out = "true";
    response($res, $out);
    exit;
}
...
function showPayCommentManageList()
{
    ...
    $amount = str_replace(',', '', $amount);
    $result_c = $Comment->getPayCommentManageList($csDate, $ceDate, $cUnit, $coUnit, $caName, $cToward, $Uncode, $amount, $page, $layer_one, $layer_two, $layer_three);
    $res = $result_c[0];
    if ($page == 1) {
        $_SESSION['calcCommManage'] = $result_c[1]; //$Comment->getPayCommentManageListCountRows($csDate,$ceDate,$cUnit,$coUnit,$caName,$cToward,$Uncode,$amount,$layer_one,$layer_two,$layer_three);
    }
    ...
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "checkBox";
    $feilds[$c]['class'] = "comment_select_row";
    $feilds[$c]['barname'] = "barname";
    $feilds[$c]['sendType'] = "sendType";
    $feilds[$c]['is_vat'] = "is_vat";

    $feilds[$c]['onclick'] = "select_comment_row(this)";
    $c++;
...
function toggle_comment_force_status()
{
    $acm = new acm();
    if (!$acm->hasAccess('changeCommentForceStatus')) {
        die("access denied");
        exit;
    }
    $Comment = new Comment();
    $res = $Comment->toggle_comment_force_status($_POST);
    if ($res) {
        response('عملیات با موفقیت انجام شد', 'true');
    } else {
        response('عملیات با خطا مواجه شد', 'false');
    }
}

function showPayCommentSendManageList()
...
function showPayCommentStagnantManageList()
{  // اظهارنظرهای راکد در کارتابل
    ...
    $result = $Comment->getPayCommentStagnantManageList($csDate, $ceDate, $cUnit, $coUnit, $caName, $cToward, $Uncode, $cardboardID, $page);
    //$ut->fileRecorder($result);
    $res = $result[0];
    if ($page == 1) {
        $_SESSION['calcStagnantCommManage'] = $result[1]; //$Comment->getPayCommentStagnantManageListCountRows($csDate,$ceDate,$cUnit,$coUnit,$caName,$cToward,$Uncode,$cardboardID);
    }
    ...
    $out = "true";
    $res_array[0] = $htm;
    $res_array[1] = $result[2];
    response($res_array, $out);
    exit;
}

function showPayCommentStagnantManageList1()
{  // اظهارنظرهای راکد از زمان ایجاد
    ...
    if ($page == 1) {
        $count_res = $Comment->getPayCommentStagnantManageListCountRows1($csDate, $ceDate, $cUnit, $coUnit, $caName, $cToward, $Uncode, $cardboardID);
        $_SESSION['calcStagnantCommManage'] = $count_res[0];
    }
    $totalRows = $_SESSION['calcStagnantCommManage'];
    $c = 0;
    $feilds = array();
    ...
    $out = "true";
    $final_res[0] = $htm;
    $final_res[1] = $count_res[1];
    response($final_res, $out);
    exit;
}

function get_view_detailes()
{
    $comment = new Comment();
    $res = $comment->get_view_detailes($_POST);
...
function get_related_property_number()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('commentManagement')) {
    ...
    $uti = new Utility();
    $Comment = new Comment();
    $res = $Comment->get_related_property_number($_POST['parent_id']);
    $ut->fileRecorder($res);
    return $res;
}
...
function doEditCreateComment()
{
    ...
    list($cid, $pMethod, $pType, $Unit, $consumerUnit, $Type, $Toward, $totalAmount, $Amount, $AccNum, $CashSection, $MaturityCash, $NonCashSection, $MaturityCheck, $RequestSource, $RequestNumbers, $Desc, $billID, $payID, $layer1, $layer2, $cFund, $cForv, $code, $ContractNum, $CheckNumber, $CheckDate, $DeliveryDate, $CheckCarcass, $CardNumber, $layer3, $goodLoan, $checkOutType, $related_project, $related_vat, $is_drivers_fare, $is_drivers_fare_type, $PropertyNumber, $creditName, $payrolls) = $uti->varCleanInput(array('cid', 'pMethod', 'pType', 'Unit', 'consumerUnit', 'Type', 'Toward', 'totalAmount', 'Amount', 'AccNum', 'CashSection', 'MaturityCash', 'NonCashSection', 'MaturityCheck', 'RequestSource', 'RequestNumbers', 'Desc', 'billID', 'payID', 'layer1', 'layer2', 'cFund', 'cForv', 'code', 'ContractNum', 'CheckNumber', 'CheckDate', 'DeliveryDate', 'CheckCarcass', 'CardNumber', 'layer3', 'goodLoan', 'checkOutType', 'related_project', 'related_vat', 'is_drivers_fare', 'is_drivers_fare_type', 'PropertyNumber', 'creditName', 'payrolls'));
    $files = $_FILES['files'];
    $Amount = str_replace(',', '', $Amount);
    ...
    if (intval($cid) > 0) {  //edit
        ...
        if (intval($res) == -1) {
            ...
            $res = "    مبلغ کل اظهارنظر از مجموع مبلغ بخش چک و نقدی کمتر است !!!";
            $out = "false";
            response($res, $out);
            exit;
        } elseif (intval($res) == -4) {
...
function deleteSelectedPayComment()
{
    ...
    $pid_array = $_POST['pid_array'];
    if (count($pid_array) == 0) {
        $res = "هیچ اظهارنظری  انتخاب نشده است";
        $out = "false";
        response($res, $out);
        exit;
    }
    ...
    $res = $Comment->deleteSelectedPayComment($pid_array);
    if ($res) {
        $out = "true";
        response($res, $out);
        exit;
...
function DeleteAutoSendRecord()
{ //Delete record from  auto send comment
    $ut = new Utility();
...
function select_receivers_id_related_barname()
{
    $ut = new Utility();
    $Comment = new Comment();

    $res = $Comment->select_receivers_id_related_barname($_POST['cid_arr']);
    $out = "true";
    response($res, $out);
    exit;
}
...
{
    $ut = new Utility();
    list($pid, $meta_data) = $ut->varCleanInput(array('pid', 'meta_data'));
    if (!intval($pid)) {
        $res = "شناسه اظهارنظر بدرستی دریافت نشد !";
        $out = "false";
        response($res, $out);
    ...
    $Comment = new Comment();
    $res = $Comment->commentWorkflowHtm($pid, $meta_data);
    $out = "true";
    response($res, $out);
    exit;
}
...
function transferPartnersCartable()
{
    ...
    list($sender, $receiver) = $ut->varCleanInput(array('sender', 'receiver'));
    $res = $Comment->transferPartnersPayCommentCartable($_POST['pid_array'], $sender, $receiver);
    if (intval($res) > 0) {
        $out = "true";
...
//--------------------------------------------------ایجاد  گروهی طرف حساب---------------------------------------
function doUploadAccountFile()
{
    ...
    // }
    $comment = new Comment();
    $gsplist = $_FILES['excAccounts'];
    $res = $comment->uploadAccountGroupFile($gsplist);
    if (is_array($res) && $res[0] == 'undefined') {
        $res = " کد تفضیلی <span color:'red'> {$code} </span> معتبر نمی باشد";
        $out = "false";
        response($res, $out);
    }
    if (is_array($res) && $res[0] == 'duplicate') {
        $res = "کد تفضیلی به شماره  <span style='color:red'>{$res[1]}</span>  تکراری می باشد ";
        $out = "false";
        response($res, $out);
    }
    if (is_array($res) && $res[0] == 'notName') {
        $res = " نام طرف حساب وارد نشده است";
        $out = "false";
        response($res, $out);
    }
    if (is_array($res) && $res[0] == 'notMatch') {
        $res = "فرمت فایل اکسل آپلود شده صحیح نمی باشد  فرمت صحیح را از قسمت  <span style='color:red'> دانلود نمونه</span> دانلود نمایید و طبق فرمت فایل اکسل را تکمیل نمایید";
        $out = "false";
        response($res, $out);
    }
...
function getSubLayers()
{
    $comment = new Comment();
    $ut = new Utility();
    list($layer1, $barnameh_select) = $ut->varCleanInput(array('layer1'));
    $ut->fileRecorder($barnameh_select);
...
function set_commentManagmentUnit()
{
    $meta_data = $_POST['meta_data'];
    $comment = new Comment();
    $ut = new Utility();
    $res = $comment->set_commentManagmentUnit($meta_data);
    $out = "true";
    response($res, $out);
    exit;
}

function get_sublayer_two()
{
    $comment = new Comment();
    $ut = new Utility();
    list($layer1, $barnameh_select) = $ut->varCleanInput(array('layer1', 'barnameh_select'));
    $res = $comment->get_sublayer_two($layer1, $barnameh_select);
    $out = "true";
    response($res, $out);
    exit;
}
...
function get_customers_recived_users()
{
    $acm = new acm();
    ...
    $Comment = new Comment();
    $res = $Comment->get_customers_recived_users();
    $res = $res;
    $out = "true";
    response($res, $out);
    exit;
...
function get_customer_receiver_workflow()
{
    $acm = new acm();
    ...
    $arr = $_POST;
    $Comment = new Comment();
    $res = $Comment->get_customer_receiver_workflow($arr);
...
function doSendReceivedCustomer()
{
    $acm = new acm();
    ...
    $arr = $_POST;
    $Comment = new Comment();
    $res = $Comment->doSendReceivedCustomer($arr);
    if ($res == 1) {
        $out = "true";
    } else {
        $out = "false";
...
function getSendedCustomerRecieved()
{
    $acm = new acm();
    $list = new Listview();
    if (!$acm->hasAccess('receivedCustomerManage')) {
    ...
    $Comment = new Comment();
    $html = $Comment->getSendedCustomerRecieved($_POST);
...
function getArchivedCustomerRecieved()
{
    $acm = new acm();
    ...
    $Comment = new Comment();
    $html = $Comment->getArchivedCustomerRecieved($_POST);
...
function getTempCustomerRecieved()
{
    $acm = new acm();
    ...
    $Comment = new Comment();
    $html = $Comment->getTempCustomerRecieved($_POST);
...
function showReceivedCustomerManageList()
{
    ...
    $list = new Listview();
    $ut = new Utility();
    list($rsDate, $reDate, $RType, $RMethod, $CName, $RAmount, $lastReceiver, $chkNum, $page) = $ut->varCleanInput(array('rsDate', 'reDate', 'RType', 'RMethod', 'CName', 'RAmount', 'lastReceiver', 'chkNum', 'page'));
    $res = $Comment->getReceivedCustomerList($rsDate, $reDate, $RType, $RMethod, $CName, $RAmount, $lastReceiver, $chkNum, $page);
    if ($page == 1) {
        $_SESSION['calcRCUManage'] = $Comment->getReceivedCustomerListCountRows($rsDate, $reDate, $RType, $RMethod, $CName, $RAmount, $lastReceiver, $chkNum);
        $t = $Comment->getTotalAmountReceivedCustomer($rsDate, $reDate, $RType, $RMethod, $CName, $RAmount, $lastReceiver, $chkNum);
    ...
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "radio";
    $feilds[$c]['Name'] = "radioCRid";
    $feilds[$c]['onclick'] = "select_row(this)";
    $c++;
    ...
    $feilds[$c]['title'] = "تاریخ واریز";
    $feilds[$c]['width'] = "6%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "depositDate";
    $c++;
    ...
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "btn";
    $feilds[$c]['param'] = "RowID,1";

    // $feilds[$c]['onclick'] = "receivedCustomerCheckInfo";
    $feilds[$c]['onclick'] = "ShowFinanceManage";
    $feilds[$c]['disabled'] = "yes";
    $feilds[$c]['icon'] = "fa-tv";
    $c++;
    ...
    $feilds[$c]['f'] = "btn";
    $feilds[$c]['param'] = "RowID";
    $feilds[$c]['onclick'] = "getSendHistory";
    $feilds[$c]['icon'] = "fa-history";
    $c++;


    if ($acm->hasAccess('receivedCustomerCheckManage')) {
        $feilds[$c]['title'] = "مدیریت مالی ";
        $feilds[$c]['width'] = "6%";
        $feilds[$c]['order'] = "none";
        $feilds[$c]['f'] = "btn";
        $feilds[$c]['param'] = "RowID";
    ...
    if ($acm->hasAccess('receivedCustomerDelete')) {
        $feilds[$c]['title'] = " ابطال ";
        $feilds[$c]['width'] = "3%";
        $feilds[$c]['order'] = "none";
        $feilds[$c]['f'] = "btn";
        $feilds[$c]['param'] = "RowID";
        $feilds[$c]['onclick'] = "delete_CR_row";
        $feilds[$c]['disabled'] = "yes";
        $feilds[$c]['icon'] = "fa-trash";
        $feilds[$c]['btnClass'] = "btn-danger";
    }
...
function get_CR_row()
{
    ...
    $Comment = new Comment();
    $ut = new Utility();
    $htm = $Comment->get_CR_row($_POST);
    $ut->fileRecorder($_POST);
    response($htm, 'true');
}

function doConfirmCR()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('receivedCustomerManage')) {
    ...
    $Comment = new Comment();
    $ut = new Utility();
    $res = $Comment->doConfirmCR($_POST);
    if ($res) {
        $out = "true";
        $msg = "درخواست با موفقیت انجام شد.";
    } else {
        $out = "false";
        $msg = "درخواست بدرستی انجام نشد.";
    }
    response($msg, $out);
}

function get_CR_row_temp_confirm()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('receivedCustomerManage')) {
    ...
    $Comment = new Comment();
    $ut = new Utility();
    $htm = $Comment->get_CR_row_temp_confirm($_POST);
...
function doEditCreateReceivedCustomer()
{
    ...
    $arr = $_POST;
    //list($rid, $RType, $RMethod, $amount, $rDate, $CstName, $desc, $bank, $pos, $serial, $chDate, $chOwner, $chOwnerC,$chBankIssuingBank,$is_sayyad,$depositDate) = $ut->varCleanInput(array('rid', 'RType', 'RMethod', 'amount', 'rDate', 'CstName',  'desc', 'bank', 'pos', 'serial', 'chDate', 'chOwner', 'chOwnerC','chBankIssuingBank','is_sayyad','depositDate'));
    // $amount = str_replace(',', '', $amount);
    if (intval($_POST['rid']) > 0) {  //edit
        $res = $Comment->editReceivedCustomer($arr);
        if ($res != false) {
            $res = 'درخواست با موفقیت انجام شد.';
            $out = "true";
            response($res, $out);
        ...
        $res = $Comment->createReceivedCustomer($arr);
        if ($res != false) {
            $res = 'درخواست با موفقیت انجام شد.';
            $out = "true";
            response($res, $out);
...
function delete_CR_row()
{
    ...
    $Comment = new Comment();
    $ut = new Utility();
    $res = $Comment->delete_CR_row($_POST);
    if ($res) {
        $msg = "ابطال با موفقیت انجام شد";
        $out = "true";
    } else {
        $msg = " تغییری اعمال نگردید!!";
        $out = "false";
    }
...
function showContractManageList()
{
    ...
    $feilds[$c]['title'] = "مبلغ کل (قرارداد + الحاقیه ها)";
    $feilds[$c]['width'] = "11%";
    $feilds[$c]['order'] = "none";
    ...
    $pagerType = 1;
    $listTitle = "<span> تعداد کل قراردادها : " . $totalRows . " عدد </span>" . "<br><span style='display:inline-block;width:20px; height:20px;border-radius:100%;margin-inline:10px;margin-block:0'class='bg-warning-light '></span><span>قرارداد  دارای الحاقیه</span>";
...
function sendAddendum()
{
    ...
    $uti = new Utility();
    $Comment = new Comment();
    $res = $Comment->sendAddendum($_POST['addendum_id'], $_POST['addendum_status']);
    if (count($res) > 0) {
        $res = $res;
        $out = "true";
        response($res, $out);
...
function do_send_addendum()
{
    ...
    $uti = new Utility();
    $Comment = new Comment();
    $res = $Comment->do_send_addendum($_POST['addendum_id'], $_POST['addendum_status'], $_POST['confirm_status'], $_POST['receiver'], $_POST['addendum_message']);
    if ($res && $res > 0) {
        if ($res == 1) {
            $unit = "  به واحد حقوقی ارسال گردید";
        } elseif ($res == 2) {
            $unit = "  به مدیریت محترم عامل ارجاع گردید";
        } elseif ($res == 3) {
            $unit = " تایید نهایی گردید";
        }
        $res = 'الحاقیه با موفقیت  ' . $unit;
        $out = "true";
    } else {
        $res = 'خطا در  ارسال الحاقیه';
        $out = "false";
    }
    response($res, $out);
    exit;
...
function showAddendumSendHistory()
{
    $acm = new acm();
    if (!$acm->hasAccess('contractManagement')) {
        die("access denied");
    ...
    $uti = new Utility();
    $Comment = new Comment();
    $res = $Comment->showAddendumSendHistory($_POST['addendum_id']);
    if ($res) {
        $out = "true";
        response($res, $out);
        exit;
...
function get_exported_payComments()
{
    $ut = new Utility();
    $Comment = new Comment();

    $res = $Comment->get_exported_payComments($_POST['cid'], $_POST['payed']);
    if ($res) {
        $res = $res;
        $out = "true";
        response($res, $out);
...
function get_contract_pay_rows()
{
    ...
    $contract_number = $_POST['contract_number'];
    $result = $comment->get_contract_pay_rows($contract_id, $pay_type, $contract_number);
    if ($result) {
        $out = "true";
        response($result, $out);
        exit;
...
function doEditCreateAccount()
{
    ...
    list($aid, $Name, $code, $an, $codeMelli, $role_array) = $uti->varCleanInput(array('aid', 'Name', 'code', 'an', 'codeMelli', 'role_array'));
    $role_array = explode(',', $role_array);
    $uti->fileRecorder($role_array);
    if (intval($aid) > 0) { //edit
        $res = $Comment->editAccount($aid, $Name, $code, $an, $codeMelli, $role_array);
        if (intval($res) == -2) {
        ...
        $res = $Comment->createAccount($Name, $code, $an, $codeMelli, $role_array);
        if (intval($res) == -2) {
            $res = "این کد تفضیلی قبلا ثبت شده است !";
            $out = "false";
            response($res, $out);
...
function showFinalPayCommentManageList()
{
    ...
    $list = new Listview();
    $ut = new Utility();
    list($pcaName, $pcToward, $pcAmount, $typeDay, $page, $unCode) = $ut->varCleanInput(array('pcaName', 'pcToward', 'pcAmount', 'typeDay', 'page', 'unCode'));
    $res = $Comment->getFinalPayCommentManageList($pcaName, $pcToward, $pcAmount, $typeDay, $unCode, $page);
    if ($page == 1) {
        $_SESSION['calcPCommManage'] = $Comment->getFinalPayCommentManageListCountRows($pcaName, $pcToward, $pcAmount, $typeDay, $unCode);
...
function showReportPayCommentManageList()
{
    ...
    $list = new Listview();
    $ut = new Utility();
    list($unCode, $rcsDate, $rceDate, $rcaName, $rcToward, $rcAmount, $rcPaytype, $rcPaySend, $rcSortType, $rpcsDate, $rpceDate, $rpnsDate, $rpneDate, $dUnit, $mUnit, $layer1, $layer2, $layer3, $page, $c_type) = $ut->varCleanInput(array('unCode', 'rcsDate', 'rceDate', 'rcaName', 'rcToward', 'rcAmount', 'rcPaytype', 'rcPaySend', 'rcSortType', 'rpcsDate', 'rpceDate', 'rpnsDate', 'rpneDate', 'dUnit', 'mUnit', 'layer1', 'layer2', 'layer3', 'page', 'c_type'));
    $rcAmount = str_replace(',', '', $rcAmount);
    $res = $Comment->getReportPayCommentManageList($unCode, $rcsDate, $rceDate, $rcaName, $rcToward, $rcAmount, $rcPaytype, $rcPaySend, $rcSortType, $rpcsDate, $rpceDate, $rpnsDate, $rpneDate, $dUnit, $mUnit, $layer1, $layer2, $layer3,  $c_type, $page);
    //$ut->fileRecorder($res);
    if ($page == 1) {
        $_SESSION['calcRPCommManage'] = $Comment->getReportPayCommentManageListCountRows($unCode, $rcsDate, $rceDate, $rcaName, $rcToward, $rcAmount, $rcPaytype, $rcPaySend, $rpcsDate, $rpceDate, $rpnsDate, $rpneDate, $dUnit, $mUnit, $layer1, $layer2, $layer3, $c_type);
        $t = $Comment->getTotalAmountPayComment($unCode, $rcsDate, $rceDate, $rcaName, $rcToward, $rcAmount, $rcPaytype, $rcPaySend, $rpcsDate, $rpceDate, $rpnsDate, $rpneDate, $dUnit, $mUnit, $layer1, $layer2, $layer3, $c_type);
        ...
        $_SESSION['payed_by_others'] = $t[5];
    }
    $totalRows = $_SESSION['calcRPCommManage'];
    $headerTxt = 'مبلغ کل = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;">' . number_format($_SESSION['calcTotalAmountt']) . '</span>&nbsp;&nbsp;ریال <button class="btn btn-primary mr-3" onclick="showSeparationTotalAmount()">جزئیات</button>
                    <br/> مبلغ واریزی نقدی = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;">' . number_format($_SESSION['calcTotalNaghdi']) . '</span>&nbsp;&nbsp;ریال <br/>
                      مبلغ واریزی چکی = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;">' . number_format($_SESSION['calcTotalCheki']) . '</span>&nbsp;&nbsp;ریال <br/>
                        مبلغ ثبت در حساب بستانکاری = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;">' . number_format($_SESSION['calcTotalSDHesab']) . '</span>&nbsp;&nbsp;ریال 
                        <br>مبلغ واریز شده توسط سایرین  = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;">' . number_format($_SESSION['payed_by_others']) . '</span>&nbsp;&nbsp;ریال 
                        <br/>  ' . (intval($_SESSION['calcTotalMandeh']) < 0 ? 'مبلغ مازاد پرداختی' : 'مبلغ پرداخت نشده') . ' = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;" dir="ltr">' . number_format($_SESSION['calcTotalMandeh']) . '</span>&nbsp;&nbsp;ریال ';

    $c = 0;
    $feilds = array();
    $feilds[$c]['title'] = "کد یکتا ";
    $feilds[$c]['width'] = "5%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['blink'] = "yes";
    $feilds[$c]['f'] = "unCode";
    ...
    $feilds[$c]['title'] = "مبلغ واریزی سایرین";
    $feilds[$c]['width'] = "8%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['blink'] = "yes";
    $feilds[$c]['f'] = "payed_other";
    $c++;
...
function payCommentReportExcel()
{
    $Comment = new Comment();
    $ut = new Utility();
    list($unCode, $rcsDate, $rceDate, $rcaName, $rcToward, $rcAmount, $rcPaytype, $rcPaySend, $rpcsDate, $rpceDate, $rpnsDate, $rpneDate, $dUnit, $mUnit, $c_type) = $ut->varCleanInput(array('unCode', 'rcsDate', 'rceDate', 'rcaName', 'rcToward', 'rcAmount', 'rcPaytype', 'rcPaySend', 'rpcsDate', 'rpceDate', 'rpnsDate', 'rpneDate', 'dUnit', 'mUnit', 'c_type'));

    $res = $Comment->getPayCommentReportExcel($unCode, $rcsDate, $rceDate, $rcaName, $rcToward, $rcAmount, $rcPaytype, $rcPaySend, $rpcsDate, $rpceDate, $rpnsDate, $rpneDate, $dUnit, $mUnit, $c_type);
    $hdarray = array('کد یکتا', 'نوع', 'نوع پرداخت اظهارنظر', 'تاریخ ثبت', 'در وجه', 'کد تفضیلی', 'بابت', 'مبلغ', 'نقدی', 'چک', 'توضیحات');
    $fieldNames = array('unCode', 'sendType', 'type', 'cDate', 'accName', 'codeTafzili', 'Toward', 'Amount', 'CashSection', 'NonCashSection', 'desc');
...
function doEditCreateCar()
{
    ...
    $ut = new Utility();
    $car = new Car();
    list($caid, $name, $plaque, $chassis, $serial, $fuelType, $TDDate, $TIDate, $BIDate, $carType, $lastKilometer, $hugging_start, $hugging_end, $hugging_flag) = $ut->varCleanInput(array('caid', 'name', 'plaque', 'chassis', 'serial', 'fuelType', 'TDDate', 'TIDate', 'BIDate', 'carType', 'lastKilometer', 'hugging_start', 'hugging_end', 'hugging_flag'));
    ...
    $files6 = $_FILES['files6'];  // آخرین وضعیت
    if (intval($caid) > 0) {  //edit
        $res = $car->editCar($caid, $name, $plaque, $chassis, $serial, $fuelType, $TDDate, $TIDate, $BIDate, $carType, $files, $files1, $files2, $files3, $files4, $files5, $files6, $hugging_start, $hugging_end, $hugging_flag);
        if (intval($res) == -1) {
            $res = "فایل ها مشکل دارند !";
            $out = "false";
            response($res, $out);
        ...
        $res = $car->createCar($name, $plaque, $chassis, $serial, $fuelType, $TDDate, $TIDate, $BIDate, $carType, $lastKilometer, $files, $files1, $files2, $files3, $files4, $files5, $files6, $hugging_start, $hugging_end, $hugging_flag);
        if (intval($res) == -1) {
            $res = "فایل ها مشکل دارند !";
            $out = "false";
            response($res, $out);
...
function downloadhuggingFile()
{
    ...
    $car = new Car();
    $res = $car->downloadhuggingFile($caid);
    $out = "true";
    response($res, $out);
    exit;
}
...
function showRecordEventsList()
{
    ...
    $list = new Listview();
    $ut = new Utility();
    list($sDate, $eDate, $desc, $page, $event_type) = $ut->varCleanInput(array('sDate', 'eDate', 'desc', 'page', 'event_type'));
    $res = $security->getRecordEventsList($sDate, $eDate, $desc, $event_type, $page);
    if ($page == 1) {
        $_SESSION['calcRecordEvents'] = $security->getRecordEventsListCountRows($sDate, $eDate, $desc, $event_type);
    }
...
function getEnventsTypeList()
{
    $ut = new Utility();
    $security = new Security();
    $res = $security->getEnventsTypeList();
    if ($res != false) {
        $out = "true";
        response($res, $out);
        exit;
...
function manage_allowed_users()
{
    $acm = new acm();

    $security = new Security();
    $array = $security->manage_allowed_users($_POST);
    $out = "true";
    if ($array == false) {
        $res = "اطلاعات بدرستی دریافت نشد !";
        $out = "false";
...
function get_events_report()
{
    $ut = new Utility();
    $security = new Security();
    $acm = new acm();
    if (!$acm->hasAccess('EventsReport')) {
        die("access denied");
        exit;
    }

    $res = $security->get_events_report($_POST);
    if ($res != false) {
        $out = "true";
        response($res, $out);
        exit;
...
function doEditCreateEventType()
{
    // $acm = new acm();
    // if (!$acm->hasAccess('recordEventManage')) {
    //     die("access denied");
    ...
    $res = $security->doEditCreateEventType($_POST);
    if ($res) {
        $res = "درخواست با موفقیت انجام شد.";
        $out = "true";
        response($res, $out);
...
function doEditCreateEvents()
{
    ...
    $uti = new Utility();
    $security = new Security();
    list($eid, $cTime, $desc, $cDate) = $uti->varCleanInput(array('eid', 'cTime', 'desc', 'cDate'));
    if (intval($eid) > 0) {  //edit
        $res = $security->editEvents($_POST);
        if ($res) {
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res, $out);
        ...
        $res = $security->createEvents($_POST);
        if ($res == 1) {
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res, $out);
            ...
            $res = "اول باید رویداد قطع برق ثبت شود";
            $out = "false";
            response($res, $out);
            exit;
        } elseif ($res == -2) {
            $res = " زمان ثبت شده برای وصل برق قبل از زمان ثبت شده برای قطع برق می باشد";
            $out = "false";
            response($res, $out);
            exit;
        } else {
...
function get_tools_status_list()
{
    $acm = new acm();
    if (!$acm->hasAccess('PropertyManagement')) {
        die("access denied");
        exit;
    }
    $p = new Property();
    $result = $p->get_tools_status_list();
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function create_prop_excel_report()
{
    $acm = new acm();
    if (!$acm->hasAccess('PropertyManagement')) {
        die("access denied");
    ...
    $p = new Property();
    $result = $p->create_prop_excel_report($_POST);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function get_transaction_type_list()
{
    ...
    $p = new Property();
    $result = $p->get_transaction_type_list();
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function get_property_tools_list()
{
    $acm = new acm();
    if (!$acm->hasAccess('PropertyManagement')) {
        die("access denied");
    ...
    $p = new Property();
    $result = $p->get_property_tools_list();
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function doEditCreatePropertyManage()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('PropertyManagement')) {
    ...
    $p = new Property();
    $file = $_FILES['prop_doc_file'];
    $result = $p->doEditCreatePropertyManage($_POST, $file);
    ...
    if (!$result) {
        ...
        if ($result == -1) {
            $res = 'یک یا چند فایل  غیر مجاز می باشد';
        } elseif ($result == -2) {
            $res = ' مجموع حجم فایلهای بارگزاری شده بیشتر از 2مگابایت می باشد';
        } elseif ($result == 1) {
            $out = "true";
...
function doEditCreatePropertyTools()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('PropertyManagement')) {
    ...
    $result = $p->doEditCreatePropertyTools($_POST);
...
function showPropertyManageList()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('PropertyManagement')) {
    ...
    $result = $p->showPropertyManageList($_POST);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function showPropertyToolsList()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('PropertyManagement')) {
    ...
    $result = $p->showPropertyToolsList($_POST);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function get_transaction_attach()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('PropertyManagement')) {
    ...
    $result = $p->get_transaction_attach($_POST);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function prop_download_file()
{
    ...
    unset($_POST['action']);
    $ut->fileRecorder($_POST);
    $result = $p->prop_download_file($_POST);
...
function get_property_info()
{
    ...
    $result = $p->get_property_info($_POST);
...
function get_property_tools()
{
    ...
    $p = new Property();
    unset($_POST['action']);
    $result = $p->get_property_tools($_POST);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function print_prop_report()
{
    ...
    $p = new Property();
    unset($_POST['action']);
    $result = $p->print_prop_report($_POST);
    $res = $result;
    $out = "true";
    response($res, $out);
    exit;
...
function deletePropertyManageAttachment()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('PropertyManagement')) {
    ...
    $result = $p->deletePropertyManageAttachment($_POST);
    $res = 'عملیات با موفقیت انجام شد';
    $out = "true";
    if (!$result) {
        $res = ' هیچ تغییری اعمال نگردید';
...
function deletePropertyManageRow()
{
    $acm = new acm();
    $ut = new Utility();
    if (!$acm->hasAccess('PropertyManagement')) {
    ...
    $result = $p->deletePropertyManageRow($_POST);
    $res = 'عملیات با موفقیت انجام شد';
    $out = "true";
    if (!$result) {
        $res = ' هیچ تغییری اعمال نگردید';
...
function showOvertimeLunchManageList()
{
    ...
    if ($page == 1) {
        ...
        $_SESSION['calcOvertimeLunch_p'] = $lunch_count_detailes['has_lunch_count_personel']; //$security->getOvertimeLunchManageListCountRows($sDate,$eDate,$personnel,$restaurant,$meal);
        $_SESSION['calcOvertimeLunch_g'] = $lunch_count_detailes['has_lunch_count_guest'];
    ...
    $totalRows = $_SESSION['calcOvertimeLunch_p'] + $_SESSION['calcOvertimeLunch_g'] + $_SESSION['calcOvertimeWhitoutLunch'] + $_SESSION['breakfast_count'];
    ...
    $pagerType = 1;
    $listTitle = '<sapn style="color:green;"> &nbsp &nbsp تعداد ناهارها(پرسنل) :&nbsp &nbsp' . $_SESSION['calcOvertimeLunch_p'] . " عدد " . ' &nbsp &nbsp &nbsp &nbsp</span>' .
        $listTitle = '<sapn style="color:#7c740a">&nbsp &nbspتعداد ناهارها (مهمان)  : ' . $_SESSION['calcOvertimeLunch_g'] . " عدد " . ' &nbsp &nbsp &nbsp &nbsp</span>' .
...
function doEditCreatePhone()
{
    ...
    $uti = new Utility();
    $phone = new Phone();
    list($cid, $name, $unit, $post, $phone1, $phone2, $phone3, $email, $mobile, $code, $phone_o) = $uti->varCleanInput(array('cid', 'name', 'unit', 'post', 'phone1', 'phone2', 'phone3', 'email', 'mobile', 'code', 'phone_o'));
    if (intval($cid) > 0) { //edit
        $res = $phone->editPhone($cid, $name, $unit, $post, $phone1, $phone2, $phone3, $email, $mobile, $code, $phone_o);
        if ($res) {
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res, $out);
        ...
        $res = $phone->createPhone($name, $unit, $post, $phone1, $phone2, $phone3, $email, $mobile, $code, $phone_o);
        if ($res) {
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res, $out);
...
function getLabelManagementListExcel()
{
    $acm = new acm();
    if (!$acm->hasAccess('labelManagement')) {
        die("access denied");
    ...
    $ut = new Utility();
    $res = $label->getLabelManagementList($labelNum, $hpCode, $piece, $status, $page, 'xls_report');
    $hdarray = array('ردیف', 'کد کالا', 'کد برچسب', 'نام برچسب', ' تاریخ تایید', 'تاریخ تغییر', 'فرمت', 'وضعیت', 'ثبت کننده', 'تایید کننده');
    $fieldNames = array('index', 'HPCode', 'labelNum', 'pName', 'statusDate', 'changeDate', 'printFormat', 'status', 'uid', 'tid');
    $name = "labelList" . date("y-m-d");
    $url = $ut->createExcel($hdarray, $res, $fieldNames, $name);
    if ($url) {
        //$res = "درخواست با موفقیت انجام شد";
        $out = "true";
...
function download_label_ftp()
{
    $ut = new Utility();
    $ftp = new FTP();
    list($file_name) = $ut->varCleanInput(array('file_name'));

    $res = $ftp->Download($file_name, true);
    $ut->fileRecorder('download.....' . $res);
    if ($res) {
        $res = $res;
        $out = "true";
        response($res, $out);
...
function log_arr($data, $comment = "coustom_log***")
{
    if (is_array($data)) {
        error_log($comment . print_r($data, true));
    } else {
...
{
    $acm = new acm();
    if (!$acm->hasAccess('recruitment_tests')) {
        die("access denied");
        exit;
    }
    $rate = new Rates();
    ...
    $res = $rate->getRecruitmentList($page);
    if ($page == 1) {
        $_SESSION['calcRecruitmentList'] = $rate->getRecruitmentListCountRows();
    }

    $totalRows = $_SESSION['calcRecruitmentList'];
    // $footerTxt = 'جمع کل هزینه ها به عدد = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;">'.number_format($_SESSION['calcTotalAmount']).'</span>&nbsp;&nbsp;ریال <br/> جمع کل هزینه ها در ماه به عدد = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;"> '.number_format($_SESSION['calcTotalMonthAmount']).'</span>&nbsp;ریال<br />جمع کل هزینه ها در روز به عدد = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;">'.number_format($_SESSION['calcTotalDayAmount']).'</span>&nbsp;&nbsp;ریال <br/> جمع کل هزینه ها در ساعت به عدد = <span style="font-family: dubai-Regular;font-weight: bold;color: green;font-size: 16px;"> '.number_format($_SESSION['calcTotalHourAmount']).'</span>&nbsp;ریال';
    $c = 0;
    $feilds = array();
    $feilds[$c]['title'] = "نام و نام خانوادگی";
    ...
    $feilds[$c]['title'] = "کد ملی";
    $feilds[$c]['width'] = "8%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "national_code";
    $c++;

    $feilds[$c]['title'] = "موبایل";
    $feilds[$c]['width'] = "10%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "mobile";
    $c++;

    $feilds[$c]['title'] = "تاریخ مصاحبه";
    $feilds[$c]['width'] = "10%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "inter_view_date";
    $c++;
    ...
    $feilds[$c]['f'] = "btn";
    $feilds[$c]['param'] = "RowID";
    $feilds[$c]['onclick'] = "editRecruitment";
    $feilds[$c]['icon'] = "fa-edit";
    $c++;

    $feilds[$c]['title'] = " آپلود/دانلود پیوست ها";
    $feilds[$c]['width'] = "10%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "btn";
    $feilds[$c]['param'] = "RowID";
    $feilds[$c]['onclick'] = "attch_recruitment_tests";
    $feilds[$c]['icon'] = "fa-file";

    $pagerType = 1;
    $listTitle = " تعداد موارد استخدامی : " . $totalRows . " عدد ";
    $tableID = "recruitmentBody-table";
    // $jsf = "showPersonnelManageList";
    $htm = $list->creat($listTitle, $res, $feilds, $totalRows, $pagerType, $page, $tableID, '', '');
    $out = "true";
    response($htm, $out);
    exit;
}
function getRecruitment()
{
    ...
    $ut = new Utility();
    $res = $rate->getRecruitment($_POST['r_id']);
    // $ut->fileRecorder($res);
    if (is_array($res) && count($res) > 0) {
        $out = "true";
        response($res, $out);
        ...
        $out = "false";
        response('موردی یافت نشد', $out);
        exit;
    }
}
function editCreateRecruitment()
{
    $acm = new acm();
    if (!$acm->hasAccess('recruitment_tests')) {
        die("access denied");
    ...
    $recruitment_test_date = !empty($_POST['recruitment_test_date']) ? $ut->jal_to_greg($_POST['recruitment_test_date']) : "";

    if ($rid) { //edit
        $res = $rate->editRecruitment($rid, $fullname, $mobile, $national_code, $description, $recruitment_test_date);
        if (res) {
            $out = "true";
            response('رکورد با موفقیت ویرایش شد', $out);
            exit;
        }
    } else { //create
        $res = $rate->createRecruitment($fullname, $mobile, $national_code, $description, $recruitment_test_date);
        if (res) {
            $out = "true";
            response('رکورد با موفقیت ایجاد  شد', $out);
            exit;
        }
    }
}

function doAttachFileToRecruitment()
{
    $uti = new Utility();
    $rate = new Rates();
    list($cid, $info) = $uti->varCleanInput(array('cid', 'info'));
    $files = $_FILES['files'];
    $res = $rate->attachFileToRecruitment($cid, $info, $files);
    // $res = $Comment->attachFileToComment($cid,$info,$files);
    if (intval($res) == -1) {
        $res = "فایل ها مشکل دارند !";
        $out = "false";
...
function get_recruitment_uploded_files()
{
    $acm = new acm();
    if (!$acm->hasAccess('recruitment_tests')) {
        die("access denied");
    ...
    $ut = new Utility();
    $cid = $_POST['cid'];
    $res = $rate->get_recruitment_uploded_files($cid);

    $out = "true";
    response($res, $out);
    exit;
}
function delete_recruitment_files()
{
    $acm = new acm();
    if (!$acm->hasAccess('recruitment_tests')) {
        die("access denied");
    ...
    $ut = new Utility();
    $cid = $_POST['RowID'];
    $res = $rate->delete_recruitment_files($cid);
    if ($res) {
        $out = "true";
        response($res, $out);
        exit;
...
function doEditCreateWarranty()
{
    ...
    $uti = new Utility();
    $documents = new Documents();
    list($w_id, $w_title, $w_acc_type, $w_type, $w_doc_owner, $w_doc_owner_nat_id, $w_account_id, $w_doc_info, $w_access_id, $w_start_date, $w_end_date, $desc, $warranty_cost) = $uti->varCleanInput(array('w_id', 'w_title', 'w_acc_type', 'w_type', 'w_doc_owner', 'w_doc_owner_nat_id', 'w_account_id', 'w_doc_info', 'w_access_id', 'w_start_date', 'w_end_date', 'desc', 'warranty_cost'));
    if (intval($w_id) > 0) {  //edit
        $res = $documents->editWarranty($w_id, $w_title, $w_acc_type, $w_type, $w_doc_owner, $w_doc_owner_nat_id, $w_account_id, $w_doc_info, $w_access_id, $w_start_date, $w_end_date, $desc, $warranty_cost);
        if ($res) {
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res, $out);
        ...
        $res = $documents->createWarranty($w_title, $w_acc_type, $w_type, $w_doc_owner, $w_doc_owner_nat_id, $w_account_id, $w_doc_info, $w_access_id, $w_start_date, $w_end_date, $desc, $warranty_cost);
        if ($res) {
            $res = "درخواست با موفقیت انجام شد.";
            $out = "true";
            response($res, $out);
...
function get_warranty_info()
{
    ...
    $documents = new Documents();
    $res = $documents->getWarrantyInfo($_POST['cid']);
    if ($res) {
        response($res, $out);
        exit;
    } else {
...
function showWarrantyManageList()
{
    $acm = new acm();
    if (!$acm->hasAccess('warrantyManage')) {
        die("access denied");
    ...
    $ut = new Utility();
    list($Name, $Code, $SDate, $EDate, $status, $type, $page) = $ut->varCleanInput(array('Name', 'Code', 'SDate', 'EDate', 'status', 'type', 'page'));
    $res = $documents->getWarrantyManageList($Name, $Code, $SDate, $EDate, $status, $type, $page);
    $ut->fileRecorder($res);
    if ($page == 1) {
        $_SESSION['calcWarrantyManage'] = $documents->getWarrantyManageListCountRows($Name, $Code, $SDate, $EDate, $status, $type);
    }
    $totalRows = $_SESSION['calcWarrantyManage'];
    if ($acm->hasAccess('editCreateNewWarranty') || $acm->hasAccess('attachFileToWarranty')) {
        $pt = array('checkBox' => 5, 'type' => 5, 'startDate' => 8, 'endDate' => 8, 'w_type' => 13, 'account_name' => 13, 'tafzili' => 10, 'users' => 10, 'w_cost' => 14, 'description' => 14, 'download' => 5, 'display' => 5);
    } else {
        $pt = array('checkBox' => 3, 'type' => 10, 'startDate' => 9, 'endDate' => 9, 'w_type' => 12, 'account_name' => 12, 'tafzili' => 10, 'w_cost' => 11, 'description' => 14, 'download' => 5, 'display' => 5);
    }
    ...
    $feilds[$c]['title'] = "انتخاب";
    $feilds[$c]['width'] = $pt['checkBox'] . "%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "checkBox";
    $feilds[$c]['class'] = "warrenty_select_row";
    $feilds[$c]['onclick'] = "select_warranty_row(this)";;
    $c++;

    $feilds[$c]['title'] = "نوع سند تضمین";
    $feilds[$c]['width'] = $pt['type'] . "%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "type";
    $c++;
    ...
    $feilds[$c]['title'] = "عنوان سند";
    $feilds[$c]['width'] = $pt['w_type'] . "%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "Name";
    $c++;

    $feilds[$c]['title'] = " نام طرف حساب";
    $feilds[$c]['width'] = $pt['account_name'] . "%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "account_name";
    $c++;

    $feilds[$c]['title'] = " کد تفضیلی";
    $feilds[$c]['width'] = $pt['tafzili'] . "%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "code_tafzili";
    $c++;

    $feilds[$c]['title'] = " مبلغ تضمین";
    $feilds[$c]['width'] = $pt['w_cost'] . "%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "warranty_cost";
    $c++;




    if ($acm->hasAccess('editCreateNewWarriant') || $acm->hasAccess('attachFileToWarriant')) {
        $feilds[$c]['title'] = "افراد مجاز جهت دانلود فایل";
        $feilds[$c]['width'] = $pt['users'] . "%";
        $feilds[$c]['order'] = "none";
        $feilds[$c]['f'] = "users";
    ...
    $feilds[$c]['f'] = "btn";
    $feilds[$c]['param'] = "RowID";
    $feilds[$c]['onclick'] = "downloadWarrantyFile";
    $feilds[$c]['icon'] = "fa-link";
    $c++;
    $feilds[$c]['title'] = "سایراطلاعات";
    $feilds[$c]['width'] = $pt['download'] . "%";
    $feilds[$c]['order'] = "none";
    $feilds[$c]['f'] = "btn";
    $feilds[$c]['param'] = "RowID";
    $feilds[$c]['onclick'] = "display_sale_warranty";
    $feilds[$c]['icon'] = "fa-desktop";

    $pagerType = 1;
    $listTitle = " تعداد تضامین : " . $totalRows . " عدد ";
    $tableID = "WarrantyManageBody-table";
    $jsf = "showWarrantyManageList";
    $htm = $list->creat($listTitle, $res, $feilds, $totalRows, $pagerType, $page, $tableID, $jsf);
    $out = "true";
    response($htm, $out);
    exit;
...
function getAttachedWarrantyFile()
{
    $ut = new Utility();
    list($cid) = $ut->varCleanInput(array('cid'));
    if (!intval($cid)) {
    ...
    $documents = new Documents();
    $res = $documents->attachedWarrantyFileHtm($cid);
    $out = "true";
    response($res, $out);
    exit;
}

function display_sale_warranty()
{
    $ut = new Utility();
    $documents = new Documents();
    $cid = $_POST['w_id'];
    ...
    $documents = new Documents();
    $res = $documents->getOtherInfoWarrantyHtm($cid);
    $out = "true";
    response($res, $out);
    exit;
}
...
function doAttachFileToWarranty()
{
    ...
    $documents = new Documents();
    list($cid, $info) = $ut->varCleanInput(array('cid', 'info'));
    $files = $_FILES['files'];
    $res = $documents->attachFileToWarranty($cid, $info, $files);
    if (intval($res) == -1) {
        $res = "فایل ها مشکل دارند !";
        $out = "false";
        response($res, $out);
        ...
        $res = "حجم فایل باید کمتر از 5مگابایت باشد";
        $out = "false";
        response($res, $out);
        exit;
    } elseif ($res == true) {
...
function downloadWarrantyFile()
{
    ...
    $documents = new Documents();
    $res = $documents->attachmentFileWarrantyHtm($cid);
    $out = "true";
    response($res, $out);
    exit;
}

function final_confirm_sale_warranty()
{
    $ut = new Utility();
    list($wid) = $ut->varCleanInput(array('wid'));
    if (!intval($wid)) {
        $res = "شناسه بدرستی دریافت نشد !";
        $out = "false";
        response($res, $out);
    ...
    $event = new Events();
    $res = $event->final_confirm_sale_warranty($wid);
    $ut->fileRecorder($res);
    $out = "true";
    response($res, $out);
    exit;
}
function deleteAttachWarrantyFile()
{
    $uti = new Utility();
    $documents = new Documents();
    list($fid) = $uti->varCleanInput(array('fid'));
    $res = $documents->deleteAttachWarrantyFile($fid);
    if ($res) {
        $res = 'با موفقیت حذف گردید.';
        $out = "true";
        response($res, $out);
...
function dodeleteWarranty()
{
    ...
    $ut = new Utility();
    $documents = new Documents();
    list($cid, $comment) = $ut->varCleanInput(array('cid', 'comment'));
    if (!intval($cid)) {
        $res = "شناسه بدرستی دریافت نشد !";
        $out = "false";
        response($res, $out);
    ...
    $res = $documents->deleteWarranty($cid, $comment);
    if ($res) {
        $res = "درخواست شما با موفقیت انجام گردید.";
        $out = "true";
        response($res, $out);
...
function doEndWarranty()
{
    ...
    $res = $documents->EndWarranty($cid, $comment);
    if ($res) {
        $res = "درخواست شما با موفقیت انجام گردید.";
        $out = "true";
        response($res, $out);
...
function get_end_date_warranty()
{
    ...
    $res = $documents->get_end_date_warranty($cid);
    if ($res) {
...
function get_sms_panel_info()
{
    $ut = new Utility();
    $sms = new SMS();
    $res = $sms->get_sms_panel_info();
    if ($res) {
        response($res, 'true');
        exit;
    } else {
...
function pay_comment_report_status_list()
{
    $ut = new Utility();
    $comm = new Comment();
    $res = $comm->pay_comment_report_status_list($_POST);
    if ($res) {
        response($res, 'true');
        exit;
    } else {
