...
function showReceivedCustomerManageList()
{
    ...
    if ($acm->hasAccess('receivedCustomerCheckManage')) {
        $feilds[$c]['title'] = "مدیریت مالی چک";
        $feilds[$c]['width'] = "7%";
        $feilds[$c]['order'] = "none";
        $feilds[$c]['f'] = "btn";
        $feilds[$c]['param'] = "RowID";
        $feilds[$c]['onclick'] = "ShowCheckManage";
        $feilds[$c]['disabled'] = "yes";
        $feilds[$c]['icon'] = "fa-edit";
    }
    if ($acm->hasAccess('receivedCustomerCheckManage')) {
...
function doEditCreateReceivedCustomer()
{
    ...
    $Comment = new Comment();
    $ut = new Utility();
    list($rid, $RType, $RMethod, $amount, $rDate, $CstName, $CstCode, $desc, $bank, $pos, $serial, $chDate, $chOwner, $chOwnerC,$chBankIssuingBank,$is_sayyad,$depositDate) = $ut->varCleanInput(array('rid', 'RType', 'RMethod', 'amount', 'rDate', 'CstName', 'CstCode', 'desc', 'bank', 'pos', 'serial', 'chDate', 'chOwner', 'chOwnerC','chBankIssuingBank','is_sayyad','depositDate'));
    $amount = str_replace(',', '', $amount);
    if (intval($rid) > 0) {  //edit
        $res = $Comment->editReceivedCustomer($rid, $RType, $RMethod, $amount, $rDate, $CstName, $CstCode, $desc, $bank, $pos,$chBankIssuingBank,$is_sayyad,$depositDate);
        if ($res != false) {
            $res = 'درخواست با موفقیت انجام شد.';
            $out = "true";
            response($res, $out);
        ...
        $res = $Comment->createReceivedCustomer($RType, $RMethod, $amount, $rDate, $CstName, $CstCode, $desc, $bank, $pos, $serial, $chDate, $chOwner, $chOwnerC,$chBankIssuingBank,$is_sayyad,$depositDate);
        if ($res != false) {
            $res = 'درخواست با موفقیت انجام شد.';
            $out = "true";
            response($res, $out);
